...
function age(c){ 
  ...
  let a = y - c.byear; 
  if (m < c.bmonth || (m===c.bmonth && state.day < c.bday)) a--; 
  return a; 
}
...
function vassalRevolt(v, liege){
  v.liege=null; chOp(v,liege,-50); chOp(liege,v,-50);
  ...
  if(v.id===state.player||liege.id===state.player){
    popup({title:'봉신 반란!', sub:'정치 위기',
      ...
      opts:[{t:'진압 전쟁 선포', f:()=>{ if(liege.id===state.player) addClaim(directCountiesOf(v.id)[0],'revenge'); }},
...
function monthlyPulse(){
  ...
  if(state.month===1){ claimExpirePulse(); opinionDecayPulse(); }
...
function goldPulse(){
  for(const bid in BARONIES) BARONIES[bid].troops=Math.min(BARONIES[bid].cap, BARONIES[bid].troops+4);
  ...
  for(const id in chars){
    ...
    if(id===state.player||c.ruler){
      for(const v of vassalsOf(id)){
        const opn=Math.max(0,opinion(v,c)+100)/200; const taxMul=TAX_RATE*opn;
        const vOwned=regionsOf(v.id); const vSeat=vOwned.includes(v.region)?v.region:vOwned[0];
        const vSeatB=vSeat?BARONIES[vSeat]:null; if(!vSeatB) continue;
...
function fertilityPulse(){
  for(const id in chars){
    ...
    const chance=(fert(c)+fert(h))/2 * 0.0475;
    if(Math.random()<chance){ c.pregnant=1; if(isPlayerFamily(c)||c.id===state.player||h.id===state.player) log(`<b>${c.name}</b>이(가) 회임했습니다.`,'fam'); }
  }
}
...
function naturalDeathPulse(){
  ...
  for(const id in chars){
    ...
    if(c.id===state.player){
      popup({title:'노환', sub:'건강', body:`${c.name}이(가) ${a}세의 나이에 조용히 눈을 감았습니다.`, opts:[{t:'...', f:()=>kill(c,'노환')}]});
    } else {
      if(c.spouse===state.player||chars[c.spouse]?.spouse===state.player||c.id===p.spouse){
        ...
        popup({title:'배우자의 죽음', sub:'가문', body:`${c.name}이(가) 조용히 눈을 감았습니다.`, opts:[{t:'명복을 빈다', f:()=>{ if(p.spouse===c.id) p.spouse=null; }}]});
...
function giveBirth(c){
  ...
  } else if(h&&h.ruler){ log(`${REGIONS[h.region].n}의 궁정에 아이가 태어났습니다.`,'dip'); }
}
...
function schemePulse(){
  state.schemes=state.schemes.filter(s=>{
    ...
    const discover=0.07+stat(t,'intr')*0.008; const roll=Math.random();
    if(roll<succ){
      log(`<b>${t.name}</b>이(가) 의문의 죽음을 맞았습니다. 독이 든 술잔이었습니다.`,'war');
      if(s.target===state.player){ popup({title:'독배', sub:'암살', body:'연회의 술잔에 독이 들어 있었습니다...', opts:[{t:'...', f:()=>kill(t,'독살')}]}); } else kill(t,'독살');
      return false;
    }
    if(roll<succ+discover){
      log(`<b>${p.name}</b>의 살해 모략이 발각되었습니다!`,'war'); chOp(t,p,-50);
...
function warPulse(){
  state.wars=state.wars.filter(w=>{
    ...
    if(newAtk!==w.atk){ w.atk=newAtk; } if(newDef!==w.def){ w.def=newDef; }
    ...
    if(tCid&&COUNTIES[tCid]){
      ...
      if(capHolder&&capHolder.id!==w.def&&capHolder.id!==w.atk){
        if(w.atk===state.player||w.def===state.player){
          popup({title:'전쟁 목표 상실', sub:'전황 변화', body:`제3자가 영지를 먼저 선점했습니다.`, opts:[
    ...
    if(tCid&&COUNTIES[tCid]){
      ...
      if(ratio>0.1 && Math.random()<0.25+ratio*0.3){ const un=bids.filter(bid=>!w.occupied.includes(bid)); if(un.length) w.occupied.push(un[Math.floor(Math.random()*un.length)]); }
    ...
    if(w.months%3===0&&(w.atk===state.player||w.def===state.player)){
      const my=w.atk===state.player?w.score:-w.score; log(`전황: ${my>0?'+':''}${Math.round(my)}% · 공성 ${Math.round(siegePct*100)}%`,'war');
    }
...
function conquerTarget(a, d, targetCid){
  let cid = targetCid; if(BARONIES[targetCid]) cid = BARONIES[targetCid].county;
  ...
  if(a.id===state.player){
    ...
    popup({title:'정복', sub:cname, body:`<b>${cname}</b>이(가) 당신의 깃발 아래 들어왔습니다!`, opts:[
      ...(canVassal?[{t:`${d.name}을 봉신으로 삼는다`, d:'영지 유지 및 세금 상납 수취', f:()=>{ d.liege=a.id; chOp(d,a,20); log(`${d.name}이(가) 봉신이 됐습니다.`,'dip'); checkVictoryHint(); }}]:[]),
      {t:'국고 확인', f:checkVictoryHint}
    ]});
  }
  if(d.id===state.player&&!d.region){ gameOver(`${cname}이(가) 함락되었습니다. 왕좌를 빼앗겼습니다.`); }
  renderMap();
}
...
function checkVictoryHint(){ const n=playerRegions().length; if(n>=7) return; if(n>=4) log(`현재 ${n}개 왕국 지배 중. [결단] 메뉴 확인.`,'good'); }
...
function buildNpcCouncil(ruler){
  ...
  for(const role in COUNCIL_ROLES){
    const cur = ruler.council[role]; if(cur && chars[cur] && !chars[cur].dead) continue;
    ruler.council[role] = null; const assigned = councilAssignedIds(ruler); const sk = COUNCIL_ROLES[role].skill;
...
function appointCouncilor(role, charId){
  if(charId){
    for(const r in state.council){ if(r!==role && state.council[r]===charId){ log(`${chars[charId].name}은(는) 이미 다른 임무 중입니다.`,'dip'); renderCourt(); return; } }
  }
  const prev = state.council[role]; if(prev && chars[prev]) chOp(chars[prev], playerChar(), -10);
    ...
    if(role==='chancellor'){
      ...
      if(Math.random()<0.08){
        if(positive){
          ...
          if(rnd){ chOp(rnd,p, Math.round(sk*0.8)); popup({title:'재상의 외교', body:`${c.name}이(가) 이웃과의 관계를 개선했습니다.`, opts:[{t:'치하한다'}]}); }
        } else { state.prestige=Math.max(0,state.prestige-10); popup({title:'외교 마찰', body:`재상이 교역 담판 중 실언을 범했습니다.`, opts:[{t:'수습한다'}]}); }
      }
    }
    if(role==='marshal'){ reg.troops = Math.min(reg.cap, reg.troops + Math.round(sk*1.5)); if(Math.random()<0.08 && positive){ popup({title:'무구 정비', body:`군대가 정비 보강되었습니다.`, opts:[{t:'훌륭하다', f:()=>{reg.troops+=50;}}]}); } }
    if(role==='steward'){ reg.gold += Math.round(sk * 1.2); if(Math.random()<0.08 && positive){ const b=30+sk*4; popup({title:'추가 세수', body:`재무관이 조세를 긴밀히 징수했습니다.`, opts:[{t:'수고했다', f:()=>{reg.gold+=b; playSynthSFX('gold');}}]}); } }
...
function npcCouncilPulse(){
  for(const id in chars){
    ...
    if(r.council.chancellor && Math.random()<0.06){
      ...
      const targets = adjCids.filter(cid=>{ const holder=countyHolder(cid); return holder && holder.id!==r.id && !r.claims.find(c=>c.rid===cid); });
      if(targets.length && Math.random() < 0.25){ const cid = targets[Math.floor(Math.random()*targets.length)]; r.claims.push({rid:cid, type:'unpressed', obtained:state.year}); if(countyHolder(cid)?.id===state.player) log(`${r.name}의 재상이 <b>${COUNTIES[cid].n}</b>에 명분을 조작했습니다!`,'war'); }
    }
    if(r.council.marshal){ const m=chars[r.council.marshal]; const msk= !m?5:stat(m,'mar'); REGIONS[r.region].troops=Math.min(REGIONS[r.region].cap, REGIONS[r.region].troops+Math.round(msk*0.8)); }
    if(r.council.steward){ const s=chars[r.council.steward]; const ssk= !s?5:stat(s,'stew'); REGIONS[r.region].gold=Math.min(2000, REGIONS[r.region].gold+Math.round(ssk*0.6)); }
  }
}

/* ---------- 이웃 상호작용 및 활동 주기 ---------- */
const NPC_ACTIVITIES = [
  { id:'feast', n:'연회', icon:'🍖', cond:(r,reg)=>reg.gold>80, run:(r,reg,adj)=>{ reg.gold -= 60; r.lastActivity = state.year; const guests = adj.filter(t=>t&&t.id!==r.id&&!t.dead); const invited = guests.sort(()=>Math.random()-0.5).slice(0,1); invited.forEach(g=>{ chOp(g,r,10); chOp(r,g,7); }); if(invited.some(g=>g.id===state.player) && Math.random()<0.5){ popup({title:`${r.name}의 연회 초대`, body:`궁정 연회 독려 서신이 당도했습니다.`, opts:[{t:'참석', d:'관계 +15, 스트레스 -10', f:()=>{ chOp(r,playerChar(),15); addStress(playerChar(),-10,'연회 여흥'); }}, {t:'거절', f:()=>chOp(r,playerChar(),-5)}]}); } } },
  { id:'hunt', n:'사냥', icon:'🦌', cond:(r,reg)=>reg.gold>20, run:(r,reg,adj)=>{ reg.gold -= 15; r.lastActivity = state.year; const partner = adj.find(t=>t&&t.id!==r.id&&!t.dead&&opinion(r,t)>-20); if(partner&&partner.id===state.player){ popup({title:`사냥 동행`, body:`${r.name}이 가을 숲 추적을 제안합니다.`, opts:[{t:'동행', f:()=>{ chOp(r,playerChar(),10); addStress(playerChar(),-8,'사냥'); }}, {t:'거부', f:()=>chOp(r,playerChar(),-5)}]}); } } },
  { id:'pilgrimage', n:'순례', icon:'✝', cond:(r)=>Math.random()<0.2, run:(r,reg)=>{ r.lastActivity = state.year; reg.pop = Math.min(100,(reg.pop||60)+6); } },
  { id:'diplomacy', n:'외교 방문', icon:'🤝', cond:(r,reg,adj)=>adj.some(t=>t&&opinion(r,t)>0), run:(r,reg,adj)=>{ r.lastActivity = state.year; const target = adj.find(t=>t&&t.id!==r.id&&opinion(r,t)>0); if(target&&target.id===state.player){ popup({title:'궁정 방문 사절', body:`${r.name} 소왕이 친히 예방했습니다.`, opts:[{t:'환대', d:'관계 +15, 금 -20', f:()=>{ if(REGIONS[playerChar().region].gold>=20) REGIONS[playerChar().region].gold-=20; playSynthSFX('gold'); chOp(r,playerChar(),15); }}, {t:'방치', f:()=>chOp(r,playerChar(),5)}]}); } } }
];
...
function npcActivityPulse(){
  for(const id in chars){
    ...
    const possible = NPC_ACTIVITIES.filter(a=>a.cond(r,reg,adj)); if(!possible.length) continue;
    possible[Math.floor(Math.random()*possible.length)].run(r, reg, adj);
  }
}

function aiPulse(){
  const p=playerChar(); npcCouncilPulse(); npcActivityPulse();
  ...
  // 임시 정의 (크래시 방지용 시스템 펄스 더미)
  if(typeof popPulse === 'function') popPulse();
}
...
/* ---------- 명분 조작 및 CB 라벨 정의 ---------- */
const CB_TYPES = {
  pressed:   { n:'확정 주장',   icon:'⚔',   cost:50,  desc:'전쟁 증명 주장 영토 명분', color:'#c9a227' },
  unpressed: { n:'미확정 주장', icon:'📜', cost:100, desc:'위조된 문서 명분',         color:'#8a9a6a' },
  revenge:   { n:'복수 선포',   icon:'🩸', cost:0,   desc:'탈환 목적 강제 반격 명분',  color:'#9e3535' },
};

function hasClaim(rid){ return state.claims.find(c=>c.rid===rid); }
function claimsForRegion(def){ const rids = regionsOf(def.id); return state.claims.filter(c=>rids.includes(c.rid)); }
function claimName(rid){ return COUNTIES[rid]?.n||BARONIES[rid]?.n||rid; }
function addClaim(rid, type){ if(hasClaim(rid)) return; state.claims.push({rid, type, obtained:state.year}); log(`<b>${claimName(rid)}</b> 명분을 확보했습니다.`, 'dip'); }
...
function npcHasClaim(r, targetRid){ return r.claims.some(c=>c.rid===targetRid); }
...
function openDeclareWar(defId){
  ...
  if(truceBetween(p.id, def.id)){ showModal({title:'휴전 협정 결속 중', body:`협정 종료 연도까지 개전이 불가능합니다.`, opts:[{t:'닫기'}]}); return; }
  const myClaims = claimsForRegion(def);
  if(myClaims.length === 0){ showModal({title:'정당한 명분 부재', body:`재상을 임명하여 공작 명분을 위조하십시오.`, opts:[{t:'닫기'}]}); return; }
  const defPow = Math.round(power(def)); const myPow = Math.round(power(p));
  let html = `<div class="kv"><span>전력 비조율</span><span>${defPow} vs 아군 ${myPow}</span></div>`;
  const opts = [];
  myClaims.forEach(cl=>{
    const cbInfo = CB_TYPES[cl.type]; const canAfford = state.prestige >= cbInfo.cost; const rid = cl.rid;
    opts.push({
      t:`${cbInfo.icon} ${cbInfo.n} — ${claimName(rid)}`, d:`위신 ${cbInfo.cost} 소모`,
      f: canAfford ? ()=>{ state.prestige -= cbInfo.cost; if(cl.type==='unpressed') cl.type='pressed'; declareWar(p, def, rid); removeClaim(rid); } : ()=>{}
    } );
  });
  opts.push({t:'취소'}); showModal({title:`선전포고 구성`, html, opts});
}

function chancellorFabricateClaim(sk){
  const p = playerChar(); if(!p||!REGIONS[p.region]) return;
  const adj = ADJ[p.region] || []; const targets = adj.filter(rid=>{ const o=ownerOf(rid); return o && o.id!==p.id && !hasClaim(rid); }); if(!targets.length) return;
  const rid = targets[Math.floor(Math.random()*targets.length)];
  if(Math.random() < (0.3 + sk * 0.05)){ addClaim(rid, 'unpressed'); popup({title:'문서 위조 달성', body:`재상이 ${claimName(rid)}의 고문서 조작에 성공했습니다.`, opts:[{t:'확인'}]}); }
  else { const o=ownerOf(rid); if(o) chOp(o,p,-20); popup({title:'공작 공작 발각', body:`위조 모의가 사전에 간파당했습니다.`, opts:[{t:'분하다'}]}); }
}
...
function openCounty(cid){
  ...
  if(war&&(war.atk===state.player||war.def===state.player)){
    ...
    const lines=bids.map(bid=>`${BARONIES[bid]?.n||bid}: ${occ.includes(bid)?'⚔ 제압됨':'🛡 수성중'}`).join('\n');
    popup({title:`${cnt.n} 전선`, body:`압박 전황 지수: ${Math.round(my)}%\n\n${lines}`, opts:[{t:'닫기'}]}); return;
  }
...
function openRegion(rid, cid_hint){
  initAudio();
  const c=ownerOf(rid); if(!c) return; const p=playerChar();
  const dispName = cid_hint ? COUNTIES[cid_hint]?.n : (BARONIES[rid]?.n || rid);
  if(c.id===p.id){ showModal({title:dispName, sub:'내 영토 영지', body:`가용 병력 ${REGIONS[rid].troops} · 금고 수지 ${Math.round(REGIONS[rid].gold)}`, opts:[{t:'닫기'}]}); return; }
  const op=opinion(c,p); const myOp=opinion(p,c); const atWar=state.wars.some(w=>(w.atk===p.id&&w.def===c.id)||(w.atk===c.id&&w.def===p.id)); const truce=truceBetween(p.id,c.id);
  let html = `<div class="kv"><span>지배 영주</span><span>${c.name} (${age(c)}세)</span></div>
              <div class="kv"><span>나를 향한 시선</span><span class="${op>15?'relGood':op<-15?'relBad':'relMid'}">${op>0?'+':''}${op}</span></div>`;
  const opts = [];
  if(!atWar){
    opts.push({t:'선물 증정', d:'금 50 소모 — 신뢰 +15', f:()=>{
      if(REGIONS[p.region].gold<50) return; REGIONS[p.region].gold-=50; chOp(c,p,15); playSynthSFX('gold'); 
    }});
    // 더미 스텁 구현들 (크래시 및 미정의 예방 안전장치)
    opts.push({t:'동맹 제안 결속', f:()=>{ if(!isAllied(p.id,c.id)){ formAlliance(p.id,c.id); chOp(c,p,10); } }});
    opts.push({t:'봉신화 요구', f:()=>{} });
    opts.push({t:'혼인 조약 검토', f:()=>{} });
    if(!truce){ if(!isAllied(p.id,c.id)){ const myClaims=claimsForRegion(c); opts.push({t:myClaims.length>0?'선전포고 집행':'무단 개전 선포 (위신 손실)', f:()=>{ closePanel('court'); closePanel('dec'); openDeclareWar(c.id); }}); } }
  }
  opts.push({t:'닫기'}); showModal({title:`${dispName} 왕국 권역`, html, opts});
}

/* ---------- 자문회 동적 렌더링 부하 분산 (궁정 패널) ---------- */
const COUNCIL_ROLES = {
  chancellor: { n:'재상',    skill:'dip',   icon:'⚖', desc:'외교 조율 및 명분 문서 조작 위조' },
  marshal:    { n:'원수',    skill:'mar',   icon:'⚔', desc:'군사 상비군 훈련 및 병력 증강 충원' },
  steward:    { n:'재무관',  skill:'stew',  icon:'💰', desc:'조세 행정 및 직할 수지 금 수취' },
  spymaster:  { n:'첩보관',  skill:'intr',  icon:'🗡', desc:'밀정 네트워크 방어 및 모략 탐색' },
  chaplain:   { n:'사제',    skill:'learn', icon:'✝', desc:'교구 민심 정화 및 위신 명예 축적' },
};
...
function renderCourt(){
  ...
  const roleBonus={
    chancellor: cid=>{ const sk=stat(chars[cid],'dip'); return `위신 +${Math.round(sk*0.4)}/월 · 명분 탐색 중`; },
    marshal:    cid=>{ const sk=stat(chars[cid],'mar'); return `상비군 +${Math.round(sk*1.5)}/월`; },
    steward:    cid=>{ const sk=stat(chars[cid],'stew'); return `직할 금 +${Math.round(sk*1.2)}/월`; },
    spymaster:  cid=>{ const sk=stat(chars[cid],'intr'); return `비밀 방어력 +${sk*2}`; },
    chaplain:   cid=>{ const sk=stat(chars[cid],'learn'); return `민심 보강 · 위신 안정`; },
  };
  let html=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid var(--line)">자문회 직무 배정</div>`;
  for(const role in COUNCIL_ROLES){
    ...
    html+=`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px dotted #2c2316">
      <span style="font-size:1.1rem;width:22px;text-align:center">${rinfo.icon}</span><div style="flex:1"><div style="font-size:.82rem;">${rinfo.n}</div></div><div style="text-align:right">`;
    if(councilor){
      html+=`<div style="font-size:.8rem;color:var(--gold)">${councilor.name}</div><div style="font-size:.68rem;color:#7a9a6a;">${roleBonus[role](councilor.id)}</div><button class="p-action" style="margin-top:2px;padding:2px 6px;font-size:.7rem" onclick="appointCouncilor('${role}',null)">해임</button>`;
    } else {
      html+=`<div style="font-size:.75rem;color:var(--parch-dim)">공석</div>`;
      if(candidates.length){
        html+=`<select style="background:#191309;color:var(--parch);border:1px solid var(--line);font-size:.7rem" onchange="if(this.value)appointCouncilor('${role}',this.value)"><option value="">선택...</option>`;
        candidates.forEach(c=>{ html+=`<option value="${c.id}">${c.name} (핵심:${stat(c,rinfo.skill)})</option>`; }); html+=`</select>`;
      }
    }
    html+=`</div></div>`;
  }
  ...
  if(myVassals.length>0){
    html+=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin:10px 0 6px;border-bottom:1px solid var(--line)">영지 봉신 서약 현황</div>`;
    myVassals.forEach(v=>{ html+=`<div class="p-row"><span><b>${v.name}</b></span><span>호감도:${opinion(v,p)}</span></div>`; });
  }

  html+=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin:12px 0 8px;border-bottom:1px solid var(--line)">궁정 내부 인적 자원 목록</div><table style="width:100%">`;
  fam.forEach(c=>{
    const rel=c.id===p.id?'본인':c.spouse===p.id?'배우자':(c.father===p.id||c.mother===p.id)?'자녀':'궁정인';
    html+=`<tr style="border-bottom:1px dotted #2c2316"><td style="padding:6px 2px"><b>${c.name}</b><br><small>${rel} · ${age(c)}세</small></td><td style="font-size:.75rem;text-align:right">외${stat(c,'dip')} 무${stat(c,'mar')} 내${stat(c,'stew')} 음${stat(c,'intr')} 학${stat(c,'learn')}</td></tr>`;
  });
...
function renderDec(){
  ...
  if(n>=4&&!state.kingdomFormed){
    addDec('⚜ 아일랜드 왕국 대관 선포',`금 250 소모 (보유:${Math.round(REGIONS[p.region].gold)}) · 위신 +200`, REGIONS[p.region].gold>=250, ()=>{
      REGIONS[p.region].gold-=250; state.kingdomFormed=true; state.prestige+=200; playSynthSFX('gold'); closePanel('dec');
      popup({title:'아일랜드 왕관 회복', body:'캐셸의 바위 성탑 위에서 장엄한 하이킹 축하 미사가 거행됩니다.', opts:[{t:'영광을 신께'}]});
    });
  }
  if(playerDuchies().length>=7){ addDec('☀ 아일랜드 통합 대 하이킹 등극','전 영토 판도 제압 승리 달성', true, ()=>{ closePanel('dec'); victory(); }); }

  addDec('대연회 개최',`금 60 소모 · 스트레스 -20 · 전 지배자 호감도 증가`, REGIONS[p.region].gold>=60, ()=>{
    REGIONS[p.region].gold-=60; addStress(p,-20,'연회 여흥'); playSynthSFX('gold');
    for(const r in REGIONS){ const o=ownerOf(r); if(o&&o.id!==p.id) chOp(o,p,8); } renderDec();
  });
  addDec('클론맥노이즈 대성당 순례',`금 30 소모 · 스트레스 -25 · 위신 축적`, REGIONS[p.region].gold>=30, ()=>{
    REGIONS[p.region].gold-=30; addStress(p,-25,'순례 안정'); state.prestige+=10; playSynthSFX('gold'); renderDec();
  });
  addDec('중갑 병력 추가 징집',`금 80 소모 · 정규 상비 징집대 +200`, REGIONS[p.region].gold>=80, ()=>{
    REGIONS[p.region].gold-=80; REGIONS[p.region].troops+=200; playSynthSFX('gold'); renderDec();
  });
  ...
  if(myCids.length>1){
    ...
    if(courtierCands.length>0){
      addDec('백작령 작위 하사 결단',`궁정 가문원에게 직할 지분 위임`, true, ()=>{
        const cOpts=myCids.filter(cid=>cid!==countyOf(p.region)).map(cid=>({
          t:COUNTIES[cid].n, d:`속령 권역 이전`,
          f:()=>{
            const vOpts=courtierCands.slice(0,4).map(v=>({ t:v.name, f:()=>{ grantCountyToVassal(p.id, v.id, cid); renderDec(); } }));
            vOpts.push({t:'취소'}); showModal({title:'봉신 위임 책봉', opts:vOpts});
          }
        }));
        cOpts.push({t:'취소'}); showModal({title:'하사 권역 지정', opts:cOpts});
      });
    }
  }
  ...
  if(courtSize<4){
    addDec('海外 역량 인재 영입',`금 80 소모 · 유능한 기사/행정관 수수 영입`, REGIONS[p.region].gold>=80, ()=>{
      REGIONS[p.region].gold-=80; playSynthSFX('gold');
      const recruit=mk({name:randName(), dyn:'방랑파', byear:state.year-30, traits:randTraits(2), base:randStats(), edu:2, eduFocus:'mar', courtOf:p.region});
      log(`새로운 인재 [<b>${recruit.name}</b>]이 궁정에 당도했습니다.`,'good'); renderDec(); renderCourt();
    });
  }

  let html = `<p style="font-size:.74rem;color:var(--parch-dim);margin-bottom:8px">판도 지배 백작령: ${myCids.length} 보유</p>`;
...
function renderChar(){
  ...
  const ttl=state.kingdomFormed?'아일랜드 고등왕':`${REGIONS[c.region]?REGIONS[c.region].n:''} 소왕`;
  ...
  if(c.lifestyle) chips+=`<span class="chip life">${SKILLS[c.lifestyle]}의 길</span>`; document.getElementById('cChips').innerHTML=chips;
  ...
  const lv=stressLvl(c); const el=document.getElementById('stressLvl'); el.textContent=['0단계 — 평온함','1단계 — 압박 불안','2단계 — 위험 한계','3단계 — 붕괴 위기'][lv];
...
function renderMap(){
  ...
  for(const cid in COUNTY_ADJ){
    COUNTY_ADJ[cid].forEach(nb=>{
      ...
      const inWar=state.wars.some(w=>{ const aH=countyHolder(cid),bH=countyHolder(nb); return aH&&bH&&((w.atk===aH.id&&w.def===bH.id)||(w.atk===bH.id&&w.def===aH.id)); });
      h+=`<line class="edge${inWar?' warEdge':''}" x1="${A.x}" y1="${A.y}" x2="${B.x}" y2="${B.y}"/>`;
    });
  }
  for(const cid in COUNTIES){
    const C=COUNTIES[cid]; const holder=countyHolder(cid); const mine=holder&&holder.id===p.id; const isVassal=holder&&holder.liege===p.id;
    const col=mine?'#3d6b4a':isVassal?'#4a7a55':(DUCHIES[C.duchy]?.color||'#555'); const stroke=mine?'#c8a24a':isVassal?'#6aaa7a':'#6a5836';
    const totalT=C.baronies.reduce((s,b)=>s+(BARONIES[b]?.troops||0),0); const avgPop=Math.round(C.baronies.reduce((s,b)=>s+(BARONIES[b]?.pop||60),0)/C.baronies.length);
    const underSiege=state.wars.some(w=>w.targetRid===cid&&w.occupied?.length>0); const rad=mine?21:17;
    h+=`<g class="node" onclick="openCounty('${cid}')">
      <circle class="body" cx="${C.x}" cy="${C.y}" r="${rad}" fill="${col}" stroke="${underSiege?'#c83030':stroke}"/>
      ${mine?`<circle cx="${C.x}" cy="${C.y}" r="${rad+5}" fill="none" stroke="#c8a24a" stroke-width="1" stroke-dasharray="2 4"/>`:''}
      <text x="${C.x}" y="${C.y+3}" style="font-size:9px">${C.n}</text>
      <text class="owner" x="${C.x}" y="${C.y+14}" style="font-size:7px;fill:#8a7858">${holder?holder.name.split(' ')[0]:'—'}</text>
      <text class="owner" x="${C.x}" y="${C.y+34}" style="font-size:7px;fill:#7a6848">⚔${totalT} 민${avgPop}</text>
    </g>`;
  }
  svg.innerHTML=h;
}
...
/* ---------- 서사 시작 및 초기 기동 ---------- */
function intro(){
  popup({title:'에이레, 1066년 연대기', sub:'브리언 보루의 유산',
    body:`삼왕이 왕관을 두고 격돌하는 잉글랜드와 달리, 바다 건너 이 섬은 파편화된 채 주인을 기다리고 있습니다.\n\n당신은 무르하드 막 돈하드 — 먼스터의 소왕.\n일곱 왕국을 규합하여 타라 언덕의 하이킹 왕좌를 복원하십시오.`,
    opts:[{t:'연대기 시작', f:()=>{ initAudio(); askLifestyle(playerChar()); }}]});
}

// 부가 유틸리티 난수 생성 스텁 (미배정 보정 루틴)
function randName(){ return ['도나하','셰클런','루어르크','돈하드','터를록'][Math.floor(Math.random()*5)]; }
function randTraits(n){ return ['brave','calm','diligent','just'].slice(0,n); }
function randStats(){ return {dip:6,mar:6,stew:6,intr:6,learn:6,prow:6}; }
...
log('지도의 백작령 노드를 클릭하면 외교 및 공성 상태를 확인할 수 있습니다.','dip');
intro();
