...
const TRAITS = {
  ...
  wrathful:  {n:'다혈질',   opp:'calm',      mod:{dip:-1,mar:3,intr:-1}, ai:{venge:2,bold:1}},
...
/* ---------- 교육 특성 ---------- */
const SKILLS = {dip:'외교력', mar:'전투력', stew:'관리력', intr:'계책력', learn:'학습력'};
const EDU_NAMES = {
...
/* ══════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════ */
/* ════════════════════════════════════════════════════
   건물 시스템 — CK3 충실 재현 (v74: v73 시스템 + 1단계 지형·시대)
   출처: CK3 Wiki Building/Military_buildings/Castle_buildings(v1.18~1.19)
         지형수치 ck3cheats.com·CK3 Wiki Modifier list / 한글명 나무위키 크킹3·건물
   구조: 지형(전투 유리함·보급·개발) + 보유지 유형/등급 + 시대 + 라인별 8단계
   ── 수치 환산(SCALE) — 게임 경제(병력 수백·금 수십) 규모로 정규화 ──
     · 건설비: CK3 150/250/340/500/750/1110/1600/2240 ÷5
     · 수비대: CK3 톤(요새 라인 +250~) ÷10
     · 징집병: CK3(+100~) ÷~1.5 / 세금·개발: CK3 톤 유지
     · 요새레벨: 건물 단계당 +1로 정규화(CK3 +2~16 → 게임 1~8)
   ── 시대(혁신 대체 스캐폴드): 연도로 시대 해제 → 건물 최대단계 게이트.
      나중에 gameEra 내부만 연도→혁신진행도로 교체하면 됨(전방 호환) ──
   ════════════════════════════════════════════════════ */

/* 지형 정의 + CK3 전투/보급/개발 보정 (이동속도 제외)
   advantage=방어자 유리함(CK3값) · supplyMul=보급 배수 · devMul=개발 배수 */
const TERRAIN = {
  farmlands:{ n:'농지',   advantage:0,  supplyMul:+0.35, devMul:+0.20 },
  floodplain:{n:'범람원', advantage:0,  supplyMul:+0.35, devMul:+0.20 },
  plains:   { n:'평원',   advantage:0,  supplyMul: 0.00, devMul: 0.00 },
  hills:    { n:'구릉',   advantage:5,  supplyMul:-0.10, devMul:-0.10 },
  forest:   { n:'삼림',   advantage:3,  supplyMul:-0.10, devMul: 0.00 },
  wetlands: { n:'저습지', advantage:2,  supplyMul:-0.15, devMul:-0.10 }, // 전투폭 60%·일부 [추정]
  mountains:{ n:'산악',   advantage:12, supplyMul:-0.50, devMul:-0.25 },
  drylands: { n:'건조지', advantage:0,  supplyMul: 0.00, devMul: 0.00 },
};
/* 1066 아일랜드 군별 지형 — CK3 실제 지형지도 판독 배정
   출처: CK3 terrain_map.png(SaucyPigeon/ck3_mapdata, CK3 Wiki Barony 지도 원본) + 범례 대조
...
function baronyTerrain(bid){ const b=BARONIES[bid]; if(!b) return 'plains';
  return b.terrain || COUNTY_TERRAIN[b.county] || 'plains'; }
function terrainAdvantage(bid){ return TERRAIN[baronyTerrain(bid)]?.advantage||0; }
function terrainSupplyMul(bid){ return TERRAIN[baronyTerrain(bid)]?.supplyMul||0; }
function terrainDevMul(bid){ return TERRAIN[baronyTerrain(bid)]?.devMul||0; }
/* county 단위 대표 지형(수도 남작령 기준) */
function countyTerrain(cid){ const cap=COUNTIES[cid]?.capital; return cap?baronyTerrain(cap):'plains'; }

/* ── 시대 스캐폴드 (혁신 대체) ──
   연도 임계는 임의값[추정] — 상수만 바꾸면 페이싱 조정, 나중에 혁신진행도로 교체 가능 */
const ERA_YEARS = { highMedieval:1120, lateMedieval:1270 }; // 1066 시작=초기중세
const ERA_NAME = { 1:'초기중세', 2:'성기중세', 3:'후기중세' };
function gameEra(year){ year=year||(state?.year||1066);
  if(year>=ERA_YEARS.lateMedieval) return 3;
  if(year>=ERA_YEARS.highMedieval) return 2;
  return 1; }
/* 시대가 해제하는 최대 건물 단계: 시대 E → 2+2E (CK3: 시대마다 2단계) */
function eraMaxLevel(year){ return 2 + 2*gameEra(year); }

/* 단계 공통 곡선 */
const BLD_COST    = [30,50,70,100,150,220,320,450];   // CK3 ÷5
const BLD_TIME    = [4,4,5,5,6,6,8,8];                  // 개월
const BLD_HOLDREQ = [1,1,2,2,3,3,4,4];                  // 요구 보유지 등급

/* 라인 정의: base=1단계, grow=단계당 증가. eff는 lvEff().
   eff 키: tax(금/월)·pop(개발/월)·levyCap(병력상한)·regen(회복/월)·fort(요새레벨)
          garrison(수비대)·def(방어유리)·supply(보급가산)
   maa: {유닛키:{dmg,tough}} — 단계1 기준값, 적용은 3단계(현재 저장만)
   holding/terrain: 건설 가능 보유지유형/지형(null=전체) */
const CK3_BUILDINGS = {
  holding:{ n:'본성', cat:'hold', icon:'', holding:null, terrain:null, maxLv:8, isHolding:true,
    nameByType:{castle:'성채',city:'도시',temple:'사원'},
    base:{ levyCap:50, tax:1.0, garrison:15 }, grow:{ levyCap:30, tax:0.6, garrison:12 },
    desc:'보유지 본건물 — 등급↑로 병력상한·세금·수비대 증가, 고단계 건물 해제' },

  /* 경제(지형 잠금) */
  farms:   { n:'농장 및 경작지', cat:'eco', icon:'', holding:null, terrain:['farmlands','plains','drylands','floodplain'],
    base:{ tax:0.5, pop:0.10, supply:5 }, grow:{ tax:0.6, pop:0.05, supply:5 },
    desc:'CK3 농장 및 경작지 — 세금·개발·보급(농지/평원/건조지/범람원). CK3 기본세 0.5' },
  hillfarm:{ n:'구릉지 농장', cat:'eco', icon:'', holding:null, terrain:['hills'],
    base:{ tax:0.4, pop:0.08, levyCap:10 }, grow:{ tax:0.5, pop:0.04, levyCap:10 },
    desc:'CK3 구릉지 농장 — 세금·개발·소폭 병력(구릉)' },
  wetfarm: { n:'저습지 농장', cat:'eco', icon:'', holding:null, terrain:['wetlands'],
    base:{ tax:0.45, pop:0.06, regen:1, supply:3 }, grow:{ tax:0.5, pop:0.04, regen:1, supply:3 },
    desc:'CK3 저습지 농장 — 세금·개발·보급(저습지)' },
  forestry:{ n:'삼림지', cat:'eco', icon:'', holding:null, terrain:['forest'],
    base:{ tax:0.4, pop:0.09 }, grow:{ tax:0.45, pop:0.045 },
    desc:'CK3 삼림지 — 세금·개발(삼림)' },
  pasture: { n:'가축 목초지', cat:'eco', icon:'', holding:null, terrain:['farmlands','plains','hills','drylands','mountains'],
    base:{ tax:0.3, levyCap:15 }, grow:{ tax:0.5, levyCap:14 }, maa:{ light_cavalry:{dmg:0.05,tough:0} },
    desc:'CK3 가축 목초지 — 세금·병력·경기병(다수 지형)' },
  hunt:    { n:'사냥터', cat:'eco', icon:'', holding:null, terrain:['forest','hills','plains','drylands'],
    base:{ pop:0.12, tax:0.2 }, grow:{ pop:0.05, tax:0.3 }, maa:{ light_cavalry:{dmg:0.05,tough:0.05} },
    desc:'CK3 사냥터 — 개발·세금·경기병(삼림/구릉/평원/건조지)' },

  /* 군사(대부분 전 지형) */
  barracks:{ n:'병영', cat:'mil', icon:'', holding:null, terrain:['farmlands','plains','hills','forest','mountains','drylands','wetlands'],
    base:{ levyCap:80, regen:2 }, grow:{ levyCap:25, regen:1 },
    maa:{ heavy_infantry:{dmg:0.20,tough:0}, pikemen:{dmg:0.20,tough:0} },
    desc:'CK3 병영 — 징집병·회복·중보병/창병 강화' },
  milcamp: { n:'군사 주둔지', cat:'mil', icon:'', holding:null, terrain:null,
    base:{ levyCap:60, regen:3 }, grow:{ levyCap:22, regen:1 },
    desc:'CK3 군사 주둔지 — 징집병·보충률' },
  militia: { n:'민병대 야영지', cat:'mil', icon:'', holding:null, terrain:null,
    base:{ levyCap:100, regen:1 }, grow:{ levyCap:35, regen:1 },
    desc:'CK3 민병대 야영지 — 대규모 징집병' },
  stables: { n:'마구간', cat:'mil', icon:'', holding:null, terrain:['farmlands','plains','hills','drylands'],
    base:{ levyCap:25, regen:1 }, grow:{ levyCap:18, regen:1 }, maa:{ heavy_cavalry:{dmg:0.10,tough:0.05} },
    desc:'CK3 마구간 — 병력·기병 강화' },
  smith:   { n:'철공소', cat:'mil', icon:'', holding:null, terrain:null,
    base:{ regen:2, levyCap:15 }, grow:{ regen:1, levyCap:12 },
    maa:{ heavy_infantry:{dmg:0,tough:0.10}, pikemen:{dmg:0,tough:0.10} },
    desc:'CK3 철공소 — 회복·무장병 유지력' },

  /* 요새화(성채 전용) */
  walls:   { n:'방벽 및 방어탑', cat:'fort', icon:'', holding:['castle'], terrain:null,
    base:{ fort:1, garrison:25, def:1, tax:0.1 }, grow:{ fort:1, garrison:25, def:1, tax:0.1 },
    maa:{ archers:{dmg:0.05,tough:0.05} },
    desc:'CK3 방벽 및 방어탑 — 요새레벨·수비대·방어·궁병(성채 전 지형)' },
  hillfort:{ n:'구릉 요새', cat:'fort', icon:'', holding:['castle'], terrain:['hills','mountains'],
    base:{ fort:1, garrison:25, def:2 }, grow:{ fort:1, garrison:25, def:1 },
    maa:{ skirmishers:{dmg:0.10,tough:0.10}, pikemen:{dmg:0.10,tough:0.10} },
    desc:'CK3 구릉 요새 — 요새레벨·수비대·방어·산병/창병(성채 구릉/산악)' },
  watchtower:{ n:'망루', cat:'fort', icon:'', holding:['castle'], terrain:null,
    base:{ fort:1, garrison:12, def:1 }, grow:{ fort:1, garrison:12 },
    desc:'CK3 망루 — 요새레벨·소규모 수비대·방어(성채 전 지형)' },
  forestfort:{ n:'삼림 요새', cat:'fort', icon:'', holding:['castle'], terrain:['forest'],
    base:{ fort:1, garrison:25, def:2 }, grow:{ fort:1, garrison:24, def:1 },
    maa:{ skirmishers:{dmg:0.10,tough:0.10} },
    desc:'CK3 삼림 요새 — 요새레벨·수비대·방어·산병(성채 삼림)' },

  /* 도시/사원 전용 */
  guild:   { n:'조합', cat:'city', icon:'', holding:['city'], terrain:null,
    base:{ tax:1.4, pop:0.10 }, grow:{ tax:1.0, pop:0.05 },
    desc:'CK3 도시 조합 — 세금·개발(도시 전용)' },
  monastery:{ n:'수도원', cat:'rel', icon:'✝', holding:['temple'], terrain:null,
    base:{ tax:1.0, pop:0.12 }, grow:{ tax:0.8, pop:0.05 },
    desc:'CK3 수도원 — 세금·개발(사원 전용)' },
};

function lvEff(lineId, L){ const ln=CK3_BUILDINGS[lineId]; if(!ln) return {};
  const out={}, n=Math.max(0,L-1);
  for(const k in ln.base){ if(typeof ln.base[k]==='number') out[k]=+(ln.base[k]+(ln.grow?.[k]||0)*n).toFixed(2); }
  return out; }
/* 무장병 보너스(단계 비례) — Phase 3 적용용. {unit:{dmg,tough}} */
function maaAt(lineId, L){ const ln=CK3_BUILDINGS[lineId]; if(!ln?.maa) return null;
  const out={}, m=Math.max(1,L);
  for(const u in ln.maa){ out[u]={ dmg:+((ln.maa[u].dmg||0)*m).toFixed(2), tough:+((ln.maa[u].tough||0)*m).toFixed(2) }; }
  return out; }
function lvCost(L){ return BLD_COST[Math.min(7,L-1)]; }
function lvTime(L){ return BLD_TIME[Math.min(7,L-1)]; }
function lvHoldReq(lineId,L){ const ln=CK3_BUILDINGS[lineId]; if(ln?.isHolding) return 1; return BLD_HOLDREQ[Math.min(7,L-1)]; }
function lineMaxLv(lineId){ return CK3_BUILDINGS[lineId]?.maxLv||8; }
function lineName(lineId,b){ const ln=CK3_BUILDINGS[lineId]; if(!ln) return lineId;
  if(ln.isHolding&&b) return ln.nameByType[b.type]||ln.n; return ln.n; }

/* 건물 필드 초기화 */
(()=>{ for(const bid in BARONIES){ const b=BARONIES[bid];
  b.builds=[]; b.hlevel=(b.type==='empty')?0:1;
  b.fortLevel=0; b.garrison=0; b.capBld=0; b.taxBld=0; b.regenBld=0;
  b.terrain=baronyTerrain(bid);
  delete b.buildings; delete b.building_queue;
  if(b.type!=='empty') b.builds.push({line:'holding', level:1, wip:null});
}})();

/* 슬롯 수 — CK3: 남작령1 + 백작령수도1 (+등급). 게임 근사: 2 + (등급3↑ +1) */
function baronySlots(bid){ const b=BARONIES[bid]; if(!b||b.type==='empty') return 0;
  return 2 + ((b.hlevel||1)>=3?1:0); }
function usedSlots(b){ return (b.builds||[]).filter(s=>s.line!=='holding').length; }

function availableLines(bid){ const b=BARONIES[bid]; if(!b||b.type==='empty') return [];
  const terr=baronyTerrain(bid), have=new Set((b.builds||[]).map(s=>s.line));
  return Object.keys(CK3_BUILDINGS).filter(id=>{ const ln=CK3_BUILDINGS[id];
    if(ln.isHolding||have.has(id)) return false;
    if(ln.holding&&!ln.holding.includes(b.type)) return false;
    if(ln.terrain&&!ln.terrain.includes(terr)) return false;
    return true; }); }

/* 다음 단계 정보 — 시대·보유지등급·금 게이트 */
function nextLevelInfo(bid, slot){ const b=BARONIES[bid];
  const toLv=slot.level+1, maxLv=lineMaxLv(slot.line);
  if(slot.wip) return {ok:false, reason:'건설 중'};
  if(toLv>maxLv) return {ok:false, reason:'최고 단계'};
  const eMax=eraMaxLevel(state?.year);
  if(toLv>eMax) return {ok:false, reason:`${ERA_NAME[gameEra(state?.year)+1]||'다음'} 시대 필요`, toLv};
  const req=lvHoldReq(slot.line,toLv);
  if((b.hlevel||1)<req) return {ok:false, reason:`보유지 ${req}등급 필요`, toLv};
  return {ok:true, toLv, cost:lvCost(toLv), time:lvTime(toLv)}; }

function startUpgrade(bid, lineId){ const p=playerChar(), b=BARONIES[bid]; if(!b||!p) return false;
  if(b.owner!==p.id){ log('직할 남작령만 건설할 수 있습니다.','dip'); return false; }
  const seat=BARONIES[p.region]; if(!seat) return false;
  let slot=(b.builds||[]).find(s=>s.line===lineId);
  if(!slot){
    if(usedSlots(b)>=baronySlots(bid)){ log('건물 슬롯이 가득 찼습니다.','dip'); return false; }
    if(!availableLines(bid).includes(lineId)){ log('이 보유지/지형에 지을 수 없습니다.','dip'); return false; }
    slot={line:lineId, level:0, wip:null}; b.builds.push(slot);
  }
  const info=nextLevelInfo(bid, slot);
  if(!info.ok){ log(info.reason||'건설 불가','dip'); if(slot.level===0) b.builds=b.builds.filter(s=>s!==slot); return false; }
  if(seat.gold<info.cost){ log(`금이 부족합니다. (필요 ${info.cost})`,'dip'); if(slot.level===0) b.builds=b.builds.filter(s=>s!==slot); return false; }
  seat.gold-=info.cost; playSynthSFX&&playSynthSFX('gold');
  slot.wip={to:info.toLv, prog:0};
  log(`${b.n}에 <b>${lineName(lineId,b)}</b> ${info.toLv}단계 건설 시작 (${info.time}개월·금${info.cost}).`,'good');
  return true; }

/* 매달: 건설 진행 + 완공 + 파생효과 재계산 + 월효과 */
function buildingPulse(){
  for(const bid in BARONIES){ const b=BARONIES[bid]; if(!b.builds) continue;
    for(const slot of b.builds){ if(!slot.wip) continue;
      slot.wip.prog++;
      if(slot.wip.prog>=lvTime(slot.wip.to)){
        const prevEff=slot.level>0?lvEff(slot.line,slot.level):{};
        slot.level=slot.wip.to; slot.wip=null;
        const ln=CK3_BUILDINGS[slot.line], newEff=lvEff(slot.line,slot.level);
        if(ln.isHolding) b.hlevel=slot.level;
        const dCap=(newEff.levyCap||0)-(prevEff.levyCap||0);
        if(dCap){ b.cap=(b.cap||b.troops||200)+dCap; b.troops=Math.min(b.cap, b.troops+Math.round(dCap*0.5)); }
        if(b.owner===state.player) log(`${b.n}의 <b>${lineName(slot.line,b)}</b> ${slot.level}단계 완공! ${ln.desc}`,'good');
      } }
    let fort=0,gar=0,tax=0,regen=0,popM=0,supply=0;
    for(const slot of b.builds){ if(slot.level<=0) continue; const e=lvEff(slot.line,slot.level);
      fort+=e.fort||0; gar+=e.garrison||0; tax+=e.tax||0; regen+=e.regen||0; popM+=e.pop||0; supply+=e.supply||0; }
    b.fortLevel=fort+duchySiegeFort(bid); b.garrison=Math.round(gar); b.taxBld=tax; b.regenBld=regen; b.supplyBld=supply;
    if(b.owner===state.player){
      const seat=BARONIES[playerChar()?.region];
      const devMul=baronyDevMul(bid); // CK3 개발도: 세금·징집 +0.5%/dev
      if(seat&&tax>0) seat.gold=Math.min(9999,(seat.gold||0)+tax*devMul*baronyControlTaxMul(bid));
      if(regen>0) b.troops=Math.min(b.cap||b.troops, b.troops+regen*devMul);
      /* 건물의 개발 기여(e.pop)는 developmentPulse가 군 개발도로 반영 — 민심(b.pop)과 분리 */
    }
  } }

function buildingBonus(bid, key){ const b=BARONIES[bid]; if(!b?.builds) return 0;
  if(key==='troops_regen') return b.regenBld||0;
  let s=0; for(const slot of b.builds){ if(slot.level>0){ const e=lvEff(slot.line,slot.level); s+=e[key]||0; } } return s; }
...
/* ════════════════════════════════════════════════════
   개발도(Development) — CK3 정식 모델 (Phase 2)
   출처: Paradox Forum(2020-09-17)·FandomSpot(2022-07-12)·CK3 Wiki Government(2026-04)
     · 개발도 1점 = 군 내 모든 남작령 세금·징집 +0.5% (50dev=+25%)
     · 개발도 1점 = 보급 한계 +150 (군 단위)
     · 성장 = 건물 Development Growth + 재무관 과업 + 이웃 군 전파, 상한 근접 시 둔화
     · 1066 지도 최고 개발도 = 비잔티움 35 → 아일랜드는 저개발
...
(()=>{ for(const cid in COUNTIES){ const c=COUNTIES[cid];
  c.dev = DEV_INIT[cid] ?? 3;
  for(const bid of (c.baronies||[])) BARONY_COUNTY[bid]=cid;
}})();
function devCap(){ return 15 + 10*gameEra(state?.year); }            // 시대 상한: 초기25/성기35/후기45
function countyDev(cid){ return COUNTIES[cid]?.dev||0; }
function countyDevMul(cid){ return 1 + 0.005*countyDev(cid); }        // CK3: 세금·징집 +0.5%/dev
...
/* 남작령 경계 폴리곤 — _barony_source.jpeg 트레이싱→게임좌표(420×470) 어파인 변환(잔차0.39px). BARONY_XY와 정렬. */
const BARONY_POLY = {
  b_fahan:'275,43 274,43 272,45 272,47 273,47 273,50 271,52 263,52 261,54 261,57 259,58 259,64 261,65 261,66 262,68 262,69 263,70 263,72 258,76 258,80 265,86 265,89 267,90 270,90 278,81 281,80 282,78 282,76 281,75 282,74 283,73 284,72 286,72 288,70 290,70 291,68 292,68 293,67 295,67 297,64 299,64 300,63 303,63 303,62 303,61 302,59 300,59 299,58 297,58 296,56 293,56 293,55 291,55 287,52 287,49 285,47 283,47 282,46 280,45 279,44 276,44',
  b_raphoe:'265,89 264,86 258,80 258,76 258,74 261,72 261,69 260,68 258,63 257,62 257,57 256,56 256,54 254,52 252,54 249,54 248,55 244,55 242,56 239,56 238,57 238,61 236,60 232,56 230,56 227,59 225,62 221,62 219,60 215,63 214,63 212,61 209,62 208,63 208,65 205,67 205,69 203,72 200,71 198,72 197,76 195,76 194,75 191,75 189,77 189,81 194,82 196,83 196,88 199,90 200,90 208,94 222,94 225,96 230,97 236,102 237,102 239,100 246,100 251,103 255,103 257,102 261,102 264,97 263,92',
  b_derry:'314,67 304,67 302,65 301,67 300,67 298,68 297,68 296,69 295,71 294,73 294,74 293,75 293,77 291,79 289,79 288,78 283,77 282,80 280,81 278,81 270,90 265,90 264,92 264,96 263,98 263,100 261,102 261,108 262,109 265,110 268,113 268,116 269,117 270,117 271,116 272,113 274,111 281,108 292,108 302,98 305,96 315,96 322,90 323,89 323,85 322,84 321,77 320,76 320,75 318,74 315,71',
  b_slemish:'356,65 353,65 351,67 349,67 347,64 343,64 342,63 336,63 334,61 333,62 331,61 328,64 321,64 320,65 317,65 316,67 315,67 315,69 315,71 318,73 320,74 322,77 322,84 323,85 323,89 325,90 327,92 327,99 329,100 329,102 330,103 330,106 332,107 335,109 336,107 338,107 343,102 345,102 346,100 348,100 351,97 356,97 359,94 360,94 362,91 364,91 364,83 361,83 360,82 360,81 361,80 361,71 360,70 360,69 358,68',
  b_donegal:'172,105 173,109 174,110 176,111 179,114 183,114 185,116 191,116 192,117 192,122 193,122 197,122 198,120 202,116 203,116 203,118 203,119 209,117 209,119 207,122 207,123 212,124 215,126 219,126 228,131 230,130 231,130 237,130 237,131 239,128 240,127 243,124 244,124 245,125 251,125 251,124 251,120 253,117 256,117 257,116 261,116 263,114 265,114 266,113 264,110 261,108 261,103 260,102 257,102 255,103 250,103 245,101 239,101 236,102 229,97 224,96 220,94 208,94 200,91 199,91 199,92 194,92 193,93 191,93 189,95 190,97 190,99 183,99 182,100 178,101 175,104 174,104',
  b_dungannon:'327,91 323,89 322,90 315,96 305,96 302,98 292,108 281,108 274,111 272,113 271,116 269,117 269,124 277,127 286,127 288,126 291,125 292,124 294,124 296,123 302,123 306,125 309,127 310,127 311,129 313,130 315,131 317,132 319,132 319,131 322,128 324,128 326,126 326,123 327,122 327,120 326,119 326,116 327,114 327,109 330,106 330,105 328,102 328,100 327,99',
  b_carrickfergus:'380,104 373,101 369,98 368,94 365,92 364,91 362,92 355,97 351,97 348,101 346,101 345,102 343,102 336,108 334,108 330,106 327,109 327,115 328,117 330,117 332,119 336,119 340,116 341,116 341,118 340,118 339,118 336,123 337,128 342,129 344,130 349,130 350,129 353,127 354,127 356,126 360,126 364,123 369,122 370,120 373,118 378,118 381,116 382,108',
  b_belcoo:'267,113 265,115 263,115 261,116 259,116 258,116 256,117 253,118 252,119 252,124 250,125 245,125 244,124 243,124 240,127 240,128 237,131 238,132 239,135 240,135 240,137 241,138 241,139 239,141 238,144 239,144 239,146 240,147 241,150 241,157 244,157 247,159 251,159 252,159 255,157 257,156 258,155 259,154 262,151 262,149 263,149 264,147 265,146 265,138 264,137 264,129 269,124 269,117 268,116 268,115',
  b_armagh:'264,129 264,137 265,138 268,138 275,143 278,144 284,143 285,141 291,142 294,144 294,147 300,150 302,153 304,154 310,153 311,154 314,158 314,159 315,161 315,167 313,171 314,173 316,173 319,170 321,169 324,167 325,166 327,164 332,162 333,159 336,157 336,156 337,155 338,155 340,153 340,152 338,151 334,151 332,150 329,147 329,137 330,135 328,133 327,134 321,134 319,133 314,131 306,125 302,123 296,123 294,125 292,125 291,126 288,126 287,127 277,127 272,125 269,124',
  b_bangor:'393,134 392,132 391,127 388,123 378,123 377,121 371,121 370,122 367,123 366,123 364,123 360,126 356,126 354,127 353,127 350,130 349,130 344,130 342,129 337,129 338,129 337,130 334,132 334,135 333,136 330,135 329,137 329,147 332,150 334,151 338,151 347,157 347,156 349,155 350,155 353,152 356,152 358,152 360,152 364,148 366,148 379,148 382,149 383,146 383,141 387,139 392,135',
  b_killala:'67,159 68,162 71,162 72,161 74,161 76,164 80,164 82,161 83,156 83,155 86,155 88,156 88,159 85,162 85,166 88,167 88,169 82,169 80,167 77,169 73,169 71,171 68,171 66,173 68,175 74,175 75,177 77,177 78,180 85,187 92,187 96,185 99,185 103,187 109,187 113,185 115,183 121,183 129,185 135,181 140,181 141,180 140,176 136,174 135,171 136,169 137,165 141,160 142,157 144,155 145,151 142,148 141,145 137,141 123,141 120,141 118,139 116,139 115,138 113,139 109,139 106,137 101,137 97,140 97,143 97,144 92,143 89,139 83,142 80,145 80,150 78,152 78,155 77,156 71,155 70,157',
  b_sligo:'190,133 189,134 186,134 183,137 182,137 181,138 179,138 176,141 176,143 178,145 181,146 179,149 177,149 176,147 174,149 173,149 172,147 169,147 168,149 164,149 161,146 160,146 158,145 155,145 153,144 152,144 148,147 148,149 145,152 145,155 142,157 141,160 137,165 137,169 135,171 136,173 138,174 142,179 146,181 153,181 156,184 160,185 162,189 162,196 163,197 165,197 166,196 168,195 171,193 173,193 180,186 184,186 186,185 189,185 193,181 195,181 197,180 204,180 204,174 205,171 202,167 202,160 201,158 199,156 198,153 196,150 196,147 192,141 192,139',
  b_dromahair:'235,131 228,131 222,128 221,128 218,126 215,126 212,125 209,125 207,123 204,125 204,127 202,129 200,129 200,130 196,130 194,129 192,129 191,130 190,134 192,138 192,141 193,142 194,144 196,147 196,150 199,153 199,156 201,158 202,159 203,161 203,167 205,171 205,172 204,174 204,180 206,179 209,177 214,177 215,176 220,176 221,175 223,173 226,171 227,167 232,162 235,161 237,160 238,160 241,158 241,157 241,150 238,145 238,141 240,139 240,138 239,136 238,134 237,134',
  b_clogher:'252,160 253,163 253,165 255,167 259,168 264,171 265,171 270,173 273,174 275,175 278,177 281,177 283,178 288,182 291,184 293,185 296,184 298,184 299,182 303,183 309,179 312,178 314,174 313,172 313,171 315,167 315,162 313,159 313,158 312,155 310,154 303,154 302,153 300,150 295,148 294,147 293,144 292,143 290,142 285,142 284,143 282,143 280,144 278,144 275,143 268,138 265,138 265,146 264,147 262,150 262,151 258,156 255,157',
  b_downpatrick:'393,134 392,135 387,139 383,141 384,146 382,149 381,149 379,148 368,148 367,148 366,148 364,148 360,152 356,152 353,153 350,155 349,155 348,156 347,157 348,158 348,161 347,164 347,168 354,175 354,177 359,181 361,177 361,171 364,169 366,169 368,167 374,167 375,169 378,169 379,167 381,167 387,162 387,161 389,159 389,157 392,154 392,152 393,151 393,150 396,146 396,144 395,143 395,140 393,139',
  b_adragh:'204,180 204,182 205,183 207,183 209,185 209,187 210,188 215,188 216,190 217,192 217,194 219,197 220,200 222,200 225,198 227,198 234,200 239,197 247,197 250,196 253,196 256,197 260,201 264,201 267,199 269,197 269,193 271,192 271,189 273,187 273,185 276,183 277,179 277,177 273,174 270,174 265,171 264,171 259,168 255,167 253,165 253,163 251,159 247,159 244,158 241,158 240,159 239,159 238,160 237,160 235,162 232,162 228,167 226,171 223,173 221,176 220,176 216,176 214,177 209,177 206,180',
  b_dundalk:'340,153 338,155 337,155 336,156 336,157 333,159 333,161 332,162 330,163 329,164 328,164 327,165 326,165 325,167 324,167 321,170 319,170 316,173 314,174 314,176 313,178 313,181 313,184 317,189 319,190 323,193 324,194 329,194 329,192 330,190 332,190 333,191 334,191 336,193 342,193 344,191 347,191 347,188 347,187 350,187 351,186 354,186 356,183 357,183 359,181 356,180 354,177 353,175 350,172 349,170 347,168 347,164 348,161 348,158 345,156 343,154',
  b_castlebar:'162,205 161,203 161,200 162,198 162,189 160,187 160,186 156,185 154,182 146,182 142,179 140,182 135,182 133,183 132,183 130,185 128,185 126,184 123,184 121,183 115,183 110,187 106,187 103,187 103,189 102,190 102,191 103,192 103,193 100,194 99,195 100,196 102,199 102,201 107,204 108,207 116,211 120,214 123,211 125,211 127,210 130,210 136,213 137,213 142,213 144,212 150,212 152,210 153,210 156,207 158,207',
  b_cruachu:'204,180 197,180 195,182 193,182 189,185 186,185 184,187 179,187 173,193 171,193 168,196 166,196 165,197 162,197 162,198 161,200 161,203 162,205 166,208 167,208 170,211 174,211 175,210 178,210 179,209 180,209 181,210 181,212 184,216 184,218 182,220 182,222 184,223 189,223 190,222 193,222 194,221 197,221 199,220 202,219 208,220 208,219 209,219 210,217 212,214 213,213 215,210 215,209 220,204 220,200 219,198 219,197 217,194 217,192 215,189 210,188 209,187 208,185 207,183 205,183 204,182',
  b_longford:'260,201 256,197 253,196 250,196 248,197 239,197 235,200 230,200 229,199 227,199 226,198 224,198 222,200 221,200 221,204 216,209 216,210 213,213 213,214 210,218 210,219 211,221 212,222 212,225 214,227 216,227 216,228 216,229 222,229 227,226 233,226 238,222 239,222 240,221 245,219 247,219 249,220 251,219 253,218 253,216 254,214 254,210 255,208 256,205',
  b_cavan:'278,178 278,179 276,181 276,183 274,185 273,187 271,189 271,191 270,193 269,197 269,198 271,199 273,201 273,203 272,206 272,209 277,213 278,214 280,214 281,213 283,213 285,215 285,216 286,217 287,217 293,221 294,221 296,222 300,222 302,224 303,224 304,221 308,217 310,217 312,215 312,214 308,209 307,208 305,206 305,204 304,204 304,203 303,200 302,195 295,189 294,186 291,185 288,182 286,181 285,180 283,179 282,178 281,178',
  b_ardee:'295,185 294,186 295,189 302,195 303,198 303,200 305,203 305,204 306,206 308,208 308,209 312,213 313,214 315,214 316,215 317,215 318,215 322,215 323,214 325,214 326,213 334,213 334,212 334,211 335,210 335,201 333,200 332,200 329,196 329,195 325,195 323,194 319,190 317,189 315,188 313,185 312,179 309,179 308,180 306,181 303,183 299,183 298,184 296,184',
  b_da_chainoc:'61,206 61,211 65,213 64,215 64,217 65,219 65,221 68,222 69,223 69,224 67,224 64,227 64,231 67,232 72,232 74,235 76,235 77,236 81,236 82,238 81,240 84,244 91,245 90,247 90,250 93,252 98,252 99,251 104,251 107,253 108,253 109,252 135,252 136,251 139,251 139,248 136,241 135,238 131,233 129,230 127,229 125,229 121,225 121,223 125,223 125,218 116,211 109,208 108,207 107,204 102,201 102,199 98,194 94,194 92,195 88,195 86,197 86,199 83,204 83,205 85,208 85,209 80,209 79,208 78,208 73,212 69,212 68,212 69,209 69,206 62,206',
  b_tuam:'180,209 178,210 176,210 174,211 171,211 167,209 166,208 162,205 158,208 156,208 154,210 150,212 144,212 142,213 136,213 130,211 127,211 125,211 125,214 123,216 125,219 126,223 127,223 128,224 127,229 128,230 130,231 131,234 136,238 136,241 139,248 139,251 141,251 143,250 145,250 146,249 148,249 150,250 155,250 157,248 161,249 162,248 165,247 170,242 170,240 167,239 166,238 166,236 167,235 169,235 170,233 173,231 176,224 180,225 183,223 182,222 182,220 183,218 183,216 181,212 181,210',
  b_athenry:'214,232 213,232 212,231 212,230 213,229 213,227 212,225 212,223 211,221 209,219 208,219 208,220 204,220 202,219 201,220 199,220 198,221 194,221 193,222 191,222 189,224 184,224 183,223 180,225 179,225 178,224 176,224 175,226 174,228 174,229 173,230 173,231 171,233 170,235 167,236 167,238 167,239 169,239 171,240 171,242 178,242 180,242 183,245 187,247 188,248 188,249 189,250 193,252 196,253 198,252 200,252 203,254 205,254 206,250 209,247 210,247 211,246 213,245 214,244 214,239 212,237 212,235 213,233 213,232',
  b_uisneach:'268,198 264,201 259,202 256,205 256,208 255,210 255,213 253,216 253,217 251,220 249,220 249,221 251,224 252,225 257,230 257,232 258,233 258,237 260,240 263,241 266,241 268,239 275,239 277,240 279,240 286,233 289,231 291,229 294,229 296,231 297,230 299,228 302,225 302,224 300,223 296,222 294,221 293,221 291,220 286,218 285,216 284,215 283,213 281,213 280,214 278,214 277,214 271,208 271,206 273,203 273,201 272,200 271,200 269,198',
  b_drogheda:'342,231 337,227 336,224 334,222 334,213 326,213 325,214 323,214 322,216 318,216 315,214 313,214 312,215 311,217 308,217 304,221 304,223 302,224 302,225 299,228 297,231 295,231 294,230 291,230 289,232 286,233 281,238 281,239 284,241 286,241 287,242 290,244 291,244 294,246 297,248 300,249 302,249 307,249 308,248 308,247 309,246 315,246 316,245 328,245 329,243 337,243 339,244 339,241 342,238',
  b_galway:'207,256 205,254 203,254 199,252 195,253 190,251 188,249 187,248 183,245 179,242 178,242 171,242 165,247 162,248 161,249 157,249 155,250 150,250 146,249 145,250 143,250 139,254 140,255 140,257 139,257 137,257 136,259 134,259 133,260 136,263 139,264 143,267 148,270 152,270 156,267 158,267 159,268 161,266 166,266 168,268 172,270 177,270 179,268 181,269 184,268 188,271 189,271 191,270 192,270 193,269 193,267 198,263 200,261 203,261 205,260 207,259',
  b_athlone:'258,233 257,232 256,230 251,225 251,224 249,222 249,220 247,219 245,219 239,222 238,222 233,226 227,226 222,229 216,229 216,230 214,231 215,233 213,233 213,234 213,237 214,239 214,244 213,246 211,246 210,248 209,248 206,250 206,253 205,254 207,256 207,258 210,260 214,261 216,262 216,264 224,266 227,266 229,267 234,267 237,270 240,272 241,271 241,268 243,266 247,266 248,264 249,262 249,258 252,256 251,253 250,252 251,247 253,245 256,244 260,240 258,237',
  b_trim:'251,247 251,251 250,252 251,253 253,252 254,253 257,252 261,255 263,255 264,255 266,255 269,257 274,257 276,258 277,258 279,259 282,260 284,261 288,262 290,263 294,264 296,265 299,268 301,266 301,262 304,259 304,255 303,253 302,249 297,249 294,246 291,244 290,244 289,243 287,243 285,241 283,241 281,239 279,241 277,241 274,239 273,240 268,240 266,241 263,241 261,240 260,240 256,244 254,245',
  b_dublin:'338,244 329,243 328,245 316,245 315,246 309,247 308,249 303,249 304,255 304,259 302,262 301,266 300,268 302,270 302,272 303,275 303,276 302,278 302,280 301,282 301,284 303,286 308,288 311,285 311,284 312,282 313,282 314,281 317,280 318,279 319,278 321,278 322,277 329,277 331,275 336,276 338,275 338,274 338,272 336,271 337,270 338,269 338,262 336,260 334,260 333,258 333,257 334,256 336,256 337,255 340,255 340,254 340,250 338,246 338,245',
  b_ennis:'158,268 157,267 152,270 148,270 143,268 140,265 136,264 133,260 127,260 126,259 122,259 119,264 117,266 114,268 112,271 111,272 111,274 109,276 109,278 106,280 106,281 107,282 111,282 112,284 108,286 107,287 107,290 105,292 103,296 100,298 95,299 89,305 88,305 87,306 82,308 79,311 76,311 75,313 73,313 72,315 72,316 72,317 79,317 80,319 83,319 84,318 86,318 93,312 95,312 99,314 104,314 106,313 108,313 111,317 113,316 116,314 120,314 121,313 129,313 130,311 130,307 132,302 136,298 140,296 141,293 144,290 146,287 146,284 152,278 158,270',
  b_kincora:'188,272 184,268 179,269 177,270 172,270 168,268 166,266 161,266 159,268 159,269 154,275 152,278 146,284 146,287 144,290 141,293 140,296 138,298 136,299 133,301 130,308 133,308 135,309 141,309 143,308 145,308 147,310 149,311 153,311 158,311 163,310 163,309 165,308 165,307 166,305 166,304 168,301 170,301 171,300 172,295 173,293 175,290 175,289 179,287 180,287 183,284 183,280 184,279 185,277 186,276 188,276',
  b_birr:'188,272 188,275 188,276 188,277 188,278 193,283 200,283 201,283 204,283 207,284 210,285 214,287 215,287 219,289 227,289 228,290 231,289 232,290 235,286 236,283 237,282 237,280 237,279 239,277 240,277 241,276 240,272 239,272 237,271 235,268 233,267 229,267 227,266 224,266 222,266 221,266 220,265 217,265 214,261 211,261 207,259 205,260 203,261 201,261 198,263 194,267 193,269 192,271 190,271',
  b_athy:'232,290 237,295 238,299 240,300 243,299 245,298 248,298 249,297 250,297 251,298 252,300 255,302 257,302 258,300 260,298 263,298 267,294 271,294 273,293 273,291 272,289 271,283 270,281 270,275 270,274 270,271 268,269 266,268 265,268 265,267 263,267 262,269 259,269 259,270 253,270 252,270 250,270 249,270 248,268 247,266 243,266 242,268 242,270 241,272 241,276 240,277 239,277 238,279 237,280 237,282 236,283 236,285 236,286',
  b_kildare:'252,253 252,256 250,258 250,262 248,263 248,266 248,268 249,269 251,270 256,270 258,270 259,269 262,268 263,267 265,267 265,268 267,268 269,270 270,271 270,281 271,283 272,282 274,280 277,280 281,284 282,284 290,284 292,283 293,282 296,283 300,283 300,282 302,280 302,278 303,276 303,275 302,272 301,270 297,266 291,263 285,262 282,260 279,260 274,258 269,257 266,256 264,256 263,255 261,255 257,253 253,253',
  b_nenagh:'188,276 186,277 184,279 183,284 180,287 179,287 176,289 176,290 173,293 172,295 172,300 170,301 168,302 166,304 166,305 165,307 165,308 164,309 164,310 162,311 156,311 157,320 155,324 155,326 156,327 156,330 157,331 158,333 159,334 163,335 166,338 170,341 173,339 179,333 179,332 180,331 182,326 184,324 185,322 183,320 183,318 184,316 185,313 186,312 186,310 187,309 188,307 187,304 190,300 190,294 193,291 193,283 188,278',
  b_roscrea:'210,285 207,285 204,283 193,283 193,291 190,294 190,300 188,304 188,309 187,309 186,310 186,312 185,313 185,316 183,318 183,320 185,322 186,322 188,320 189,320 192,317 194,317 195,316 201,316 202,315 202,314 203,313 204,313 205,314 207,315 211,315 212,315 213,315 214,312 216,310 215,308 216,307 216,306 217,306 217,299 214,295 212,295 212,294 210,292 210,291 212,289 213,287',
  b_kilkenny:'213,287 212,289 210,291 210,292 212,294 214,294 216,296 216,297 217,299 217,306 217,307 216,308 216,310 215,311 214,312 214,315 212,316 212,318 212,319 213,320 215,321 220,321 223,323 226,323 229,325 230,325 231,326 234,326 235,325 238,322 239,322 238,322 238,319 239,318 239,315 237,313 237,301 239,299 238,298 237,295 231,290 229,290 228,290 227,290 219,290 216,288 215,288 214,287',
  b_gowran:'275,294 274,293 271,295 267,295 263,298 260,299 259,300 258,301 257,303 255,303 253,301 250,297 249,297 248,298 245,298 243,300 239,300 239,299 237,301 237,313 240,315 240,318 239,319 239,322 242,322 246,325 249,325 251,326 262,326 265,327 267,327 269,327 269,322 270,319 270,317 272,316 272,308 274,306 274,305 275,303 275,295',
  b_carlow:'271,283 272,285 272,288 273,290 274,292 276,295 276,303 275,303 274,306 272,308 272,315 273,315 276,315 279,312 282,312 284,311 286,311 288,308 292,308 294,307 298,306 301,308 304,308 305,307 308,307 309,306 309,301 304,297 304,294 306,292 307,290 307,288 305,287 304,287 303,286 302,286 300,283 296,284 295,283 293,283 292,283 290,285 282,285 281,284 277,280 274,281 272,283',
  b_wicklow:'339,275 336,276 332,276 331,275 329,277 322,277 321,278 319,278 318,279 318,280 317,281 316,281 314,281 313,282 312,282 311,284 311,285 310,286 308,288 308,290 305,294 305,297 309,300 309,306 311,307 322,307 327,309 331,312 333,310 334,308 336,306 336,305 338,303 338,302 339,301 339,296 338,295 338,289 339,287 339,285 340,283 340,278 339,276',
  b_limerick:'156,311 149,311 145,308 143,308 141,310 135,310 133,308 130,308 130,311 129,313 122,313 120,314 116,315 113,317 111,317 110,316 109,314 107,313 106,313 104,314 99,314 94,312 86,318 88,319 88,321 82,326 80,326 81,330 84,333 84,334 85,335 85,336 86,338 87,341 89,343 89,345 93,348 99,351 101,353 103,350 109,347 112,347 116,343 121,341 123,341 125,342 127,342 129,341 131,341 134,339 137,339 138,338 141,338 145,335 153,335 157,333 157,332 156,330 156,327 155,326 155,322 156,320',
  b_clonmel:'207,320 205,322 204,324 204,326 204,327 199,332 199,338 200,339 201,341 202,342 202,347 203,348 203,351 205,353 206,354 210,354 211,355 211,354 211,353 213,352 215,349 223,349 225,349 226,350 228,350 229,351 230,350 232,351 234,350 234,349 234,347 236,345 237,343 237,342 238,340 239,338 239,337 239,335 237,333 238,330 234,326 231,326 230,326 229,325 226,323 223,323 220,322 215,322 213,321 211,319 210,320',
  b_enniscorthy:'322,328 321,327 307,327 305,328 288,328 287,330 274,330 273,330 273,333 272,335 272,343 271,344 273,345 276,345 279,348 280,349 281,350 285,351 287,351 290,349 293,349 296,351 310,351 312,352 322,342 322,340 323,339 323,332 322,330',
  b_ferns:'331,312 327,309 325,309 324,308 323,308 322,307 311,307 309,306 308,307 305,307 304,308 301,308 298,307 294,307 292,308 288,308 286,311 285,311 282,313 280,313 279,312 276,315 273,315 272,315 272,316 271,317 271,319 269,322 269,327 270,327 273,330 274,329 287,329 288,328 305,328 307,327 321,327 323,327 323,326 324,325 326,325 327,323 327,322 328,321 329,319 330,318 330,314',
  b_kilmallock:'175,364 175,353 170,343 170,341 166,339 163,336 161,335 158,333 153,336 145,336 141,338 139,338 137,339 134,339 131,341 130,341 127,342 125,342 123,341 121,341 116,344 112,347 109,348 103,351 102,353 105,356 105,359 106,361 107,363 111,366 115,366 117,368 126,367 128,366 132,366 135,364 136,363 137,362 141,363 146,365 153,365 156,364 159,363 160,362 163,362 166,364 170,364 172,365',
  b_emly:'211,315 207,315 205,314 203,314 202,315 202,316 201,316 195,316 194,318 191,318 189,320 185,322 185,324 183,326 181,331 180,332 179,333 173,339 170,341 170,343 175,353 175,364 177,364 183,364 186,365 195,366 198,368 201,367 201,365 202,364 202,361 204,360 204,358 205,355 205,354 202,351 202,348 201,347 201,342 199,338 199,332 203,327 204,324 205,322 207,319 210,319 211,319 212,318',
  b_carrick:'238,322 235,325 234,327 236,328 237,328 238,330 238,333 240,335 240,337 237,342 237,343 237,345 235,347 235,349 239,350 240,351 240,352 241,353 241,355 245,359 248,361 249,361 255,361 257,362 261,362 262,362 262,360 263,359 264,356 267,353 267,351 267,350 268,348 269,346 271,343 271,335 273,333 273,330 269,327 267,328 265,328 262,326 251,326 249,325 246,325 242,322',
  b_wexford:'263,360 263,362 264,364 264,367 266,372 265,377 266,377 266,378 267,379 268,379 269,379 269,377 271,375 274,375 276,372 284,372 288,377 300,377 301,378 303,378 304,379 305,379 307,377 308,375 309,374 311,374 313,372 312,370 311,369 309,368 308,367 307,365 306,364 306,363 310,360 310,358 309,357 309,356 310,355 310,353 311,352 310,351 296,351 292,349 291,350 290,350 288,351 285,351 281,350 276,346 273,345 271,344 269,346 269,348 268,350 268,351 267,353 264,356 264,359',
  b_tralee:'117,368 110,366 106,362 105,357 99,351 93,349 89,345 86,340 83,333 81,329 80,326 79,327 71,327 68,330 68,333 74,335 74,339 73,342 68,346 65,345 65,341 63,339 62,339 60,341 60,344 59,345 55,344 53,341 51,340 50,342 47,342 45,344 41,345 39,347 34,348 30,352 31,354 30,358 24,358 25,362 29,362 30,359 33,361 40,360 55,361 56,359 60,359 62,361 60,366 51,367 47,370 44,370 37,376 34,376 30,380 32,385 35,387 35,391 38,392 44,389 45,392 44,394 46,395 48,398 57,396 60,394 64,394 65,392 72,392 84,385 92,385 93,383 107,384 109,380 115,374',
  b_cork:'107,384 108,388 109,390 120,390 122,391 126,391 128,390 132,390 133,389 138,390 148,390 151,389 158,390 161,389 164,389 167,392 167,394 171,397 171,404 172,405 173,404 174,404 176,401 179,402 179,403 183,403 184,401 191,401 191,400 193,400 194,398 197,396 202,396 206,393 206,391 202,389 203,383 204,382 204,378 205,375 205,371 201,367 197,368 186,365 183,364 175,364 174,365 171,365 163,363 160,363 159,364 156,364 153,365 146,365 141,363 137,363 132,366 128,366 126,368 117,368 117,370 116,374 109,380',
  b_waterford:'201,365 201,367 206,371 206,375 204,378 204,382 203,383 203,389 206,391 212,391 215,388 219,388 220,387 222,387 224,385 224,383 226,382 227,378 228,377 230,377 231,375 246,375 247,377 249,377 251,375 252,375 254,377 256,377 257,378 260,378 261,378 262,377 265,377 266,373 266,371 264,367 263,363 262,362 261,363 257,363 255,361 248,361 245,360 241,355 240,353 239,350 235,349 233,351 228,351 222,349 221,350 215,350 212,353 212,354 211,355 206,355 204,357 204,360 203,361 203,364',
  b_baltimore:'113,424 112,422 107,417 105,414 105,412 107,410 108,401 110,398 108,395 109,390 107,388 107,384 93,384 92,385 84,385 73,391 71,393 68,393 64,396 61,396 61,397 59,398 57,399 57,403 52,403 50,406 47,405 41,410 42,411 51,411 52,412 55,412 60,409 61,409 63,411 66,411 67,410 70,409 72,407 75,407 76,405 84,405 85,404 88,404 89,406 88,407 86,407 84,408 80,408 76,411 73,411 72,412 71,412 66,416 66,417 73,417 75,416 78,416 75,419 71,419 69,421 66,421 65,423 67,427 74,427 76,423 79,423 81,424 85,424 89,423 90,424 91,427 86,428 86,433 90,433 93,431 94,428 100,428 102,427 108,427',
  b_kinsale:'171,396 169,396 167,394 166,392 165,391 165,390 164,389 161,389 160,390 155,390 153,389 151,389 148,391 137,390 136,390 133,389 132,390 128,390 126,392 122,392 120,390 109,390 109,395 110,397 110,399 109,399 109,405 108,407 108,410 105,412 105,414 107,417 112,422 113,424 115,424 116,423 117,423 119,421 124,421 125,423 130,423 133,421 133,419 134,419 137,419 139,421 144,421 145,421 145,418 146,417 147,415 151,415 151,416 154,416 155,416 155,414 156,413 157,411 163,411 165,408 170,408 172,406 171,404',
};
const BARONY_ADJ = {
...
/* 남작령 경로 탐색(BFS) — Phase 3 군대 이동용. 최단 남작령 경로 배열 반환(없으면 null) */
function baronyPath(from, to){ if(from===to) return [from]; const q=[[from]]; const seen=new Set([from]);
  while(q.length){ const path=q.shift(); const last=path[path.length-1];
    for(const nb of (BARONY_ADJ[last]||[])){ if(seen.has(nb)) continue; seen.add(nb);
      if(nb===to) return path.concat(nb); q.push(path.concat(nb)); } }
  return null; }
/* ════════════════════════════════════════════════════════════════════════════ */

/* ════════ Phase 3: 군대(Army) 객체·이동·지도 마커 ════════ */
function ensureArmies(){ if(!Array.isArray(state.armies)) state.armies=[]; return state.armies; }
function _armyId(){ state._armySeq=(state._armySeq||0)+1; return 'army'+state._armySeq; }
/* 군대 소집: owner의 capBid에서 units(병종별 병사 수) 구성으로 생성 */
function raiseArmy(owner, capBid, units){ ensureArmies();
  const a={ id:_armyId(), owner, units:Object.assign({levy:0},units||{}), pos:capBid, dest:null, path:null, hop:0, prog:0, cmd:null, state:'idle' };
...
function playerRaiseArmy(){ const p=playerChar(); if(!p) return null; const cap=p.region; if(!cap) return null;
  ...
  if(armyTroops({units})<=0) return null;
  return raiseArmy(p.id, cap, units); }
...
/* 마커 위치 (행군 중이면 현재 변 위 보간) */
function armyXY(a){ const at=BARONY_XY[a.pos]; if(a.state!=='march'||!a.path) return at;
  const cur=BARONY_XY[a.path[a.hop]], nxt=BARONY_XY[a.path[a.hop+1]]; if(!cur||!nxt) return at;
...
/* 병종별 지형 가산값 (CK3 정확, 평탄 가산) [나무위키 군사 각주] */
const MAA_TERRAIN_FLAT = {
  skirmishers: { forest:{dmg:4,tough:6} },                                    // 삼림 피해+4·유지+6
  archers:     { hills:{dmg:10,tough:4}, forest:{tough:4,screen:4} },          // 구릉 피해+10·유지+4
  light_cav:   { plains:{dmg:15}, farmlands:{dmg:15}, drylands:{dmg:15},        // 개활지 피해+15
                 hills:{dmg:-5}, mountains:{dmg:-5,pursuit:-20},
                 wetlands:{dmg:-15,tough:-10,pursuit:-30,screen:-30} },          // 험지 큰 불리
  spearmen:    { mountains:{tough:12}, hills:{tough:8} },                       // 산악 유지+12·구릉+8
  heavy_inf:   {},                                                              // 지형 무보정
};
function maaTerrainFlat(k, terr){ const m=(MAA_TERRAIN_FLAT[k]||{})[terr]||{}; return {dmg:m.dmg||0,tough:m.tough||0,pursuit:m.pursuit||0,screen:m.screen||0}; }
/* 군대 전투 스탯 합 (지형 반영) — 피해도/유지력/추격/차단 총합 + 평균 유지력 */
function armyStats(a, terrainKey){ let dmg=0,toughTot=0,troops=0,pursuit=0,screen=0;
  for(const k in (a.units||{})){ const num=a.units[k]||0; if(num<=0) continue;
    if(k==='levy'){ dmg+=num*LEVY_DMG; toughTot+=num*LEVY_TOUGH; troops+=num; continue; }
    if(k==='knights'){ dmg+=num*KNIGHT_DMG_PER_PROWESS*8; toughTot+=num*KNIGHT_TOUGH_PER_PROWESS*8; troops+=num; continue; } // 기사 1기≈기량8 가정
    const t=MAA_TYPES[k]; if(!t) continue; const tb=maaTerrainFlat(k,terrainKey);
    dmg+=num*Math.max(0,(t.dmg||0)+tb.dmg); toughTot+=num*Math.max(1,(t.tough||0)+tb.tough);
    pursuit+=num*Math.max(0,(t.pursuit||0)+tb.pursuit); screen+=num*Math.max(0,(t.screen||0)+tb.screen); troops+=num; }
  return { dmg, toughTot, avgTough: troops>0?toughTot/troops:1, troops, pursuit, screen }; }
/* 군대 주력(최다) 무장병 병종 */
function armyDominantMaa(a){ let best=null,bn=0; for(const k in (a.units||{})){ if(k==='levy'||k==='knights')continue; const num=a.units[k]||0; if(num>bn){bn=num;best=k;} } return best; }
/* 상성 피해 배수(<1=내 피해 감소): 적 주력이 내 주력을 카운터하면 감소 (2:1 −90%·1:1 −50%) */
function counterDamageMul(myType, enemyType, myCnt, enemyCnt){ if(!myType||!enemyType) return 1;
  if(MAA_COUNTERS[enemyType]===myType){ const ratio=(myCnt>0)?enemyCnt/myCnt:1; return 1-Math.min(0.9, 0.45*Math.max(0,ratio)); }
  return 1; }
/* 비율로 군대 병력 축소(units 비례 차감) → 실제 손실 병력 반환 */
function _scaleArmyUnits(a, keepFrac){ keepFrac=Math.max(0,Math.min(1,keepFrac)); let lost=0;
  for(const k in a.units){ const before=a.units[k]||0; const after=Math.round(before*keepFrac); lost+=(before-after); a.units[k]=after; }
  return lost; }
/* ── 4단계 전투 해결 (기동→초기→후기→추격). 양 군 units를 변형하고 결과 반환 ── */
function runBattle(atk, def, terrainKey){
  const atk0=armyTroops(atk), def0=armyTroops(def);
  const atkComp0=Object.assign({},atk.units), defComp0=Object.assign({},def.units); // UI용 초기 편성
  ...
  while(day<maxDays && aScale>0 && dScale>0){
    ...
    if(aScale<=0.30||dScale<=0.30) break;             // 한쪽 붕괴 → 종전
    if(phase==='late' && Math.min(aScale,dScale)/Math.max(aScale,dScale)<0.62) break; // 후기: 열세측 후퇴 결정
  }
  const atkWins=aScale>=dScale; const phaseEndedEarly=(phase==='early');
  // 전투 손실 적용 (치명 30%/패주 70% [CK3]). 승자는 패주 복귀(치명만 손실), 패자는 전부 손실.
  const aCasFrac=1-aScale, dCasFrac=1-dScale;
  const winner=atkWins?atk:def, loser=atkWins?def:atk;
  const winKeep = 1-(atkWins?aCasFrac:dCasFrac)*0.30;   // 승자: 치명(30%)만 영구 손실
  const FATAL = phaseEndedEarly?1.0:0.30;               // 초기 패배 시 패주도 전부 치명 [CK3]
  const loseCasFrac=(atkWins?dCasFrac:aCasFrac);
  // ④ 추격: 승자 추격 vs 패자 차단 → 패주 추가 치명 (간이: 추격/차단 비로 패자 손실 가중)
  const ws=armyStats(winner,terrainKey), ls=armyStats(loser,terrainKey);
  const pursuitExtra=Math.min(0.45, (ws.pursuit/Math.max(1,ws.troops))*0.012 - (ls.screen/Math.max(1,ls.troops))*0.012*0.33);
...
function retreatArmy(a){ const opts=(BARONY_ADJ[a.pos]||[]).filter(Boolean);
  ...
  if(dest) a.pos=dest; a.state='idle'; a.dest=null; a.path=null; a.hop=0; a.prog=0; }
/* 적 county에 도착한 플레이어 군대 상대로 수비군 자동 편성 (데모·전선 형성) */
function ensureDefenderArmy(cid){ const cnt=COUNTIES[cid]; if(!cnt) return null;
  const holder=countyHolder(cid); if(!holder||holder.id===state.player) return null;
  if((state.armies||[]).some(a=>a.owner===holder.id && BARONY_COUNTY[a.pos]===cid)) return null; // 이미 있음
  const now=(state.year||0)*12+(state.month||0); if(!state._defCD) state._defCD={};
  if((state._defCD[cid]||0)>now) return null;               // 쿨다운: 매월 재편성 방지
  let levy=0; for(const bid of (cnt.baronies||[])) levy+=(BARONIES[bid]?.troops||0);
  if(levy<300) return null;                                  // 잔여 수비 병력 부족 → 더는 편성 안 함
  const raise=Math.round(levy*0.6);
  let drain=raise; for(const bid of (cnt.baronies||[])){ if(drain<=0)break; const b=BARONIES[bid]; if(!b)continue;
...
function battlePulse(){ ensureArmies();
  // 플레이어 군대가 전쟁 중인 적 county에 대기 중이면 수비군 자동 편성
  for(const a of state.armies.slice()){ if(a.owner!==state.player||a.state!=='idle') continue;
    const cid=BARONY_COUNTY[a.pos]; const holder=cid&&countyHolder(cid);
    if(holder&&holder.id!==state.player && state.wars.some(w=>(w.atk===state.player&&w.def===holder.id)||(w.atk===holder.id&&w.def===state.player))) ensureDefenderArmy(cid); }
  // 동일 남작령의 적대 군대 쌍 → 전투
  const byPos={}; for(const a of state.armies){ (byPos[a.pos]=byPos[a.pos]||[]).push(a); }
  for(const pos in byPos){ const list=byPos[pos]; if(list.length<2) continue;
    for(let i=0;i<list.length;i++){ for(let j=i+1;j<list.length;j++){ const A=list[i],B=list[j];
      if(!armiesHostile(A,B)) continue;
      const terr=baronyTerrain(pos), cn=COUNTIES[BARONY_COUNTY[pos]]?.n||'전장';
      const r=runBattle(A,B,terr); if(!r) continue;
...
function _compStr(comp){ const parts=[]; for(const k in (comp||{})){ const num=comp[k]||0; if(num<=0)continue;
  const nm=k==='levy'?'징집병':k==='knights'?'기사':(MAA_TYPES[k]?.n||k); parts.push(`${nm} ${Math.round(num).toLocaleString()}`); }
  return parts.join(' · ')||'—'; }
function battleModalHtml(d){ const b=d&&d.r; if(!b) return '<div>전투 기록 없음</div>';
  const atkWon=b.atkWins;
  const allPhases=['기동','초기 전투','후기 전투','추격'];
  const phaseBar=allPhases.map(ph=>{ const on=(b.phases||[]).includes(ph);
    return `<div style="flex:1;text-align:center;padding:3px 1px;font-size:.58rem;border-radius:3px;background:${on?'#3a2c18':'#15110a'};color:${on?'var(--gold)':'#5a5040'};border:1px solid ${on?'var(--gold-dim)':'#241a0e'}">${ph}</div>`;
  }).join('<span style="align-self:center;color:#5a5040;font-size:.55rem;padding:0 1px">▸</span>');
  const panel=(name,mine,comp0,t0,left,lost,won)=>`
    <div style="flex:1;background:#15110a;border:1px solid ${won?'var(--gold-dim)':'#3a241a'};border-radius:5px;padding:8px">
      <div style="display:flex;justify-content:space-between;font-size:.76rem;margin-bottom:4px">
        <b style="color:${mine?'#7a9ad0':'#d08a6a'}">${name}</b><span style="color:${won?'var(--gold)':'#9e5a5a'};font-size:.72rem">${won?'승리':'패배'}</span></div>
      <div style="font-size:.6rem;color:var(--parch-dim);margin-bottom:6px;line-height:1.4">${_compStr(comp0)}</div>
      <div style="display:flex;justify-content:space-between;font-size:.72rem"><span style="color:var(--parch-dim)">병력</span><span>${t0.toLocaleString()} → <b>${left.toLocaleString()}</b></span></div>
      <div style="display:flex;justify-content:space-between;font-size:.68rem;color:#c07a5a"><span>손실</span><span>${lost.toLocaleString()}</span></div>
    </div>`;
  ...
  if(b.pursuit) mods.push(`추격전 — 패주병 추가 손실`);
  return `
    <div style="display:flex;gap:2px;align-items:stretch;margin:4px 0 10px">${phaseBar}</div>
    <div style="display:flex;gap:6px;margin-bottom:10px">
      ${panel(d.atkMine?'아군 (공격)':d.atkName+' (공)', d.atkMine, b.atkComp0, b.atk0, atkLeft, atkLost, atkWon)}
      ${panel(d.defMine?'아군 (수비)':d.defName+' (수)', d.defMine, b.defComp0, b.def0, defLeft, defLost, !atkWon)}
    </div>
    <div style="font-size:.64rem;color:var(--parch-dim);line-height:1.8">${mods.map(m=>'• '+m).join('<br>')}</div>
    <div style="font-size:.6rem;color:#6a6048;margin-top:6px;text-align:right">전투 ${b.days}일 · ${d.cn}</div>`;
}
function showLastBattle(){ if(!state._lastBattle) return;
  popup({title:`${state._lastBattle.cn} 전투`, sub:'전투 보고', html:battleModalHtml(state._lastBattle)}); }
/* ════════════════════════════════════════════════════════════════════════════ */

/* 월 1회: 군별 개발도 성장 (건물 성장 + 재무관 + 이웃 전파 − 상한 둔화) */
function developmentPulse(){
  const cap=devCap(), p=playerChar();
  for(const cid in COUNTIES){ const c=COUNTIES[cid]; if(c.dev==null) continue;
    // 건물 개발성장 기여: 군 내 남작령 건물 pop 효과 합 × 지형 개발 배수
    let bld=0;
    for(const bid of (c.baronies||[])){ const b=BARONIES[bid]; if(!b?.builds) continue;
      for(const s of b.builds){ if(s.level>0){ const e=lvEff(s.line,s.level); bld+=(e.pop||0); } } }
    bld *= (1 + terrainDevMul(c.capital||''));
    // 재무관 기여는 increase_development 과업이 대상 군에 직접 반영(여기선 제외)
    let stew=0;
    // 이웃 전파: 더 높은 개발 군에서 확산
    let spread=0;
    for(const n of (COUNTY_ADJ[cid]||[])){ const nd=COUNTIES[n]?.dev||0; if(nd>c.dev) spread+=(nd-c.dev)*0.008; }
    const penalty = Math.max(0.1, 1 - c.dev/cap);
    let gain = (0.05 + bld + stew + spread) * penalty * 0.1;
    if(c.dev>=cap) gain = Math.min(gain,0);
    c.dev = Math.max(0, Math.min(cap, +(c.dev+gain).toFixed(3)));
  }
}

/* ════════════════════════════════════════════════════
   장악력(Control) — CK3 County Control 재현
   출처: gamerempire.net(2023-02-08)·ck3cheats.com — 군별 0~100,
     100 미만 1점당 세금 −1%·징집 −0.5% (0=세금0%·징집50%).
     정복 시 급락 · 평시 서서히 상승 · 무관장 「장악력 증대」 과업 · 휴양 궁전 +성장.
   ════════════════════════════════════════════════════ */
(()=>{ for(const cid in COUNTIES){ COUNTIES[cid].control = 90; } })(); // 시작=정착 상태(고장악)
function countyControl(cid){ const v=COUNTIES[cid]?.control; return v==null?90:v; }
function controlTaxMul(cid){ return Math.max(0, Math.min(1, countyControl(cid)/100)); }          // CK3: 세금 ×C/100
function controlLevyMul(cid){ return Math.max(0.5, Math.min(1, 0.5+countyControl(cid)*0.005)); }  // CK3: 징집 ×(0.5+C/200)
function baronyControlTaxMul(bid){ const cid=BARONY_COUNTY[bid]; return cid?controlTaxMul(cid):1; }
function baronyControlLevyMul(bid){ const cid=BARONY_COUNTY[bid]; return cid?controlLevyMul(cid):1; }
/* 월 1회: 평시 장악력 성장 + 휴양 궁전(공작건물) + 저장악 민심 침식(반란 연동) */
function controlPulse(){
  const p=playerChar();
  const leisureGrow={};
  if(p){ for(const d of heldDuchiesOf(p.id)){ const cap=duchyCapitalBarony(d), db=BARONIES[cap]?.duchyBuild;
  ...
  for(const cid in COUNTIES){ const c=COUNTIES[cid]; if(c.control==null) continue;
    ...
    if(c.control<40 && state.month%2===0){ const b=BARONIES[c.capital];
      if(b&&b.owner===state.player) b.pop=Math.max(0,(b.pop||60)-1); }
    // 이민족 군 → 직할 민심 추가 침식(문화 촉진 전까지)
    if(isForeignCultureCounty(cid) && state.month%3===0){ const b=BARONIES[c.capital];
      if(b) b.pop=Math.max(0,(b.pop||60)-1); }
  }
}

/* ════════════════════════════════════════════════════
   문화(Culture) — 최소 구현
   출처: Kingdom of Dublin/Munster(위키피디아)·CK3 문화 메커니즘(나무위키 크킹3/문화)
     1066 아일랜드: 대부분 게일(아일랜드), 노르드 항구(더블린·워터퍼드)=노르드-게일.
     CK3: 군 보유자와 다른 문화 → 민심·장악력 페널티. 재무관 「문화 촉진」으로 전환.
     (노르만은 1169년 이후라 1066엔 미배치)
   ════════════════════════════════════════════════════ */
const CULTURES = {
  irish:     { n:'아일랜드',    grp:'게일',      color:'#3d8a4a' },
  norsegael: { n:'노르드-게일', grp:'노르드-게일', color:'#5a6e9c' },
  norman:    { n:'노르만',      grp:'프랑스',     color:'#9c6b3c' }, // 정의만 — 1066엔 미배치
};
const COUNTY_CULTURE_INIT = { c_dublin:'norsegael', c_ormond:'norsegael' }; // 노르드 항구(더블린·워터퍼드)
(()=>{ for(const cid in COUNTIES){ COUNTIES[cid].culture = COUNTY_CULTURE_INIT[cid] || 'irish'; } })();
function playerCulture(){ return 'irish'; }                       // 우어 브리언 = 게일 아일랜드
function countyCulture(cid){ return COUNTIES[cid]?.culture || 'irish'; }
function cultureName(cid){ return CULTURES[countyCulture(cid)]?.n || '아일랜드'; }
/* 플레이어 직할 & 군 문화가 플레이어 문화와 다르면 이민족 군(페널티 대상) */
function isForeignCultureCounty(cid){ const cap=COUNTIES[cid]?.capital;
  return BARONIES[cap]?.owner===state.player && countyCulture(cid)!==playerCulture(); }
/* 전환 대상: 플레이어 직할 이민족 군 1개(수도 군 우선) */
function pickPromoteCultureTarget(){
  const p=playerChar(); if(!p) return null;
  const capCid=BARONY_COUNTY[p.region];
  if(capCid && isForeignCultureCounty(capCid)) return capCid;
  for(const cid in COUNTIES) if(isForeignCultureCounty(cid)) return cid;
  return null;
}

/* ════════════════════════════════════════════════════
   공작령(특수) 건물 — CK3 Duchy Buildings
   출처: CK3 Wiki Duchy_buildings · HighGroundGaming(2024-05-27)·TheGamer(2025-03-31)·GameRant(2023-05-25)
     · 조건: 공작령 de jure 전 군 보유 + de jure 수도 군 슬롯 1개 · 3단계(시대별 해제)
     · 왕실 무기고: 공작령 징집병 최대 +40% · 무장병 유지비↓ · MaA 강화
     · 공성 병기 제작소: 공작령 요새레벨 최대 +6 · 공성 강화
     · 마상 시합장: 기병 강화 · 무장병 유지비 −10%
     · 왕립 병기창: 보병(중보병·창병) 강화
     · 휴양 궁전: 위신 · 통제/민심 (CK3 Leisure Palace)
   환산: 요새 +1/2/3(게임), 징집 +8/14/20%, 유지비·MaA·위신 게임 규모 [밸런스 추정]
   ════════════════════════════════════════════════════ */
const DUCHY_BUILDINGS = {
  royal_armory:    { n:'왕실 무기고', icon:'', desc:'공작령 징집병 +%·무장병 유지비↓·보병/기병 강화',
    eff:[{levy:0.08,upkeep:0.05,inf:0.05,cav:0.05},{levy:0.14,upkeep:0.10,inf:0.10,cav:0.10},{levy:0.20,upkeep:0.15,inf:0.15,cav:0.15}] },
  siege_works:     { n:'공성 병기 제작소', icon:'', desc:'공작령 모든 남작령 요새레벨 +·공성 가속',
    eff:[{fort:1,siege:0.15},{fort:2,siege:0.28},{fort:3,siege:0.40}] },
  jousting_grounds:{ n:'마상 시합장', icon:'', desc:'경기병 강화·무장병 유지비 −',
    eff:[{cav:0.12,upkeep:0.05},{cav:0.20,upkeep:0.08},{cav:0.28,upkeep:0.10}] },
  crown_smithy:    { n:'왕립 병기창', icon:'', desc:'중보병·창병 강화·무장병 유지비 −',
    eff:[{inf:0.12,upkeep:0.03},{inf:0.20,upkeep:0.05},{inf:0.28,upkeep:0.08}] },
  leisure_palace:  { n:'휴양 궁전', icon:'', desc:'매월 위신·직할 민심',
    eff:[{prestige:0.3,pop:0.05},{prestige:0.6,pop:0.10},{prestige:1.0,pop:0.15}] },
};
const DUCHYB_COST=[120,220,360], DUCHYB_TIME=[8,10,12];
function duchyCapitalBarony(d){ const cs=DUCHIES[d]?.counties; return cs?.length?COUNTIES[cs[0]]?.capital:null; }
function duchyOfBarony(bid){ const cid=BARONY_COUNTY[bid]; return cid?COUNTIES[cid]?.duchy:null; }
/* 공작령 전 de jure 군 보유 여부 */
function ownsWholeDuchy(charId, d){ const cs=DUCHIES[d]?.counties||[];
  return cs.length>0 && cs.every(cid=>{ const cap=COUNTIES[cid]?.capital; return BARONIES[cap]?.owner===charId; }); }
function duchyBuildLevelCap(){ return Math.min(3, gameEra(state?.year)); } // 시대별: 1/2/3단계 해제
/* 플레이어가 보유한 공작 건물 효과 합산 */
function playerDuchyEff(){ const p=playerChar(); const t={levy:0,upkeep:0,fort:0,siege:0,cav:0,inf:0,prestige:0,pop:0};
  if(!p) return t;
  for(const d of heldDuchiesOf(p.id)){ const cap=duchyCapitalBarony(d); const db=BARONIES[cap]?.duchyBuild;
    if(db&&db.level>0){ const e=DUCHY_BUILDINGS[db.line]?.eff?.[db.level-1]; if(e) for(const k in e) t[k]=(t[k]||0)+e[k]; } }
  return t; }
/* 특정 남작령이 속한 공작령의 siege_works 요새 가산 / royal_armory 징집 배수 */
function duchySiegeFort(bid){ const d=duchyOfBarony(bid); if(!d) return 0;
  const cap=duchyCapitalBarony(d), db=BARONIES[cap]?.duchyBuild;
  if(db?.line==='siege_works'&&db.level>0) return DUCHY_BUILDINGS.siege_works.eff[db.level-1].fort||0; return 0; }
function duchyLevyMul(bid){ const d=duchyOfBarony(bid); if(!d) return 1;
  const cap=duchyCapitalBarony(d), db=BARONIES[cap]?.duchyBuild;
  if(db?.line==='royal_armory'&&db.level>0) return 1+(DUCHY_BUILDINGS.royal_armory.eff[db.level-1].levy||0); return 1; }
/* 공작 건물 건설 시작 */
function startDuchyBuild(d, lineId){ const p=playerChar(); if(!p) return false;
  if(!heldDuchiesOf(p.id).includes(d)){ log('해당 공작 작위를 보유해야 합니다.','dip'); return false; }
  if(!ownsWholeDuchy(p.id,d)){ log('공작령의 모든 군을 직접 보유해야 합니다.','dip'); return false; }
  const cap=duchyCapitalBarony(d), b=BARONIES[cap]; if(!b) return false;
  if(b.owner!==p.id){ log('공작령 수도를 보유해야 합니다.','dip'); return false; }
  let db=b.duchyBuild;
  if(db&&db.wip){ log('이미 건설 중입니다.','dip'); return false; }
  if(db&&db.line!==lineId&&db.level>0){ /* 교체: CK3는 교체 가능. 여기선 확인 */ }
  const curLv=(db&&db.line===lineId)?db.level:0, toLv=curLv+1;
  if(toLv>3){ log('최고 단계입니다.','dip'); return false; }
  if(toLv>duchyBuildLevelCap()){ log(`${ERA_NAME[gameEra(state?.year)+1]||'다음'} 시대가 필요합니다.`,'dip'); return false; }
  const cost=DUCHYB_COST[toLv-1], seat=BARONIES[p.region];
  if(!seat||seat.gold<cost){ log(`금이 부족합니다. (필요 ${cost})`,'dip'); return false; }
  seat.gold-=cost; playSynthSFX&&playSynthSFX('gold');
  if(!db||db.line!==lineId) b.duchyBuild=db={line:lineId, level:curLv, wip:null};
  db.wip={to:toLv, prog:0};
  log(`${DUCHIES[d].n} 수도에 <b>${DUCHY_BUILDINGS[lineId].n}</b> ${toLv}단계 건설 시작 (${DUCHYB_TIME[toLv-1]}개월·금${cost}).`,'good');
  return true; }
/* 월간: 공작 건물 건설 진행 + 휴양 궁전 위신/민심 */
function duchyPulse(){
  for(const bid in BARONIES){ const b=BARONIES[bid], db=b.duchyBuild; if(!db) continue;
    if(db.wip){ db.wip.prog++;
      if(db.wip.prog>=DUCHYB_TIME[db.wip.to-1]){ db.level=db.wip.to; db.wip=null;
        if(b.owner===state.player) log(`<b>${DUCHY_BUILDINGS[db.line].n}</b> ${db.level}단계 완공!`,'good'); }
    } }
  const p=playerChar(); if(!p) return;
  const eff=playerDuchyEff();
  if(eff.prestige>0) state.prestige=(state.prestige||0)+eff.prestige;
  if(eff.pop>0){ const seat=BARONIES[p.region]; if(seat) seat.pop=Math.min(100,(seat.pop||60)+eff.pop); }
}
...
// ── 초기 공작 작위 부여 (CK3 1066 아일랜드: 공작은 먼스터·코나위트 둘뿐) ──
// 무르하드: 먼스터 공작 (토몬드/에니스 직할 + 오몬드 봉신, 데스몬드는 독립 백작)
murchad.heldTitles.push('d_munster');
kConn.heldTitles.push('d_connacht');   // 코나위트 공작 (코나위트 직할 + 메이요 봉신)
// CK3 1066: 레인스터·더블린·미스·브레프네·얼스터는 공작위 없는 독립 백작
...
function canUsurpTitle(charId, tid){
  ...
  // de jure 지배율 51%+
  const ctrl=deJureControl(charId,tid);
  if(ctrl<0.51) return {ok:false, reason:`규범 권역 백작령 51% 지배 필요 (현재 ${Math.round(ctrl*100)}%)`};
...
function cleanupAfterDeath(c){
  ...
  // ── 작위(heldTitles) 상속/소멸 (CK3: 상위 작위는 권역 최대 지배자=주 상속자에게 이관, 아니면 소멸) ──
  const titles=(c.heldTitles||[]).slice();
  for(const tid of titles){
    const tinfo=titleInfo(tid);
    if(!tinfo||!tinfo.counties||!tinfo.counties.length) continue;
    // 권역(de jure)을 가장 많이 지배한 살아있는 통치자 탐색 (직계·방계 무관, 플레이어 제외)
    let topHolder=null, topShare=0;
    for(const h of Object.values(chars)){
      if(h.dead||!h.ruler||h.id===state.player) continue;
      const realm=new Set(realmCountiesOf(h.id));
      const owned=tinfo.counties.filter(cid=>realm.has(cid)).length;
      if(owned>topShare){ topShare=owned; topHolder=h; }
    }
    if(topHolder && topShare/tinfo.counties.length>=0.34){
      if(!topHolder.heldTitles) topHolder.heldTitles=[];
      if(!topHolder.heldTitles.includes(tid)) topHolder.heldTitles.push(tid);
    }
    // 아니면 소멸 (de jure 권역 정의는 남으므로 누구든 재창설 가능)
  }
  c.heldTitles=[]; // 사망자 작위 비움
  // ── 봉신 재배치 (CK3: 죽은 주군의 봉신은 그 위 주군에게, 없으면 독립) ──
  // ── 봉신 재배치 (CK3: 죽은 주군의 봉신은 주 상속자에게 승계, 상위 주군이 있으면 그에게) ──
  const seatHeir = c.liege ? null : ownerOf(c.region); // 독립 통치자면 수도를 물려받은 주 상속자
  for(const id in chars){
    const v=chars[id];
    if(!v.dead && v.liege===c.id){
      if(c.liege) v.liege = c.liege;                                          // 상위 주군에게
      else if(seatHeir && seatHeir.id!==c.id && !seatHeir.dead) v.liege = seatHeir.id; // 주 상속자에게 (realm 유지)
      else v.liege = null;                                                    // 후계자 없으면 독립
    }
  }
}
...
function pickGuardian(c,adults){
  const opts=adults.slice(0,5).map(g=>({
    t:`${g.name}`, d:`${SKILLS[c.eduFocus]} ${stat(g,c.eduFocus)} · 학습력 ${stat(g,'learn')} · 성격: ${g.traits.map(t=>TRAITS[t].n).join('·')||'—'}`,
  ...
  popup({title:`${c.name}의 후견인`, sub:'후견 — 교육의 질을 결정합니다',
    body:'후견인의 해당 스킬(×0.4)과 학습력(×0.2)이 교육 점수에 영향을 줍니다.\n아이는 후견인의 성격을 닮아갑니다.', opts});
}
...
function monthlyPulse(){
  ...
  guestActivityPulse(); // 게스트 참석 활동도 월 단위로 진행 (플레이어 활동과 동일)
  maaUpkeepPulse(); // 무장병 월 유지비 차감
...
/* 권세 봉신이 자문회에 있는지 — 없으면 -40 호감 페널티 적용 대상 */
...
const VASSAL_STANCES={
  belligerent:{ n:'호전적', icon:'', desc:'고압적 통치·결투 승리·사냥 선호. 전투력 높은 후계자 선호',  pref:'mar' },
  courtly:    { n:'궁정적', icon:'', desc:'궁정직·작위 하사·연회 선호. 외교력 높은 후계자 선호',       pref:'dip' },
  glory:      { n:'영광추구', icon:'', desc:'분할상속·정복 전쟁 승리 선호. 전투력 높은 후계자 선호',   pref:'mar' },
  parochial:  { n:'편협',   icon:'', desc:'건물 건설·내정 담당 선호. 고압적 통치 싫어함. 관리력 높은 후계자 선호', pref:'stew' },
  zealot:     { n:'광신도', icon:'✝', desc:'미덕 특성·사원 건설·순례 선호. 죄악 특성 싫어함. 학습력 높은 후계자 선호', pref:'learn' },
...
/* ── 봉신 계약 협상 (CK3 Negotiation 근사) ──
   인상 요구 시 봉신이 호감·권력·스탠스·인상폭을 따져 수락/거부함. 인하(양보)는 무조건 수락.
   개념 출처: CK3 — 의무 인상은 폭정·봉신 반발을 유발 */
function obligationAcceptance(v, kind, targetLevel){
  const p=playerChar(); if(!p||!v) return 0;
  const ct=v.contract||initVassalContract(v);
  const curIdx=OBLIG_ORDER.indexOf(ct[kind]);
  const tgtIdx=OBLIG_ORDER.indexOf(targetLevel);
  if(tgtIdx<=curIdx) return 100;            // 인하·유지는 무조건 수락
  let acc=68;
  acc += opinion(v,p)*0.5;                  // 호감 1당 +0.5%
  acc -= (tgtIdx-curIdx)*22;                // 인상 폭 1단계당 -22
  acc -= tgtIdx*4;                          // 높음·막대로 갈수록 추가 저항(완만)
  if(powerfulVassalsOf(p.id).includes(v.id)) acc -= 18; // 권세 봉신
  const st=vassalStance(v);
  if(st==='landholder') acc -= 20;          // 소영주: 낮은 왕권 선호
  else if(st==='parochial') acc -= 10;
  else if(st==='belligerent') acc -= 6;
  return Math.max(3, Math.min(97, Math.round(acc)));
}
/* 봉신에게 의무 조정 제안 — 인상은 수락 판정, 인하는 양보(무조건 수락) */
function proposeObligation(vId, kind, targetLevel){
  const v=chars[vId]; if(!v) return;
  const p=playerChar(); if(!p) return;
  const ct=initVassalContract(v);
  const curIdx=OBLIG_ORDER.indexOf(ct[kind]), tgtIdx=OBLIG_ORDER.indexOf(targetLevel);
  if(tgtIdx===curIdx||tgtIdx<0) return;
  const kn=kind==='tax'?'세금':'레비';
  const tbl=kind==='tax'?OBLIGATION_TAX:OBLIGATION_LEVY;
  if(tgtIdx<curIdx){
    // 인하 = 양보 → 무조건 수락, 호감 상승
    ct[kind]=targetLevel;
    chOp(v,p, 8*(curIdx-tgtIdx));
    log(`<b>${v.name}</b>의 ${kn} 의무를 <b>${tbl[targetLevel].n}</b>(으)로 인하했습니다. 봉신이 만족합니다.`,'good');
    renderFief();
    return;
  }
  // 인상 요구 → 수락 판정
  const acc=obligationAcceptance(v,kind,targetLevel);
  if(Math.random()*100 < acc){
    ct[kind]=targetLevel;
    chOp(v,p, -6*(tgtIdx-curIdx));           // 동의해도 약간의 불만
    log(`<b>${v.name}</b>이(가) ${kn} 인상(<b>${tbl[targetLevel].n}</b>)을 받아들였습니다.`,'dip');
  } else {
    chOp(v,p, -10);                          // 압박에 대한 반발
    log(`<b>${v.name}</b>이(가) ${kn} 인상 요구를 <b>거부</b>했습니다. 호감이 하락했습니다.`,'war');
  }
  renderFief();
}
/* 봉신 계약 협상 UI 한 줄 (세금/레비) — 현재 단계 + 인하/인상요구 버튼(수락% 표시) */
function obligNegotiateRow(v, kind){
  const ct=v.contract||initVassalContract(v);
  const tbl=kind==='tax'?OBLIGATION_TAX:OBLIGATION_LEVY;
  const kn=kind==='tax'?'세금':'레비';
  const curIdx=OBLIG_ORDER.indexOf(ct[kind]);
  const cur=tbl[ct[kind]];
  const btn='display:inline-flex;align-items:center;gap:3px;background:#1c1509;border:1px solid var(--line);color:var(--parch);font-size:.6rem;padding:2px 6px;border-radius:3px;cursor:pointer;font-family:inherit';
  let row=`<div style="display:flex;gap:5px;align-items:center;flex-wrap:wrap;margin-top:4px">
    <span style="font-size:.62rem;color:var(--parch-dim);width:28px">${kn}</span>
    <span style="font-size:.66rem;color:var(--gold-dim);min-width:62px">${cur.n} ${Math.round(cur.rate*100)}%</span>`;
  if(curIdx>0){
    const down=OBLIG_ORDER[curIdx-1];
    row+=`<button style="${btn}" onclick="proposeObligation('${v.id}','${kind}','${down}')">▼ 인하</button>`;
  }
  if(curIdx<OBLIG_ORDER.length-1){
    const up=OBLIG_ORDER[curIdx+1];
    const acc=obligationAcceptance(v,kind,up);
    const ac=acc>=66?'#7a9a6a':acc>=40?'#c8a24a':'#9e5a5a';
    row+=`<button style="${btn}" onclick="proposeObligation('${v.id}','${kind}','${up}')">▲ ${tbl[up].n} 요구 <span style="color:${ac}">${acc}%</span></button>`;
  }
  row+=`</div>`;
  return row;
}
...
function holdingIncome(bid, ownerId, stewStat, overPenalty){
  ...
  if(b.type==='empty') return 0; // 빈 슬롯: 수입 없음
...
function goldPulse(){
  // ① 모든 남작령: 병력 자연 회복 (barracks 보너스 + 개발도 levy size)
  for(const bid in BARONIES){
    const b=BARONIES[bid];
    const regen=4+buildingBonus(bid,'troops_regen');
    const effCap=Math.round((b.cap||b.troops)*baronyDevMul(bid)*duchyLevyMul(bid)*baronyControlLevyMul(bid)); // 개발도 × 공작 무기고 × 장악력
    b.troops=Math.min(effCap, b.troops+regen);
  }
...
function declareWar(atk,def,targetRid){
  ...
  if(def.id===state.player){
    ...
    const _cmdLine = _myCmd2 ? `\n\n⚔ 지휘관: ${_myCmd2.name} (전투력 ${stat(_myCmd2,'mar')} · 용맹 ${stat(_myCmd2,'prow')})` : '';
...
/* ── 무장병 (Men-at-Arms) — CK3 MAA 시스템 ──
   연대 유형별 전력·유지비·상성. 작위 등급에 따라 보유 연대 한도가 늘어남.
   전력에 합산되며 매월 금 유지비가 든다. */
/* 무장병 병종 정의.
   dmg(피해도)/tough(유지력)/pursuit(추격)/screen(차단) = CK3 정확값 [나무위키 군사].
   ※ power/hire/upkeep는 기존 경제·추상전투 호환 위해 유지(Phase 4 전투 엔진에서 4스탯으로 전환). */
const MAA_TYPES = {
  spearmen:    { n:'창병',   icon:'', size:100, counter:'기병',   hire:80,  upkeep:0.5, power:200, dmg:22, tough:24, pursuit:0,  screen:0  },
  archers:     { n:'궁병',   icon:'', size:100, counter:'산병',   hire:100, upkeep:0.6, power:220, dmg:25, tough:10, pursuit:0,  screen:0  },
  skirmishers: { n:'산병',   icon:'', size:100, counter:'중보병', hire:70,  upkeep:0.4, power:190, dmg:10, tough:16, pursuit:10, screen:16 },
  heavy_inf:   { n:'중보병', icon:'',  size:100, counter:'창병',   hire:130, upkeep:0.8, power:280, dmg:32, tough:22, pursuit:0,  screen:0  },
  light_cav:   { n:'경기병', icon:'', size:100, counter:'궁병',   hire:120, upkeep:0.7, power:240, dmg:22, tough:15, pursuit:30, screen:30 },
};
/* 징집병·기사 전투 스탯 (CK3, Phase 4 전투 엔진에서 사용) [나무위키 군사] */
const LEVY_DMG=10, LEVY_TOUGH=10;                            // 징집병 1인당
...
/* ── 건물(병영·마구간·철공소·요새화)의 무장병 보너스 → 무장병 유형별 전력 배수 ──
   ...
   직할 남작령 건물 maa를 합산하되 계수·상한으로 스택 폭주 방지(최대 +120%). */
const MAA_BLD_MAP = { heavy_infantry:'heavy_inf', pikemen:'spearmen', light_cavalry:'light_cav',
  heavy_cavalry:'light_cav', archers:'archers', skirmishers:'skirmishers' };
const MAA_BLD_COEF = 0.08, MAA_BLD_CAP = 1.2;
function maaBuildBonus(){ const p=playerChar(); const raw={};
  if(p){ for(const bid of regionsOf(p.id)){ const b=BARONIES[bid]; if(!b?.builds) continue;
    for(const s of b.builds){ if(s.level<=0) continue; const ma=maaAt(s.line, s.level); if(!ma) continue;
      for(const u in ma){ const mk=MAA_BLD_MAP[u]; if(!mk) continue;
        raw[mk]=(raw[mk]||0)+(ma[u].dmg||0)+(ma[u].tough||0); } } } }
  const mult={}; for(const k in MAA_TYPES) mult[k]=1+Math.min(MAA_BLD_CAP,(raw[k]||0)*MAA_BLD_COEF);
  // 공작 건물(왕실 무기고·왕립 병기창·마상 시합장) 보병/기병 강화 추가 배수
  const de=playerDuchyEff();
  if(de.inf>0){ mult.heavy_inf*=(1+de.inf); mult.spearmen*=(1+de.inf); }
  if(de.cav>0){ mult.light_cav*=(1+de.cav); }
  return mult; }
/* 무장병이 더하는 전력 (플레이어 군대) — 건물 보너스 반영 */
function maaPower(){ const m=ensureMAA(), mb=maaBuildBonus(); let t=0;
  for(const k in m) t+=(m[k]||0)*(MAA_TYPES[k]?.power||0)*(mb[k]||1); return t; }

/* ── 무장병 병종 상성 (CK3: 편성 상성이 전투 효율에 영향) ──
   CK3 루프: 산병>중보병>창병>기병>궁병>산병. key → 이 key가 카운터하는 key */
const MAA_COUNTERS = { spearmen:'light_cav', archers:'skirmishers', skirmishers:'heavy_inf', heavy_inf:'spearmen', light_cav:'archers' };
/* 적 군대 주력 병종 — char id로 결정적 시드(스카우트 가능·일관) */
function enemyMaaType(id){ const keys=Object.keys(MAA_TYPES); let h=0; const s=String(id||'');
  for(let i=0;i<s.length;i++) h=(h*31+s.charCodeAt(i))>>>0; return keys[h%keys.length]; }
/* 플레이어 편성이 '적 주력을 카운터하는' 공격적 상성율 (+, [0,0.18]) — 역상성은 적 측 델타가 처리 */
function maaCounterMod(enemyType){
  const m=ensureMAA(), mb=maaBuildBonus(); let total=0, counter=0;
  for(const k in m){ const pw=(m[k]||0)*(MAA_TYPES[k]?.power||0)*(mb[k]||1); total+=pw;
    if(MAA_COUNTERS[k]===enemyType) counter+=pw; } // 내 k가 적 주력을 카운터
  return total>0 ? 0.18*counter/total : 0;
}
/* 플레이어 주력(최강) 병종 */
function playerMaaType(){ const m=ensureMAA(), mb=maaBuildBonus(); let best=null,bp=0;
  for(const k in m){ const pw=(m[k]||0)*(MAA_TYPES[k]?.power||0)*(mb[k]||1); if(pw>bp){bp=pw;best=k;} }
  return best; }
/* 전투에 가산할 플레이어 상성 전력 (양수=유리) */
function maaCounterDelta(enemyChar){ return enemyChar ? maaPower()*maaCounterMod(enemyMaaType(enemyChar.id)) : 0; }

/* ── 무장병 병종별 지형 유불리 (CK3: 병종마다 유리/불리 지형) ──
   산병=험지 유리, 창병=구릉/산악 유리, 경기병=개활지 유리·험지 불리,
   중보병=지형 보정 없음(CK3 확정), 궁병=기본형 중립. (±는 해당 병종 전력 비례) */
const MAA_TERRAIN = {
  skirmishers: { hills:0.10, mountains:0.10, forest:0.10, wetlands:0.10 },
  spearmen:    { hills:0.10, mountains:0.10 },
  light_cav:   { plains:0.10, farmlands:0.10, floodplain:0.10, drylands:0.10, hills:-0.10, mountains:-0.15, forest:-0.10, wetlands:-0.10 },
  archers:     {},
  heavy_inf:   {},
};
/* 전장 지형에서 플레이어 무장병 편성의 순 지형 전력 보정 */
function maaTerrainDelta(terrainKey){
  if(!terrainKey) return 0;
  const m=ensureMAA(), mb=maaBuildBonus(); let delta=0;
  for(const k in m){ const tmod=MAA_TERRAIN[k]?.[terrainKey]||0;
    if(tmod) delta += (m[k]||0)*(MAA_TYPES[k]?.power||0)*(mb[k]||1)*tmod; }
  return delta;
}
/* ── 적 측 무장병 보정 (CK3 대칭: 적도 상성·지형 효과를 받음) ──
   적은 단일 주력 병종으로 추상화 — 총 전력의 일부를 명목 무장병으로 간주 */
const ENEMY_MAA_FRAC = 0.35;
function enemyMaaPower(c){ return c ? power(c)*ENEMY_MAA_FRAC : 0; }
/* 적 주력이 플레이어 주력을 카운터하면 +, 지형이 적 주력에 유불리면 ± — 적 전력에 가산 */
function enemyMaaDelta(enemyChar, terrainKey){
  if(!enemyChar) return 0;
  const eType=enemyMaaType(enemyChar.id), pType=playerMaaType();
  let mod=0;
  if(pType && MAA_COUNTERS[eType]===pType) mod+=0.18;       // 적 주력이 내 주력을 카운터
  if(terrainKey) mod+=(MAA_TERRAIN[eType]?.[terrainKey]||0); // 적 주력의 지형 유불리
  return enemyMaaPower(enemyChar)*mod;
}
/* 무장병 월 유지비 합 */
function maaUpkeep(){ const m=ensureMAA(); let g=0; for(const k in m) g+=(m[k]||0)*(MAA_TYPES[k]?.upkeep||0);
  const up=playerDuchyEff().upkeep||0; return g*(1-Math.min(0.30,up)); } // 공작 건물 유지비 감면(최대 −30%)
function hireMAA(type){
  initAudio(); const p=playerChar(); if(!p) return;
  const t=MAA_TYPES[type]; if(!t) return;
  const m=ensureMAA(); const seat=BARONIES[p.region];
  if(maaCount()>=maaCap(p)) return;        // 한도 초과
  if(!seat||seat.gold<t.hire) return;      // 금 부족
  playSynthSFX&&playSynthSFX('gold');
  seat.gold-=t.hire; m[type]++;
  log(`<b>${t.n}</b> 연대(${t.size}명)를 모집했습니다. (금 -${t.hire})`,'war');
  renderWar(); renderHeader&&renderHeader();
}
function disbandMAA(type){
  const p=playerChar(); if(!p) return;
  const m=ensureMAA(); if(!(m[type]>0)) return;
  m[type]--;
  log(`<b>${MAA_TYPES[type].n}</b> 연대를 해산했습니다.`,'dip');
  renderWar(); renderHeader&&renderHeader();
}
/* 월간 무장병 유지비 차감 */
function maaUpkeepPulse(){
  const p=playerChar(); if(!p) return;
...
function power(c){
  ...
  if(c.id===state.player) t+=maaPower(); // 무장병(MAA) 전력 합산
  if(!t) t=150;
...
/* ── 보급 한계 (Supply Limit) ── CK3: 영지·작위 규모에 비례.
   동원 군세가 이를 넘으면 보급난으로 사기↓·소모↑(장기전 압박) */
function supplyLimit(c){
  let lim = 1500;
  lim += heldDuchiesOf(c.id).length*500;
  lim += heldKingdomsOf(c.id).length*1000;
  lim += stat(c,'stew')*50;
  // 건물 보급(농장·저습지 농장 등) + 수도 지형 보급 배수 (CK3: 농지 +35%·산악 −50%)
  let bldSupply=0; for(const bid of regionsOf(c.id)) bldSupply += (BARONIES[bid]?.supplyBld||0)*5;
  lim += bldSupply;
  // 개발도 보급(CK3 +150/dev → 게임 +15/dev): 직할 군별 합산
  const myCounties=new Set(); for(const bid of regionsOf(c.id)){ const cc=BARONY_COUNTY[bid]; if(cc)myCounties.add(cc); }
  for(const cc of myCounties) lim += countyDevSupply(cc);
  const capTerr = c.region ? terrainSupplyMul(c.region) : 0;
  lim = Math.round(lim*(1+capTerr));
  return Math.round(lim);
}
/* 보급 계수 — 한계 이내면 1.0, 초과 시 전력 감소(최저 0.65) */
function supplyFactor(c){
  const army=power(c), lim=supplyLimit(c);
  if(army<=lim) return 1;
  const over=(army-lim)/lim;
  return Math.max(0.65, 1 - over*0.4);
}
...
function warPulse(){
  state.wars=state.wars.filter(w=>{
    ...
    let pa=powerWithCommander(a,w,'atk')+allyPow+vPow;
    const garrisonPow=(tCid&&COUNTIES[tCid])?COUNTIES[tCid].baronies.reduce((s,bid)=>s+(BARONIES[bid]?.garrison||0),0):0;
    const terrAdvMul=1+(tCid?terrainAdvantage(COUNTIES[tCid]?.capital):0)*0.015; // CK3 지형 방어 유리함(advantage pt당 +1.5%)
    let pd=(powerWithCommander(d,w,'def')+vassalsOf(w.def).reduce((s,v)=>s+power(v)*0.4,0)+garrisonPow)*terrAdvMul;
    // 보급 한계 초과 시 플레이어 군대 사기·전력 감소 (장기전·대군 압박)
    if(w.atk===state.player) pa*=supplyFactor(a);
    if(w.def===state.player) pd*=supplyFactor(d);
    // ── 무장병 병종 상성·지형 (CK3, 양측 대칭) — 플레이어/적 각자 전력에 가산
    const battleTerr=tCid?countyTerrain(tCid):null;
    const enemyOfPlayer = w.atk===state.player ? d : (w.def===state.player ? a : null);
    if(enemyOfPlayer){
      const pDelta = maaCounterDelta(enemyOfPlayer) + maaTerrainDelta(battleTerr); // 플레이어 편성
      const eDelta = enemyMaaDelta(enemyOfPlayer, battleTerr);                     // 적 편성
      if(w.atk===state.player){ pa+=pDelta; pd+=eDelta; }
      else { pd+=pDelta; pa+=eDelta; }
    }
    ...
    // ── 보급 초과 소모 — 플레이어 군세가 보급 한계 초과 시 추가 이탈병
    if(w.atk===state.player||w.def===state.player){
    ...
    // 지형 보급: 방어적 지형(보급 음수, 산악/구릉 등) 공성 시 공격자 추가 소모(CK3 supply limit 반영)
    const tSupplyMul=tCid?terrainSupplyMul(COUNTIES[tCid]?.capital):0;
    if(tSupplyMul<0){ for(const bid of regionsOf(a.id)) BARONIES[bid].troops=Math.max(60,Math.round(BARONIES[bid].troops*(1+tSupplyMul*0.06))); }
    ...
    if(w.months%3===0&&(w.atk===state.player||w.def===state.player)){
      ...
      const enemyC = w.atk===state.player ? d : a;
      const pDelta = maaCounterDelta(enemyC) + maaTerrainDelta(battleTerr); // 내 편성 효과
      const eDelta = enemyMaaDelta(enemyC, battleTerr);                     // 적 편성 효과
      const net = pDelta - eDelta;
      const mStr = net>5 ? ' · 무장병 우위' : net<-5 ? ' · 무장병 열위' : '';
      log(`전황: ${my>=0?'+':''}${Math.round(my)}%${siege} — ${my>30?'아군 우세':my<-30?'적군 우세':'교착 상태'}${cmdStr}${mStr}`,'war');
    }
...
const NPC_ACTIVITIES = [
  ...
  /* ① 연회 — 관계 개선, 금 소모 */
  {
    ...
    run:(r,reg,adj)=>{
      ...
      // CK3: 친구·동맹·가문·봉신인 플레이어는 기본 손님 목록에 포함됨
      const pc=playerChar();
      if(pc&&guests.some(g=>g.id===pc.id)&&npcSocialBondToPlayer(r)&&!invited.some(g=>g.id===pc.id)) invited.push(pc);
      invited.forEach(g=>{ chOp(g,r,10); chOp(r,g,7); });
      if(invited.some(g=>g.id===state.player)&&!playerBusy()&&npcSocialBondToPlayer(r)&&tryPlayerInvite()){
        const p=playerChar();
  ...
  /* ② 사냥 — 소규모 유대, 금 소모 */
  {
    ...
    run:(r,reg,adj)=>{
      ...
      if(partner.id===state.player&&!playerBusy()&&npcSocialBondToPlayer(r)&&tryPlayerInvite()){
  ...
  /* ④ 외교 방문 — 우호 관계 강화 */
  {
    ...
    run:(r,reg,adj)=>{
      ...
      if(target.id===state.player&&tryPlayerInvite()){
    ...
    run:(r,reg)=>{
      ...
      if(r.id===state.player||Object.values(chars).some(c=>c.id===state.player&&(c.father===r.id||c.mother===r.id))){
        popup({title:'후계자 교육',sub:`${r.name}의 가문`,
          body:`${kid.name}이(가) 궁중 교육을 받으며 성장하고 있습니다.\n「${['외교력','전투력','관리력','계책력','학습력'][skills.indexOf(sk)]} 분야에서 두각을 나타내고 있습니다.」`,
          opts:[{t:'흐뭇하게 지켜본다',f:()=>{}}]});
      } else {
...
/* CK3 활동 손님 카테고리 — 호스트가 플레이어를 초대할 관계가 있는가
   (주군/봉신/동료봉신/배우자/근친/동맹/친구 수준 호감). 무관한 이웃은 초대 안 함. */
function npcSocialBondToPlayer(r){
  const p=playerChar(); if(!p||r.id===p.id) return false;
  if(r.liege===p.id || p.liege===r.id) return true;                              // 주군-봉신
  if(r.liege && r.liege===p.liege) return true;                                  // 동료 봉신
  if(r.spouse===p.id || p.spouse===r.id) return true;                            // 배우자
  if(r.father===p.id||r.mother===p.id||p.father===r.id||p.mother===r.id) return true; // 직계 근친
  if(r.dyn && p.dyn && r.dyn===p.dyn) return true;                               // 같은 가문
  if(isAllied(r.id,p.id)) return true;                                           // 동맹
  if(opinion(r,p)>=25) return true;                                              // 친구 수준 호감 (이 게임 척도, 친구 시스템 근사)
  return false;
}
/* 동시다발 초대 방지 — CK3: 활동은 중복 불가(여행/체류). 플레이어 초대 팝업 최소 간격(개월). */
const PLAYER_INVITE_GAP=6;
function canShowPlayerInvite(){
  const now=state.year*12+state.month;
  return (now-(state._lastPlayerInvite||-999))>=PLAYER_INVITE_GAP;
}
function markPlayerInvite(){ state._lastPlayerInvite=state.year*12+state.month; }
/* 원자적 게이트 — 가능하면 즉시 슬롯 점유. 같은 달 중복 팝업 완전 차단. */
function tryPlayerInvite(){
  if(!canShowPlayerInvite()) return false;
  markPlayerInvite();
  return true;
}
...
function npcActivityPulse(){
  for(const id in chars){
    ...
    // 활동 선택은 균등 무작위 (CK3: AI는 플레이어를 겨냥해 활동을 집중하지 않음)
...
function npcTitlePulse(){
  ...
  for(const r of rulers){
    const ambitious=r.traits.includes('ambitious');
    const ambit=0.4 + aiW(r,'bold')*0.06 + (ambitious?0.3:0);
    if(Math.random()>ambit) continue;
    // 야심가는 공작위 상한을 높여 라이벌로 성장 가능 (일반 2개, 야심가 4개)
    const duchyCap = ambitious ? 4 : 2;
    let acted=false;

    // ① 빈 공작위 창설 (de jure 51% 지배)
    if(heldDuchiesOf(r.id).length < duchyCap){
      for(const did of Object.keys(DUCHIES)){
        if(holdsTitle(r.id,did) || titleHolder(did)) continue; // 빈 작위만
    ...
    if(!acted && heldDuchiesOf(r.id).length < duchyCap){
      for(const did of Object.keys(DUCHIES)){
        ...
        if(chk.ok){
          ...
          log(`<b>${r.name}</b>이(가) <b>${chk.info.n}</b>을(를) ${h.name}에게서 찬탈했습니다.`,'dip');
          break;
        }
      }
    }
    // ③ 왕국 작위 창설/찬탈 (공작 2개+ 보유 시) — 라이벌 하이킹 등극 경로
    if(!acted && heldDuchiesOf(r.id).length>=2){
...
/* 라이벌 세력 진척 알림 — 패배(게임오버) 없음. 향후 맵 확장(브리튼 제도) 대비 */
function checkRivalProgress(){
  let topRival=null, topD=0, kingRival=null;
  for(const c of Object.values(chars)){
    if(c.dead||c.id===state.player) continue;
    if(heldKingdomsOf(c.id).length>0) kingRival=c;
    const d=heldDuchiesOf(c.id).length;
    if(d>topD){ topD=d; topRival=c; }
  }
...
/* ── NPC 규범 권역(de jure) 전쟁 AI — CK3식 통일 경쟁 ──
   공작령에 발판을 둔 독립 통치자가 자기 규범 권역 내 '독립' 백작을 흡수(승리 시 봉신화).
   세력 확장 → 작위 창설(npcTitlePulse) → 왕국 → 하이킹 라이벌로 성장하는 눈덩이. */
function npcDejureWarPulse(){
  const rulers=Object.values(chars).filter(c=>!c.dead&&c.ruler&&c.id!==state.player&&!c.liege);
  for(const r of rulers){
    if(state.wars.some(w=>w.atk===r.id||w.def===r.id)) continue; // 이미 전쟁 중이면 skip
    const ambitious=r.traits.includes('ambitious');
    const hasDuchy=heldDuchiesOf(r.id).length>0;
    const aggr = 0.16 + aiW(r,'bold')*0.03 + aiW(r,'greed')*0.025 + (ambitious?0.12:0) + (hasDuchy?0.08:0);
    if(Math.random()>aggr) continue;
    // 발판 있는 공작령 중 '자신의 지배율이 가장 높은' 본거지부터 통합 (51% 도달 → 작위 창설 유도)
    const myPow=power(r);
    const footholds=Object.keys(DUCHIES)
      .map(did=>({did, myCtrl:deJureControl(r.id,did)}))
      .filter(x=>x.myCtrl>0)
      .sort((a,b)=>b.myCtrl-a.myCtrl);
    ...
    for(const {did,myCtrl} of footholds){
      for(const cid of (DUCHIES[did].counties||[])){
        ...
        if(myPow < hp*1.05) continue;                          // 확실히 이길 싸움만
        if(hp < bestPow){ best={target:holder, cid, did}; bestPow=hp; } // 본거지 내 가장 약한 표적
      }
      if(best) break; // 본거지(최고 지배율 공작령)에서 표적을 찾으면 그곳부터 마무리
    }
    // 본거지 통합을 마친 공작 → 인접 독립 백작 정복으로 새 권역 발판 확보 (CK3 대왕의 확장)
    if(!best && heldDuchiesOf(r.id).length>0){
      const adjIndep=new Set();
      ...
      for(const hid of adjIndep){
        const h=chars[hid]; const hp=power(h);
        if(myPow<hp*1.15) continue;                 // 정복은 더 큰 우세 요구
        const cid=directCountiesOf(hid)[0]; if(!cid) continue;
        if(hp<wpow){ wpow=hp; wtgt=h; wcid=cid; }
      }
      if(wtgt){
        const did=Object.keys(DUCHIES).find(d=>DUCHIES[d].counties.includes(wcid));
        best={target:wtgt, cid:wcid, did};
      }
    }
    if(!best) continue;
    declareWar(r, best.target, best.cid);
    const w=state.wars.find(x=>x.atk===r.id&&x.def===best.target.id);
    if(w){ w.dejure=true; w.dejureCids=[best.cid]; }
    npcGrantRevenge(best.target, best.cid);
    log(`<b>${r.name}</b>이(가) 규범 권역 권리로 <b>${best.target.name}</b>에게 선전포고했습니다 (${DUCHIES[best.did]?.n||COUNTIES[best.cid]?.n||'규범 권역'}).`,'war');
  }
}
...
function aiPulse(){
  ...
  npcCouncilPulse();
  npcActivityPulse();
  npcDejureWarPulse(); // NPC 규범 권역 전쟁 — CK3식 통일 경쟁
  ...
  // 분기별 작위 창설 점검 (51% 도달 시 빠르게 공작/왕국 창설 → 통일 경쟁 가속)
  else if(state.month===4||state.month===7||state.month===10){
    npcTitlePulse();
  }
...
"당신이 너무 지쳐 보여요. 잠시라도 쉬어요."`,
  ...
  {cond:c=>stat(c,'intr')>=6, w:2, run:c=>popup({title:'밀정의 보고', sub:'계책',
    ...
    opts:[
      {t:'약점을 캔다', d:'계책력 경험, 스트레스 +5', f:()=>{addStress(c,5,'은밀한 모의'); state.prestige+=8; log('약점을 손에 넣었습니다. 언젠가 쓸모가 있겠지요.','dip');}},
      {t:'관심 없다', d:'—', f:()=>{}},
    ]})},
  ...
  /* ── 외교/이웃 ── */
  {cond:c=>canShowPlayerInvite()&&Object.values(chars).some(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)>10), w:1, run:c=>{
  ...
  {cond:c=>c.lifestyle==='mar'&&stat(c,'mar')>=8, w:2, run:c=>popup({title:'전략의 통찰', sub:'군사 인생관',
  ...
  {cond:c=>c.lifestyle==='stew'&&REGIONS[c.region].gold>200, w:2, run:c=>popup({title:'재정 혁신', sub:'관리 인생관',
  ...
  {cond:c=>c.lifestyle==='intr'&&stat(c,'intr')>=7, w:2, run:c=>popup({title:'그림자 네트워크', sub:'책략 인생관',
  ...
  {cond:c=>c.lifestyle==='learn'&&stat(c,'learn')>=7, w:2, run:c=>popup({title:'새로운 이론', sub:'학습 인생관',
...
function openBuildMenu(bid){
  const b=BARONIES[bid]; if(!b||b.type==='empty') return;
  const seatGold=Math.round(BARONIES[playerChar().region]?.gold||0);
  const terrKr=TERRAIN[baronyTerrain(bid)]?.n||'?';
  const _cid=BARONY_COUNTY[bid]; const _dev=_cid?countyDev(_cid):0;
  const opts=[];
  const effStr=(lineId,L)=>{ const e=lvEff(lineId,L); const p=[];
    if(e.tax) p.push(`세금+${e.tax}`); if(e.levyCap) p.push(`병력+${e.levyCap}`);
    if(e.regen) p.push(`회복+${e.regen}`); if(e.pop) p.push(`개발+${e.pop}`);
    if(e.fort) p.push(`요새Lv${e.fort}`); if(e.garrison) p.push(`수비${e.garrison}`);
    if(e.def) p.push(`방어+${e.def}`); if(e.supply) p.push(`보급+${e.supply}`);
    const ma=CK3_BUILDINGS[lineId]?.maa; if(ma){ const us=[...new Set(Object.keys(ma).map(u=>MAA_TYPES[MAA_BLD_MAP[u]]?.n).filter(Boolean))]; if(us.length) p.push(us.join('·')+' 강화'); } return p.join(' · '); };
  for(const slot of (b.builds||[])){
    const ln=CK3_BUILDINGS[slot.line], info=nextLevelInfo(bid, slot), nm=lineName(slot.line,b);
    if(slot.wip) opts.push({t:`${nm} (${slot.wip.to}단계 건설중)`, d:`${slot.wip.prog}/${lvTime(slot.wip.to)}개월`, f:()=>{}});
    else if(!info.ok) opts.push({t:`${nm} Lv${slot.level} — ${info.reason}`, d:effStr(slot.line,slot.level), f:()=>{}});
    else opts.push({t:`${nm} Lv${slot.level} → ${info.toLv} 증축`,
      d:`금${info.cost}·${info.time}개월 → ${effStr(slot.line,info.toLv)}${seatGold<info.cost?' (금 부족)':''}`,
      f:()=>{ startUpgrade(bid, slot.line); renderCourt&&renderCourt(); }});
  }
  if(baronySlots(bid)-usedSlots(b)>0){
    for(const id of availableLines(bid)){ const ln=CK3_BUILDINGS[id];
      opts.push({t:`＋ ${ln.n} (1단계 건설)`,
        d:`금${lvCost(1)}·${lvTime(1)}개월 → ${effStr(id,1)}${seatGold<lvCost(1)?' (금 부족)':''}`,
        f:()=>{ startUpgrade(bid, id); renderCourt&&renderCourt(); }});
    }
  }
  opts.push({t:'닫기'});
  showModal({title:`${b.n} 건설`,
    sub:`${b.type==='city'?'도시':b.type==='temple'?'사원':'성채'} · 지형 ${terrKr} · 개발도 ${_dev.toFixed(1)} · ${ERA_NAME[gameEra(state?.year)]}(최대 ${eraMaxLevel(state?.year)}단계) · 슬롯 ${usedSlots(b)}/${baronySlots(bid)} · 금 ${seatGold}`,
    body:'건물을 선택해 건설·증축합니다. (CK3: 지형·보유지 등급·시대 조건 적용)', opts});
  pause();
}
function openCounty(cid){
  // Phase 3: 선택된 내 군대가 있으면 이 county로 파견 (외교창 대신)
  if(state.selectedArmy){ const a=(state.armies||[]).find(x=>x.id===state.selectedArmy);
    state.selectedArmy=null;
    if(a&&a.owner===state.player){ const dest=COUNTIES[cid]?.capital;
      if(dest&&orderArmyMove(a,dest)){ log(`${COUNTIES[cid]?.n||'영지'}(으)로 군대를 파견했습니다.`,'war'); renderMap(); return; } }
    renderMap();
  }
...
/* 공작 건물 건설 메뉴 */
function openDuchyBuildMenu(d){
  const p=playerChar(); if(!p) return;
  const cap=duchyCapitalBarony(d), b=BARONIES[cap]; if(!b) return;
  const seatGold=Math.round(BARONIES[p.region]?.gold||0), lvCapE=duchyBuildLevelCap();
  const cur=b.duchyBuild;
  const effLine=(lineId,L)=>{ const e=DUCHY_BUILDINGS[lineId].eff[L-1]; const a=[];
    if(e.levy)a.push(`징집+${Math.round(e.levy*100)}%`); if(e.upkeep)a.push(`유지비−${Math.round(e.upkeep*100)}%`);
    if(e.fort)a.push(`요새+${e.fort}`); if(e.siege)a.push(`공성+${Math.round(e.siege*100)}%`);
    if(e.inf)a.push(`보병+${Math.round(e.inf*100)}%`); if(e.cav)a.push(`기병+${Math.round(e.cav*100)}%`);
    if(e.prestige)a.push(`위신+${e.prestige}/월`); if(e.pop)a.push(`민심+${e.pop}/월`); return a.join(' · '); };
  const opts=[];
  for(const lineId in DUCHY_BUILDINGS){ const def=DUCHY_BUILDINGS[lineId];
    const isCur=cur&&cur.line===lineId, lv=isCur?cur.level:0;
    if(cur&&cur.wip&&cur.line===lineId){ opts.push({t:`${def.n} (${cur.wip.to}단계 건설중)`, d:`${cur.wip.prog}/${DUCHYB_TIME[cur.wip.to-1]}개월`, f:()=>{}}); continue; }
    if(cur&&cur.wip){ opts.push({t:`${def.n}`, d:'다른 공작 건물 건설 중', f:()=>{}}); continue; }
    const toLv=lv+1;
    if(toLv>3){ opts.push({t:`${def.n} L${lv} (최고 단계)`, d:effLine(lineId,lv), f:()=>{}}); continue; }
    if(toLv>lvCapE){ opts.push({t:`${def.n} ${isCur?'L'+lv+' ':''}— ${ERA_NAME[gameEra(state?.year)+1]||'다음'} 시대 필요`, d:effLine(lineId,toLv), f:()=>{}}); continue; }
    const cost=DUCHYB_COST[toLv-1];
    const head = lv>0?`${def.n} L${lv}→${toLv} 증축`:(cur&&!isCur?`${def.n} (교체 건설)`:`${def.n} 건설`);
    opts.push({t:head, d:`금${cost}·${DUCHYB_TIME[toLv-1]}개월 → ${effLine(lineId,toLv)}${seatGold<cost?' (금 부족)':''}`,
      f:()=>{ startDuchyBuild(d, lineId); renderCourt&&renderCourt(); }});
  }
  opts.push({t:'닫기'});
  showModal({title:`${DUCHIES[d].n} 공작 건물`,
    sub:`수도 ${b.n} · ${ERA_NAME[gameEra(state?.year)]}(최대 ${lvCapE}단계) · 금 ${seatGold} · 공작령당 1개`,
    body:'공작령 전체에 적용되는 특수 건물입니다. (CK3 Duchy Buildings — 교체 시 기존 건물 대체)', opts});
  pause();
}
/* ═══════════════════════════════════════════════
...
function openMyCounty(cid, dispName){
  ...
  const opts=[];
  // 공작 건물: 이 군이 공작령 de jure 수도이고 플레이어가 해당 공작이며 전 군 보유 시
  if(cnt && duchyCapitalBarony(cnt.duchy)===cnt.capital && heldDuchiesOf(p.id).includes(cnt.duchy)){
    const d=cnt.duchy, db=BARONIES[cnt.capital]?.duchyBuild, whole=ownsWholeDuchy(p.id,d);
    const cur=db&&db.level>0?`${DUCHY_BUILDINGS[db.line].icon}${DUCHY_BUILDINGS[db.line].n} L${db.level}${db.wip?` (→${db.wip.to} 건설중)`:''}`:'없음';
    opts.push({t:`공작 건물 — ${DUCHIES[d].n}`,
      d: whole?`현재: ${cur} · 탭하여 건설/증축`:`현재: ${cur} · ⚠ 공작령 전 군 보유 필요`,
      f: whole?()=>openDuchyBuildMenu(d):()=>log('공작령의 모든 군을 직접 보유해야 공작 건물을 지을 수 있습니다.','dip')});
  }
  for(const bid of bids){
    const b=BARONIES[bid]; if(!b) continue;
    const inProg=(b.builds||[]).find(s=>s.wip);
    const slotsUsed=(b.builds||[]).filter(s=>s.line!=='holding').length;
    const slotMax=(b.type==='empty')?0:baronySlots(bid);
    const builtList=(b.builds||[]).filter(s=>s.level>0)
      .map(s=>`${CK3_BUILDINGS[s.line]?.icon||''}${lineName(s.line,b)}${s.level>1?('L'+s.level):''}`);
    const doneNames=builtList.length?builtList.join(' '):'없음';
    ...
    const canBuild=isDirect&&b.type!=='empty';
    ...
    if(b.type==='empty'&&isMyCounty){
            ...
            opts:[
              ...
              {t:hIcon('temple')+' 사원 건설 (금 100)', d:'경건·chaplain 연동 · Bishop 봉신 필요',
                f:()=>{
                  ...
                  b.builds=[{line:'holding',level:1,wip:null}]; b.hlevel=1; b.terrain=baronyTerrain(bid); b.fortLevel=0; b.garrison=0; b.taxBld=0; b.regenBld=0; b.capBld=0;
                  log(`<b>${cnt?.n}</b>에 사원을 건설했습니다. Bishop을 임명하십시오.`,'good');
                }},
              {t:'취소', f:()=>{}}
            ]});
        }});
      ...
      opts.push({t:`${typeIcon} ${b.n} 건설`,
        d:`${ownerStr} · ${terrKr} · 슬롯 ${slotsUsed}/${slotMax} · ${doneNames}${wipText}`,
        f:()=>openBuildMenu(bid)});
    } else {
      opts.push({t:`${typeIcon} ${b.n}`,
        d:`${ownerStr}${terrKr?' · '+terrKr:''} · 슬롯 ${slotsUsed}/${slotMax} · ${doneNames}${wipText}${slotsUsed>=slotMax&&slotMax>0?' · 슬롯 가득':''}`,
        f:()=>{}});
    }
  }
  opts.push({t:'닫기'});
  const devVal=countyDev(cid), devPct=Math.round((countyDevMul(cid)-1)*1000)/10;
  showModal({
    ...
    html:`<div class="kv"><span>총 병력</span><span>${totalTroops}</span></div>
          ...
          <div class="kv"><span>개발도</span><span>${devVal.toFixed(1)} <span style="color:var(--parch-dim);font-size:.82em">(세금·징집 +${devPct}% · 보급 +${countyDevSupply(cid)})</span></span></div>
          <div class="kv"><span>장악력</span><span>${Math.round(countyControl(cid))} <span style="color:var(--parch-dim);font-size:.82em">(세금 ×${(controlTaxMul(cid)*100).toFixed(0)}% · 징집 ×${(controlLevyMul(cid)*100).toFixed(0)}%${countyControl(cid)<40?' · 반란 위험':''})</span></span></div>
          <div class="kv"><span>문화</span><span style="color:${CULTURES[countyCulture(cid)]?.color||'inherit'}">${cultureName(cid)}${countyCulture(cid)!==playerCulture()?' <span style="color:var(--parch-dim);font-size:.82em">(이민족 · 문화 촉진 대상)</span>':''}</span></div>
...
function buildProfileHTML(c){
  ...
  /* B. 스킬 + 특성 */
  const SKILL_FULL={dip:'외교력',mar:'전투력',stew:'관리력',intr:'계책력',learn:'학습력',prow:'용맹'};
  let skillsHtml='';
  ...
  const claimsHtml=isPlayer&&state.claims.length
    ?state.claims.map(cl=>{
        ...
        return `<div class="pm-kv"><span>${claimName(cl.rid)}</span><span style="color:${cb.color}">${cb.n}</span></div>`;
      }).join(''):'';
  const secC=`
...
function openRegion(rid, cid_hint){
  ...
  // ── 영지 정보 탭 데이터 ──
  const cid2=cid_hint||countyOf(rid);
  const cnt2=COUNTIES[cid2];
  const bids2=cnt2?cnt2.baronies:[];
  const landHtml=`
  ...
  const tabModalHtml=`
    <div style="display:flex;border-bottom:1px solid var(--line);margin-bottom:10px">
      <button onclick="
        document.getElementById('_dipTab').style.display='';
        document.getElementById('_landTab').style.display='none';
        this.style.color='var(--gold)';this.style.borderBottom='2px solid var(--gold)';
        this.nextElementSibling.style.color='var(--parch-dim)';this.nextElementSibling.style.borderBottom='none';
      " style="flex:1;background:none;border:none;border-bottom:2px solid var(--gold);color:var(--gold);font-family:inherit;font-size:.78rem;padding:7px 0;cursor:pointer;letter-spacing:.1em">⚔ 외교</button>
      <button onclick="
        document.getElementById('_dipTab').style.display='none';
        document.getElementById('_landTab').style.display='';
        this.style.color='var(--gold)';this.style.borderBottom='2px solid var(--gold)';
        this.previousElementSibling.style.color='var(--parch-dim)';this.previousElementSibling.style.borderBottom='none';
      " style="flex:1;background:none;border:none;color:var(--parch-dim);font-family:inherit;font-size:.78rem;padding:7px 0;cursor:pointer;letter-spacing:.1em">영지</button>
    </div>
    <div id="_dipTab">${html}</div>
    <div id="_landTab" style="display:none">${landHtml}</div>
  `;
  showModal({title:`${dispName}`, sub:`${c.name}의 영지`, body:'', html:tabModalHtml, opts});
  pause();
}
...
const CB_TYPES = {
  pressed:   { n:'압박 명분',   icon:'',  cost:50,  desc:'작위 권리 주장을 전쟁으로 강행', color:'#c9a227' },
  unpressed: { n:'미압박 명분', icon:'', cost:100, desc:'위조되거나 약한 작위 권리 주장',  color:'#8a9a6a' },
  ...
  dejure:    { n:'규범 권역 장악', icon:'', cost:0,  desc:'규범 권역 내 백작령을 회수 — 승리 시 봉신화', color:'#7a9ac8' },
};
...
function claimExpirePulse(){
  state.claims = state.claims.filter(c=>{
    ...
    if(c.type==='unpressed' && state.year - c.obtained > 10){
      log(`${COUNTIES[c.rid]?.n||BARONIES[c.rid]?.n||'영지'}에 대한 미압박 명분이 만료됐습니다.`, 'dip');
      return false;
    }
    return true;
  });
...
• 사제에게 명분 위조를 맡긴다
• 침략을 받아 복수 명분을 얻는다
• 공작위를 창설해 규범 권역의 백작령을 회수한다${relDiff?`\n\n✝ 성전: ${hw.reason}`:''}`,
      opts:[{t:'닫기'}]}); return;
  }
  ...
  const eType = enemyMaaType(def.id);
  const eCounterKey = Object.keys(MAA_COUNTERS).find(k=>MAA_COUNTERS[k]===eType);
  let html = `<div class="kv"><span>상대 전력</span><span>${defPow.toLocaleString()} vs 내 전력 ${myPow.toLocaleString()}</span></div>
    <div class="kv"><span>적 주력 병종</span><span>${MAA_TYPES[eType]?.n||'?'} <span style="color:var(--parch-dim);font-size:.82em">(${MAA_TYPES[eCounterKey]?.n||'?'}(으)로 상성 우위)</span></span></div>
  ...
  for(const role in state.council){
    ...
    if(role === 'chancellor'){
      if(task === 'foreign_affairs'){
        ...
        if(fireEvent && positive && canPos){
          ...
          if(v){ const gain=Math.round(sk*0.5); chOp(v,p,gain);
            popup({title:'왕실의 비호 효과', sub:`${c.name}의 보고`,
      ...
      if(task === 'organize_army'){
        ...
        if(fireEvent){
          if(positive && canPos){
            ...
            popup({title:'징집병 조직 성과', sub:`${c.name}의 보고`,
        ...
        /* CK3: 무관장 장악력 증대 — 대상 수도 군 장악력 월 +0.06×스킬 (정복지 재안정화) */
        const cid=BARONY_COUNTY[p.region], cc=COUNTIES[cid];
        if(cc){
          cc.control=Math.min(100, +((cc.control??90)+skEff*0.06).toFixed(2)); // 스킬12 ≈ +0.72/월
          if(fireEvent && positive && canPos){
            cc.control=Math.min(100, cc.control+3);
            popup({title:'장악력 강화', sub:`${c.name}의 보고`,
              body:`${c.name}이(가) 직할령의 질서를 다잡았습니다.\n장악력 +3 (현재 ${Math.round(cc.control)})`,
              opts:[{t:'수고했다', f:()=>chOp(c,p,5)}]});
          } else if(fireEvent && canNeg){
            cc.control=Math.max(0, cc.control-2);
            popup({title:'영지 마찰', sub:`${c.name}의 보고`,
              body:`강압적 통제에 백성이 반발합니다.\n장악력 -2 · 민심 -3`,
    ...
    /* ── 재무관 (Steward) ── */
      ...
      if(task === 'collect_taxes'){
        ...
        reg.gold = Math.min(9999, (reg.gold||0) + taxBonus);
        if(fireEvent){
          if(positive && canPos){
            /* 긍정: Extra Taxes */
            ...
            popup({title:'세금 성과', sub:`${c.name}의 보고`,
              ...
              opts:[{t:'수고했다', f:()=>{reg.gold=Math.min(9999,reg.gold+bonus); chOp(c,p,5); log(`집사장 추가 징수 +${bonus}금.`,'good');}}]});
          } else if(canNeg){
        ...
        /* CK3: 대상 수도 군에 Development Growth/스킬/월 + 진행 100% 시 개발도 +1 (정식 개발도 c.dev 반영) */
        ...
        if(dco){
          ...
          if(state.councilProgress.steward >= 100){
            ...
            dco.dev = Math.min(cap, +((dco.dev||0) + 1).toFixed(3)); // 이정표: 개발도 +1
            log(`집사장의 영지 개발로 <b>${reg.n} 일대</b>의 개발도가 도약했습니다. (+1 · 현재 ${dco.dev.toFixed(1)})`, 'good');
            if(canPos) popup({title:'개발도 증가 성과', sub:`${c.name}의 보고`,
              body:`수도 군의 개발이 한 단계 도약했습니다.\n개발도 +1 (현재 ${dco.dev.toFixed(1)})`,
              opts:[{t:'훌륭하다', f:()=>chOp(c,p,10)}]});
          } else if(fireEvent && canNeg){
            popup({title:'개발 지연', sub:`${c.name}의 보고`,
              body:`개발 사업이 지연되고 있습니다.\n(진행: ${Math.round(state.councilProgress.steward)}%)`,
              opts:[{t:'추가 지원 (금 -20)', f:()=>{
        ...
        /* CK3: (0.25 + skill÷20)%/month 진행 → 100%: 대상 군 문화를 군주 문화로 전환 */
        const rate2 = 0.25 + skEff/20;
        state.councilProgress.steward = Math.min(100, (state.councilProgress.steward||0) + rate2);
        if(state.councilProgress.steward >= 100){
          state.councilProgress.steward = 0;
          const tgt = pickPromoteCultureTarget();
          if(tgt){
            COUNTIES[tgt].culture = playerCulture();
            state.prestige += 15;
            const b=BARONIES[COUNTIES[tgt].capital]; if(b) b.pop=Math.min(100,(b.pop||60)+8);
            log(`<b>${COUNTIES[tgt].n}</b>의 문화가 아일랜드로 동화되었습니다! 위신 +15 · 민심 +8`, 'good');
          } else {
            state.prestige += 15;
            log(`집사장의 문화 촉진 완료 (전환할 이민족 군 없음) · 위신 +15`, 'good');
          }
        }
        if(fireEvent && positive && canPos){
          /* 긍정: Increased Levies */
          ...
          popup({title:'문화 저항', sub:`${c.name}의 보고`,
            body:'일부 백성들이 문화 촉진 정책에 완강히 반발합니다.',
      ...
      if(task === 'disrupt_schemes'){
        ...
        if(Math.random() < secretChance && positive && canPos){
          ...
          popup({title:'첩보 역탐지', sub:`${c.name}의 보고`,
            body:`${c.name}의 비밀 탐색이 역으로 탐지됐습니다.\n위신 -5`,
    ...
    /* ── 배우자 (Confidant) — 월간 0.75% 스킬 modifier 성장 기회
      ...
      /* 게임 내 근사: CK3 외교관/전략가/건축가/책략가/학자 대신 대응 인격특성(사교적/용감/근면/기만/인내) 부여 */
      const confTask=state.councilTasks?.confidant||'assist_ruler';
      if(confTask!=='assist_ruler'){
        ...
        if(Math.random()<0.0075){
          ...
          if(c.id===p.spouse)
            log(`<b>${c.name}</b>이(가) ${{dip:'외교력',mar:'전투력',stew:'관리력',intr:'계책력',learn:'학습력'}[bk]} 역량이 향상됐습니다. (+1)`, 'fam');
  ...
  if(!isCurrent){
    opts.push({
      ...
      f: ()=>{
        ...
        log(`사제가 <b>${cname}</b>에 대한 명분 위조를 시작합니다.`, 'dip');
        renderCourt();
      }
    });
  } else {
...
function renderCouncil(){
  ...
  if(spouse){
    ...
    const SKILL_NAME_MAP={dip:'외교력',mar:'전투력',stew:'관리력',intr:'계책력',learn:'학습력'};
    if(spTask==='assist_ruler'){
      ...
      html+=`관리력+${Math.floor(stat(spouse,'stew')*0.5)} · 직할 한도 +${limBonus}`;
    } else {
  ...
  for(const role in COUNCIL_ROLES){
    ...
    if(councilor){
      ...
      const BENEFIT_LINE={
        chancellor:`위신 소폭 상승 · 봉신 호감 +5 · 외교력 경험 +5%`,
        marshal:`용맹 +1(연간) · 병력 유지비 -5% · 전투력 경험 +5%`,
        steward:`금 +${Math.round(1+stat(councilor,'stew')*0.08)}/월 · 직할세금 +2.5% · 관리력 경험 +5%`,
        spymaster:`위협감 +10 · 음모 보안 +5% · 계책력 경험 +5%`,
        chaplain:`민심 +0.5/년 · 경건 소폭 상승 · 학습력 경험 +5%`,
      };
...
function renderTitleSection(p){
  ...
  /* 작위 찬탈 (남이 보유 + 규범 권역 51%+ 지배) */
  const usurpable=[];
  for(const did of Object.keys(DUCHIES)){
    if(holdsTitle(p.id,did)) continue;
    const h=titleHolder(did); if(!h||h.id===p.id) continue;
    if(deJureControl(p.id,did)<0.51) continue;
    usurpable.push({did, h});
  }
  if(usurpable.length){
    html+=`<div style="font-size:.66rem;letter-spacing:.1em;color:var(--parch-dim);margin:8px 0 4px">작위 찬탈 (호감 -50)</div>`;
    for(const u of usurpable){
      const info=titleInfo(u.did); const chk=canUsurpTitle(p.id,u.did);
      if(chk.ok){
        html+=`<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px dotted #2c2316">
          <span style="font-size:.74rem"><b>${info.n}</b><span style="color:var(--parch-dim);font-size:.64rem"> · 금 ${chk.cost} · ${u.h.name}</span></span>
          <button onclick="onUsurpTitle('${u.did}')" style="background:linear-gradient(#3a2418,#241410);color:#d8a24a;border:1px solid var(--gold-dim);border-radius:3px;font-size:.66rem;padding:3px 8px;cursor:pointer;font-family:inherit">찬탈</button>
        </div>`;
      } else {
        html+=`<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px dotted #2c2316;opacity:.55">
          <span style="font-size:.74rem">${info.n}<span style="color:var(--parch-dim);font-size:.62rem"> · ${chk.reason}</span></span>
          <span style="font-size:.6rem;color:#6a6048">불가</span>
        </div>`;
      }
    }
  ...
  /* 백작령 하사 (궁정인을 백작으로 임명 — 새 봉신 창출) */
  const grantCids=directCountiesOf(p.id).filter(cid=>cid!==countyOf(p.region));
  const courtierCands=Object.values(chars).filter(c=>!c.dead&&c.courtOf===p.region&&c.id!==p.id&&!c.ruler);
  if(grantCids.length>0 && courtierCands.length>0){
    html+=`<div style="font-size:.66rem;letter-spacing:.1em;color:var(--parch-dim);margin:8px 0 4px">백작령 하사</div>
      <div style="display:flex;justify-content:space-between;align-items:center;padding:4px 0">
        <span style="font-size:.72rem">궁정인을 백작으로 임명 — 봉신 체계 구축</span>
        <button onclick="onGrantCountyMenu()" style="background:linear-gradient(#3a2c18,#241a0e);color:var(--gold);border:1px solid var(--gold-dim);border-radius:3px;font-size:.66rem;padding:3px 8px;cursor:pointer;font-family:inherit">하사</button>
      </div>`;
  }
...
/* 작위 찬탈 핸들러 */
function onUsurpTitle(tid){
  initAudio();
  const p=playerChar(); if(!p) return;
  const holder=titleHolder(tid); const info=titleInfo(tid);
  if(usurpTitle(p.id,tid)){
    playSynthSFX&&playSynthSFX('gold');
    renderFief(); renderChar(); renderMap&&renderMap(); renderHeader&&renderHeader();
    popup({title:info.n, sub:'작위 찬탈',
      body:`<b>${info.n}</b>을(를) ${holder?holder.name:'이전 보유자'}에게서 찬탈했습니다!\n전쟁 없이 정당한 권리로 작위를 가져왔습니다 (호감 -50).`,
      opts:[{t:'정당한 권리다'}]});
  }
}
/* 백작령 하사 핸들러 (궁정인 → 백작) */
function onGrantCountyMenu(){
  initAudio();
  const p=playerChar(); if(!p) return;
  const myCids=directCountiesOf(p.id).filter(cid=>cid!==countyOf(p.region));
  const courtierCands=Object.values(chars).filter(c=>!c.dead&&c.courtOf===p.region&&c.id!==p.id&&!c.ruler);
  if(!myCids.length||!courtierCands.length) return;
  const cOpts=myCids.map(cid=>({
    ...
    f:()=>{
      const vOpts=courtierCands.slice(0,4).map(v=>({
        ...
        f:()=>{ grantCountyToVassal(p.id, v.id, cid); renderFief(); renderChar(); }
      }));
      vOpts.push({t:'취소'});
...
function renderFief(){
  ...
  html+=`<div style="display:flex;justify-content:space-between;font-size:.72rem;color:var(--parch-dim);margin-top:5px">
    <span>권세 봉신</span><span>${pvIds.length}/${powerfulVassalCount(p)}명</span></div>`;
  /* 권세 봉신 중 자문회 미포함 경고 */
  const pvNotOnCouncil=pvIds.filter(vId=>!powerfulVassalOnCouncil(p.id,vId));
  if(pvNotOnCouncil.length){
    html+=`<div style="font-size:.66rem;color:#c87a3a;margin-top:3px">⚠ 권세 봉신 ${pvNotOnCouncil.length}명이 자문회에 없음 (각 −40 호감)</div>`;
  }
  html+=`</div>`;
  ...
  for(const v of sorted){
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start">
        <div style="flex:1;min-width:0">
          <div style="font-size:.8rem;color:var(--parch)">${vIcon} <b>${v.name}</b>
            ${isPowerful?`<span style="color:var(--gold);font-size:.62rem" title="권세 봉신">${onCouncil?'':'⚠'}</span>`:''}
          </div>
    ...
    /* 계약 의무 협상 — castle 봉신만 (city/temple은 고정) */
    if(vType==='castle'){
      html+=obligNegotiateRow(v,'tax')+obligNegotiateRow(v,'levy');
...
/* host의 연회/사냥에 참석. 플레이어 활동과 동일하게 월 단위로 단계 진행됨 */
function startGuestActivity(host, type){
  const p=playerChar(); if(!p||!host) return;
  state.guestActivity={ host:host.id, type, progress:0, total:(type==='hunt'?2:2), track:0, rapport:0, awaitingPick:false };
  log(`<b>${host.name}</b>의 ${type==='hunt'?'사냥':'연회'}에 손님으로 향합니다.`,'dip');
  flashActivityWidget(); renderActivityWidget();
  // 첫 단계는 다음 달부터 (플레이어 자신의 활동과 동일하게 시간이 흐름)
}
/* 게스트 활동 다음 단계 이벤트 표시 (월 1단계씩, monthlyPulse가 호출) */
function guestActivityNext(){
  ...
  g.awaitingPick=true; // 단계 모달 응답 대기 (다음 달 진행 보류)
  const ev = g.type==='hunt' ? guestHuntEvent(g,host) : guestFeastEvent(g,host);
  flashActivityWidget(); renderActivityWidget();
  showGuestModal(ev, host, g);
}
/* 게스트 활동 월간 진행 — 플레이어 활동(activityPulse)과 동일하게 monthlyPulse에서 호출 */
function guestActivityPulse(){
  const g=state.guestActivity; if(!g) return;
  if(g.awaitingPick) return;           // 아직 이번 단계 선택 전이면 대기
  const host=chars[g.host];
  if(!host||host.dead){ finishGuestActivity(); return; }
  guestActivityNext();                 // 다음 단계 모달 (또는 종료)
}
/* 게스트 사냥 단계 이벤트 */
...
/* 게스트 모달 선택 처리 → 다음 단계는 다음 달 monthlyPulse가 진행 */
function guestModalPick(i){
  const modal=document.getElementById('actModal');
  const o=modal._guestOpts?modal._guestOpts[i]:null;
  if(o&&o.f) o.f();
  if(state.guestActivity) state.guestActivity.awaitingPick=false; // 이번 단계 완료
  renderActivityWidget();
  closeActModal(); // 게임 재개 (다음 단계는 다음 달)
}
/* 게스트 모달 닫기 (중도 이탈) */
...
function renderActivityWidget(){
  ...
  const act=state.activeActivity;
  if(!act){
    // 게스트 활동도 위젯 표시 (플레이어 활동과 동일하게)
    const g=state.guestActivity;
    if(g){
      const gt=ACTIVITY_TYPES[g.type]||{icon:g.type==='hunt'?'':''};
      const frac=Math.max(0,Math.min(1, g.total?g.progress/g.total:0));
      const R=33, C=2*Math.PI*R, off=C*(1-frac);
      el.innerHTML=`
        ...
        <span class="aw-icon">${gt.icon}</span>
        <span class="aw-phase">손님</span>
        <span class="aw-badge"></span>`;
      el.classList.add('show');
      return;
    }
    el.classList.remove('show','alert'); el.innerHTML=''; return;
  }
  const t=ACTIVITY_TYPES[act.type];
  // 진행도 0~1 계산 (단계별)
...
function renderCourt(){
  ...
  /* 궁정 인물 목록 */
  let html=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid var(--line)">${rn} 궁정의 사람들</div>
...
function renderDec(){
  ...
  // ═══════ 결단 (CK3 Decisions) ═══════
  function addHeader(t){ items.push({header:t}); }
  addHeader('결단');
  // 아일랜드 왕국 형성 (CK3 고유 봉역 생성 결단) — 공작령 7개 보유 시 하이킹 등극
  if(playerDuchies().length>=7){
    addDec('아일랜드 왕국 형성','에이레 전토 통일 — 하이킹 등극(승리)', true, ()=>{
      closePanel('dec');
      const pc=playerChar();
      if(pc && !holdsTitle(pc.id,'k_ireland')){ if(!pc.heldTitles) pc.heldTitles=[]; pc.heldTitles.push('k_ireland'); }
      victory();
    });
  }
  // 아일랜드에 대한 명분 요청 (CK3 실제 결단: 신앙도 100 → 아일랜드 백작령 명분)
  {
    const irish=Object.keys(COUNTIES).filter(cid=>{ const h=countyHolder(cid); return !h||h.id!==p.id; });
    const targets=irish.filter(cid=>!hasClaim(cid));
    addDec('아일랜드에 대한 명분 요청',
      targets.length ? `신앙도 100 → 미보유 아일랜드 백작령 ${targets.length}곳에 명분`
                     : '이미 모든 아일랜드 백작령에 명분 보유',
      (state.piety||0)>=100 && targets.length>0, ()=>{
        if(spendPiety(100)){
          targets.forEach(cid=>addClaim(cid,'unpressed'));
          popup({title:'교황의 칙서', sub:'명분 요청',
            body:`교황청이 에이레에 대한 당신의 권리를 인정하는 칙서를 내렸습니다.\n미보유 아일랜드 백작령 <b>${targets.length}곳</b>에 명분이 생겼습니다.`,
            opts:[{t:'신의 뜻이다'}]});
          renderDec();
        }
      });
  ...
  if(state.successionLaw==='partition'){
    addDec('장자상속으로 변경',`위신 500 소모 · 사망 시 장남이 전 영지 계승`, state.prestige>=500, ()=>{
  ...
  if(state.successionLaw==='primogeniture'){
    addDec('분할상속으로 복귀','위신 200 소모', state.prestige>=200, ()=>{
    ...
    if(vassalsOf(state.player).length>=3){
      addDec('선출제로 변경','봉신 3명 이상 필요 · 봉신들이 후계자 선출', true, ()=>{
  ...
  if(vacancies>0||courtSize<3){
    addDec('인재 모집',`금 80 · 궁정에 새 인재 1명 영입`, REGIONS[p.region].gold>=80, ()=>{
      ...
      base[sk]=8+Math.floor(Math.random()*4);
      const recruit=mk({
  ...
  const claimHtml = state.claims.length > 0
    ...
    : '<div style="font-size:.76rem;color:var(--parch-dim);margin-bottom:10px">명분 없음 — 사제에게 명분 위조를 맡기세요</div>';
  let html = claimHtml + `<p style="font-size:.74rem;color:var(--parch-dim);margin-bottom:12px;letter-spacing:.05em">지배 왕국 ${n}/7</p>`;
  // 빈 카테고리 헤더 제거 (뒤에 실제 항목이 없는 헤더는 숨김)
  const vis=items.filter((it,idx)=>{
    if(!it.header) return true;
    for(let k=idx+1;k<items.length;k++){ return !items[k].header; }
    return false;
  });
  if(!vis.length){ html+='<p style="color:var(--parch-dim);font-size:.84rem">조건이 충족된 결단이 없습니다.</p>'; }
  vis.forEach(it=>{
    if(it.header){
      html+=`<div style="font-size:.64rem;letter-spacing:.2em;color:var(--gold-dim);margin:14px 0 6px;padding-bottom:4px;border-bottom:1px solid var(--line)">${it.header}</div>`;
      return;
    }
...
/* ---------- 군사 탭 ---------- */
function openWar(){ togglePanel('war'); }
function renderWar(){
  const p=playerChar(); if(!p) return;
  const el=document.getElementById('warContent'); if(!el) return;
  const seat=BARONIES[p.region];
  let direct=0; for(const bid of regionsOf(p.id)){ const b=BARONIES[bid]; if(b) direct+=b.troops; }
  direct=Math.round(direct);
  const total=mobilizableTroops(p);
  const levy=Math.max(0,total-direct);
  const gold=Math.round(seat?.gold||0);
  const hd=(t)=>`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin:12px 0 6px;padding-bottom:5px;border-bottom:1px solid var(--line)">${t}</div>`;
  let html='';
  /* 병력 현황 */
  html+=hd('병력 현황');
  const lim=supplyLimit(p); const overSupply=total>lim;
  html+=`<div style="background:#15110a;border:1px solid var(--line);border-radius:4px;padding:8px 10px;margin-bottom:8px;font-size:.8rem">
    <div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--parch-dim)">직할 병력</span><span><b>${direct}</b></span></div>
    <div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--parch-dim)">봉신 동원</span><span><b>${levy}</b></span></div>
    <div style="display:flex;justify-content:space-between;padding:3px 0;border-top:1px dotted #2c2316;margin-top:3px"><span style="color:var(--gold-dim)">총 동원 가능</span><span style="color:${overSupply?'#d05a4a':'var(--gold)'}"><b>${total}</b></span></div>
    <div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--parch-dim)">보급 한계</span><span style="color:${overSupply?'#d05a4a':'var(--parch)'}">${lim}</span></div>
    <div style="display:flex;justify-content:space-between;padding:2px 0"><span style="color:var(--parch-dim)">전투력(지휘)</span><span>${stat(p,'mar')}</span></div>
    ${overSupply?`<div style="font-size:.64rem;color:#d05a4a;margin-top:4px">⚠ 보급 한계 초과 — 전쟁 시 사기 −${Math.round((1-supplyFactor(p))*100)}% · 병력 소모 가중</div>`:''}
  </div>`;
  /* ── Phase 3: 군대 (소집·이동) ── */
  html+=hd('군대');
  html+=`<div style="font-size:.64rem;color:var(--parch-dim);margin-bottom:5px">군대를 소집한 뒤 <b>선택</b>하고, 지도에서 목적지 백작령을 클릭하면 그곳으로 행군합니다.</div>`;
  html+=`<button class="p-action" onclick="playerRaiseArmy();renderWar();renderMap();">⚔ 군대 소집<span class="pd">현재 직할 병력 + 무장병으로 출진 부대 편성</span></button>`;
  const _myArmies=(state.armies||[]).filter(a=>a.owner===p.id);
  const _abtn='border:1px solid var(--gold-dim);border-radius:3px;font-size:.62rem;padding:2px 7px;cursor:pointer;font-family:inherit';
  if(_myArmies.length){ for(const a of _myArmies){ const sel=state.selectedArmy===a.id;
    const cn=COUNTIES[BARONY_COUNTY[a.pos]]?.n||'?';
    const st=a.state==='march'?`행군 중 → ${COUNTIES[BARONY_COUNTY[a.dest]]?.n||'?'}`:'대기';
    html+=`<div style="background:#15110a;border:1px solid ${sel?'var(--gold)':'var(--line)'};border-radius:4px;padding:6px 8px;margin:4px 0;font-size:.72rem">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:6px">
        <span>⚔ ${armyTroops(a).toLocaleString()}명 · ${cn} · <span style="color:var(--parch-dim)">${st}</span></span>
        <span style="display:flex;gap:4px;flex-shrink:0">
          <button onclick="selectArmy('${a.id}')" style="${_abtn};background:${sel?'linear-gradient(#3a2c18,#241a0e)':'#241a10'};color:${sel?'var(--gold)':'var(--parch)'}">${sel?'선택됨':'선택'}</button>
          <button onclick="disbandArmy('${a.id}');renderWar();renderMap();" style="${_abtn};background:#241410;color:#9e5a5a">해산</button>
        </span>
      </div></div>`;
  } } else html+=`<div style="font-size:.7rem;color:var(--parch-dim);margin:5px 0 4px">편성된 군대 없음</div>`;
  /* 병력 모집 */
  html+=hd('병력 모집');
  const mkBtn=(label,desc,enabled,fn)=>`<button class="p-action${enabled?'':' off'}" onclick="${fn}">${label}<span class="pd">${desc}</span></button>`;
  html+=mkBtn('병력 소집','금 80 → 직할 병력 +200', gold>=80, 'raiseLevy()');
  /* 무장병 (Men-at-Arms) */
  ensureMAA();
  const cap=maaCap(p), used=maaCount(), up=maaUpkeep();
  html+=hd('무장병 (Men-at-Arms)');
  html+=`<div style="display:flex;justify-content:space-between;font-size:.72rem;margin-bottom:6px">
    <span style="color:${used>=cap?'#c8a24a':'var(--parch-dim)'}">연대 <b>${used}/${cap}</b></span>
    <span style="color:var(--parch-dim)">월 유지비 금 ${up.toFixed(1)}</span></div>`;
  const sbtn='border:1px solid var(--gold-dim);border-radius:3px;font-size:.62rem;padding:2px 7px;cursor:pointer;font-family:inherit';
  const maaMb=maaBuildBonus();
  for(const k in MAA_TYPES){
    const t=MAA_TYPES[k]; const cnt=state.maa[k]||0;
    const canHire = used<cap && gold>=t.hire;
    const mb=maaMb[k]||1; const effPow=Math.round(t.power*mb);
    const bonusTxt = mb>1.001 ? ` <span style="color:#7fae6a">×${mb.toFixed(2)} 건물</span>` : '';
    const favT=Object.entries(MAA_TERRAIN[k]||{}).filter(([,v])=>v>0).map(([tk])=>TERRAIN[tk]?.n).filter(Boolean);
    const badT=Object.entries(MAA_TERRAIN[k]||{}).filter(([,v])=>v<0).map(([tk])=>TERRAIN[tk]?.n).filter(Boolean);
    const terrTxt=(favT.length?` · <span style="color:#7fae6a">유리 ${favT.join('·')}</span>`:'')+(badT.length?` · <span style="color:#c07a5a">불리 ${badT.join('·')}</span>`:'');
    html+=`<div style="background:#15110a;border:1px solid var(--line);border-radius:4px;padding:6px 8px;margin-bottom:5px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:.76rem">${t.icon} <b>${t.n}</b>${cnt>0?` <span style="color:var(--gold)">×${cnt}</span>`:''}${bonusTxt}</span>
        <span style="display:flex;gap:4px">
          ${cnt>0?`<button onclick="disbandMAA('${k}')" style="${sbtn};background:#241410;color:#9e5a5a">해산</button>`:''}
          <button onclick="hireMAA('${k}')" style="${sbtn};background:linear-gradient(#3a2c18,#241a0e);color:${canHire?'var(--gold)':'#6a6048'};${canHire?'':'opacity:.55;cursor:default'}">고용</button>
        </span>
      </div>
      <div style="font-size:.62rem;color:var(--parch-dim);margin-top:2px">${t.size}명 · 전력 ${mb>1.001?`${effPow} <span style="color:#7fae6a">(기본 ${t.power})</span>`:t.power} · 금 ${t.hire} · 유지 ${t.upkeep}/월 · ${t.counter} 상대 유리${terrTxt}</div>
      <div style="font-size:.6rem;color:#8a7a58;margin-top:1px">⚔피해 ${t.dmg} · 유지 ${t.tough}${t.pursuit?` · 추격 ${t.pursuit}`:''}${t.screen?` · 차단 ${t.screen}`:''}</div>
    </div>`;
  }
  /* 요새 · 수비대 (직할 남작령 방어 증강) */
  html+=hd('요새 · 수비대');
  const myBids=regionsOf(p.id).filter(bid=>BARONIES[bid]&&BARONIES[bid].type!=='empty');
  if(!myBids.length){
    ...
    for(const bid of myBids){
      ...
      const tAdv=terrainAdvantage(bid), tKr=TERRAIN[baronyTerrain(bid)]?.n||'';
      const fortLines=(b.builds||[]).filter(s=>CK3_BUILDINGS[s.line]?.cat==='fort'&&s.level>0)
        .map(s=>`${lineName(s.line,b)}L${s.level}`).join(', ')||'없음';
      html+=`<div style="background:#15110a;border:1px solid var(--line);border-radius:4px;padding:6px 8px;margin-bottom:5px">
        <div style="font-size:.76rem;margin-bottom:2px">${tIcon[b.type]||'•'} <b>${b.n}</b>
          <span style="color:var(--parch-dim);font-size:.64rem"> · 요새 Lv ${fl}${fl?` (+${fl*4}% 점령저항)`:''} · 수비대 ${gar} · 지형 ${tKr}${tAdv?`(방어+${tAdv})`:''}</span></div>
        <div style="font-size:.62rem;color:var(--parch-dim)">요새화 건물: ${fortLines}${b.type==='castle'?'':' · (요새화는 성채 전용)'}</div>
      </div>`;
    }
    html+=`<div style="font-size:.6rem;color:var(--parch-dim);margin:2px 2px 0">CK3 고증: 요새 레벨·수비대는 <b>방벽 및 방어탑·구릉 요새</b> 등 요새화 건물(성채 전용)에서 나옵니다. 지형은 방어 유리함을 더합니다. 영지 화면 건설 메뉴에서 증축하십시오.</div>`;
  }
  /* 전쟁 현황 */
  html+=hd('전쟁 현황');
  const myWars=state.wars.filter(w=>w.atk===p.id||w.def===p.id);
  if(!myWars.length){
    html+=`<div style="font-size:.78rem;color:var(--parch-dim)">진행 중인 전쟁이 없습니다.</div>`;
  } else {
    for(const w of myWars){
      const isAtk=w.atk===p.id;
      const side=isAtk?'atk':'def', foeSide=isAtk?'def':'atk';
      const foe=chars[isAtk?w.def:w.atk];
      const tgt=COUNTIES[w.targetRid]?.n||'';
      const cmd=warCommander(w,side);
      const myP=Math.round(powerWithCommander(p,w,side));
      const foeP=foe?Math.round(powerWithCommander(foe,w,foeSide)):0;
      const adv=myP>=foeP;
      const widx=state.wars.indexOf(w);
      html+=`<div style="background:#15110a;border:1px solid var(--line);border-radius:4px;padding:7px 10px;margin-bottom:6px">
        <div style="display:flex;justify-content:space-between;font-size:.78rem">
          <span>${isAtk?'⚔ 공격':'방어'} · <b>${foe?foe.name:'?'}</b></span>
          <span style="color:var(--parch-dim);font-size:.7rem">${tgt}</span>
        </div>
        <div style="display:flex;justify-content:space-between;font-size:.72rem;margin-top:4px">
          <span style="color:${adv?'#7a9a6a':'#9e5a5a'}">아군 ${myP}</span>
          <span style="color:var(--parch-dim)">vs</span>
          <span style="color:${adv?'#9e5a5a':'#7a9a6a'}">적군 ${foeP}</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;font-size:.7rem;margin-top:5px;padding-top:4px;border-top:1px dotted #2c2316">
          <span style="color:var(--parch-dim)">지휘관 <b style="color:var(--parch)">${cmd?cmd.name.split(' ')[0]:'없음'}</b>${cmd?` <span style="color:#7a9a6a">+${commanderBonusPct(cmd)}%</span>`:''}</span>
          <button onclick="onCommanderPick(${widx})" style="border:1px solid var(--gold-dim);border-radius:3px;font-size:.6rem;padding:2px 7px;cursor:pointer;font-family:inherit;background:linear-gradient(#3a2c18,#241a0e);color:var(--gold)">임명</button>
        </div>
      </div>`;
    }
  }
...
/* (폐지) 요새/수비대 직접 구매 — CK3 고증대로 요새화 건물에서만 파생.
   b.fortLevel·b.garrison은 buildingPulse가 산출. */
function raiseLevy(){
  initAudio(); const p=playerChar(); if(!p) return;
  const seat=BARONIES[p.region]; if(!seat||seat.gold<80) return;
  playSynthSFX&&playSynthSFX('gold');
  seat.gold-=80; seat.troops+=200;
  log('창병 200이 소집됐습니다.','war'); renderWar(); renderHeader&&renderHeader();
}
/* ── 장수(지휘관) 임명 ── */
/* 지휘관 후보 — 본인·가족·궁정인·봉신 중 무예 높은 순 */
function commanderCandidates(){
  const p=playerChar(); if(!p) return [];
  const seen=new Set();
  const cands=Object.values(chars).filter(c=>{
    if(c.dead||seen.has(c.id)) return false;
    const ok=(c.id===p.id || c.courtOf===p.region || c.father===p.id || c.mother===p.id || c.spouse===p.id || c.liege===p.id);
    if(ok) seen.add(c.id);
    return ok;
  });
  return cands.sort((a,b)=>(stat(b,'mar')+stat(b,'prow')*0.5)-(stat(a,'mar')+stat(a,'prow')*0.5)).slice(0,8);
}
/* 지휘관이 주는 전력 보정 % (powerWithCommander와 동일 산식) */
function commanderBonusPct(cmd){
  if(!cmd) return 0;
  let m=1+stat(cmd,'mar')*0.02, pr=1+stat(cmd,'prow')*0.01, tr=1;
  if(cmd.traits.includes('brave'))    tr+=0.05;
  if(cmd.traits.includes('wrathful')) tr+=0.03;
  if(cmd.traits.includes('diligent')) tr+=0.03;
  if(cmd.traits.includes('patient'))  tr+=0.02;
  if(cmd.traits.includes('craven'))   tr-=0.05;
  if(cmd.traits.includes('lazy'))     tr-=0.03;
  return Math.round((m*pr*tr-1)*100);
}
/* 전쟁 지휘관 임명 메뉴 */
function onCommanderPick(warIdx){
  initAudio(); const p=playerChar(); const w=state.wars[warIdx]; if(!p||!w) return;
  const side = w.atk===p.id?'atk':'def';
  const cur = side==='atk'?w.atkCmd:w.defCmd;
  const opts=commanderCandidates().map(c=>({
    t:`${c.name}${c.id===cur?' ✓':''}`,
    d:`전투력 ${stat(c,'mar')} · 용맹 ${stat(c,'prow')} · 전력 +${commanderBonusPct(c)}%`,
    f:()=>{
      if(side==='atk') w.atkCmd=c.id; else w.defCmd=c.id;
      log(`<b>${c.name}</b>을(를) 전쟁 지휘관으로 임명했습니다.`,'war');
      renderWar();
    }
  }));
  opts.push({t:'취소'});
  showModal({title:'지휘관 임명', sub:'군사', body:'누구에게 군대를 맡기겠습니까?', opts});
}
function victory(){
...
function renderChar(){
  ...
  portrait.innerHTML = makePortraitSVG(c, 84, 100);
  portrait.style.cursor='pointer';
  portrait.onclick=()=>{ initAudio(); openProfile(c); };
  /* 이름·작위·능력치는 하단 바에서 제외 — 초상화 클릭 시 인물창에서 확인. 잔존 요소만 null-안전 갱신 */
  const _txt=(id,v)=>{ const e=document.getElementById(id); if(e) e.textContent=v; };
  const _html=(id,v)=>{ const e=document.getElementById(id); if(e) e.innerHTML=v; };
  _txt('cNm', c.name);
  _txt('cTtl', `${charTitle(c)} · ${age(c)}세 · ${c.dyn} 가문`);
  let chips='';
  ...
  _html('cChips', chips);
  _html('stats', Object.entries(SKILLS).map(([k,n])=>`<span>${n} <b>${stat(c,k)}</b></span>`).join('')+`<span>용맹 <b>${stat(c,'prow')}</b></span>`);
  // 스트레스 (슬림 바)
  const _sf=document.getElementById('stressFill'); if(_sf) _sf.style.width=Math.min(100,c.stress/1.5)+'%';
  _txt('stressNum', `${c.stress} / 150`);
  const _sl=document.getElementById('stressLvl');
  if(_sl){ const lv=stressLvl(c); _sl.textContent=['0단계 — 평온함','1단계 — 불안','2단계 — 위험','3단계 — 죽음'][lv];
    _sl.style.color=lv===0?'var(--parch-dim)':lv===1?'#c8a24a':lv===2?'#c87a4a':'#d05a4a'; }
}
/* ── 지도 모드 (CK3 Map Modes) — 카운티 채움색을 모드별로 결정 ── */
/* ── 좌하단 인물 정보 모달 (가문·문화·신앙·정통성·인생관·소지품) ── */
function openDynasty(){
  const p=playerChar(); if(!p) return;
  const seen=new Set(); const rows=[];
  for(const c of Object.values(chars)){
    if(c.dead) continue;
    const isFam=c.id===p.id||c.spouse===p.id||c.father===p.id||c.mother===p.id||c.dyn===p.dyn;
    if(!isFam||seen.has(c.id)) continue; seen.add(c.id);
    const rel=c.id===p.id?'\ubcf8\uc778':c.spouse===p.id?'\ubc30\uc6b0\uc790':(c.father===p.id||c.mother===p.id)?(c.sex==='m'?'\uc544\ub4e4':'\ub538'):'\uce5c\uc871';
    rows.push(`<div style="display:flex;justify-content:space-between;padding:3px 0;border-bottom:1px dotted #2c2316"><span>${c.name}</span><span style="color:var(--parch-dim);font-size:.85em">${rel} \u00b7 ${age(c)}\uc138</span></div>`);
  }
  const fl=(typeof FAME_LEVELS!=='undefined')?FAME_LEVELS[state.fameLevel||0]:null;
  showModal({title:`${p.dyn} \uac00\ubb38`, sub:'\uc655\uc870', body:
    `<div style="margin-bottom:10px;color:var(--gold)">\uac00\ubb38 \uba85\uc131: ${fl?fl.n:'\u2014'} <span style="color:var(--parch-dim);font-size:.85em">(${Math.round(state.fameProgress||0)})</span></div>`+
    `<div style="font-size:.85rem">${rows.join('')||'<span style=\"color:var(--parch-dim)\">\uae30\ub85d\ub41c \uce5c\uc871 \uc5c6\uc74c</span>'}</div>`, opts:[{t:'\ub2eb\uae30'}]});
}
function openCulture(){
  const ck=(typeof playerCulture==='function')?playerCulture():'irish'; const C=CULTURES[ck];
  showModal({title:C?C.n:'\ubb38\ud654', sub:'\ubb38\ud654', body:
    C?`<div style="margin-bottom:6px;color:var(--parch-dim)">\ubb38\ud654\uad8c: ${C.grp}</div><div style="font-size:.86rem;line-height:1.6">${C.n} \ubb38\ud654\uad8c\uc758 \uc804\ud1b5\uacfc \uad00\uc2b5\uc744 \ub530\ub984.</div>`
     :'<div style="color:var(--parch-dim)">\uc815\ubcf4 \uc5c6\uc74c</div>', opts:[{t:'\ub2eb\uae30'}]});
}
function openFaith(){
  const p=playerChar(); const F=p?FAITHS[p.faith]:null;
  if(!F){ showModal({title:'\uc2e0\uc559', body:'<div style="color:var(--parch-dim)">\uc815\ubcf4 \uc5c6\uc74c</div>', opts:[{t:'\ub2eb\uae30'}]}); return; }
  const vir=(F.virtues||[]).map(t=>TRAITS[t]&&TRAITS[t].n).filter(Boolean).join(' \u00b7 ')||'\u2014';
  const sin=(F.sins||[]).map(t=>TRAITS[t]&&TRAITS[t].n).filter(Boolean).join(' \u00b7 ')||'\u2014';
  const dv=DEVOTION_LEVELS[state.devotionLevel||0];
  showModal({title:F.n, sub:'\uc2e0\uc559', body:
    `<div style="font-size:.86rem;line-height:1.6;margin-bottom:8px">${F.desc||''}</div>`+
    `<div style="color:var(--parch-dim);font-size:.84rem">\ubbf8\ub355: ${vir}</div>`+
    `<div style="color:var(--parch-dim);font-size:.84rem;margin-bottom:8px">\uc8c4\uc545: ${sin}</div>`+
    `<div>\uc2e0\uc2ec: <b>${dv?dv.n:'\u2014'}</b> <span style="color:var(--parch-dim);font-size:.85em">(${Math.round(state.devotionProgress||0)})</span></div>`+
    (dv?`<div style="color:var(--parch-dim);font-size:.8rem;margin-top:2px">${dv.desc}</div>`:''), opts:[{t:'\ub2eb\uae30'}]});
}
function openLegit(){
  ensureLegit(); const lv=legitLevel(); const L=LEGIT_LEVELS[lv]; const th=legitThresholds();
  const v=Math.round(state.legitimacy||0); const next=th[lv+1];
  showModal({title:L.n, sub:'\uc815\ud1b5\uc131', body:
    `<div style="font-size:.86rem;line-height:1.6;margin-bottom:8px">${L.desc}</div>`+
    `<div>\ud604\uc7ac \uc815\ud1b5\uc131: <b>${v}</b>${next!=null?` <span style="color:var(--parch-dim);font-size:.85em">/ \ub2e4\uc74c \ub2e8\uacc4 ${Math.round(next)}</span>`:' (\ucd5c\uace0 \ub2e8\uacc4)'}</div>`+
    `<div style="color:var(--parch-dim);font-size:.84rem;margin-top:4px">\ubd09\uc2e0\u00b7\ubbfc\uc2ec \ud638\uac10 ${L.op>=0?'+':''}${L.op}</div>`, opts:[{t:'\ub2eb\uae30'}]});
}
function openLifestyle(){
  const p=playerChar(); const ls=(p&&p.lifestyle)?SKILLS[p.lifestyle]:null;
  showModal({title:ls?`${ls}\uc758 \uae38`:'\uc778\uc0dd\uad00', sub:'\uc0dd\ud65c\uc591\uc2dd', body:
    ls?`<div style="font-size:.86rem;line-height:1.6">${ls} \ub2a5\ub825\uc744 \uc911\uc2ec\uc73c\ub85c \uc815\uc9c4\ud558\ub294 \uc778\uc0dd\uad00.</div><div style="margin-top:8px">\uacbd\ud5d8\uce58: <b>${p.lifeXP||0}</b></div>`
      :'<div style="color:var(--parch-dim)">\uc544\uc9c1 \uc778\uc0dd\uad00\uc744 \uc815\ud558\uc9c0 \uc54a\uc74c.</div>', opts:[{t:'\ub2eb\uae30'}]});
}
function openInventory(){
  showModal({title:'\uc18c\uc9c0\ud488', sub:'\ubcf4\ubb3c\u00b7\uc720\ubb3c', body:'<div style="color:var(--parch-dim);text-align:center;padding:16px 8px">\uc900\ube44 \uc911 \u2014 \ucd94\ud6c4 \uad6c\ud604 \uc608\uc815</div>', opts:[{t:'\ub2eb\uae30'}]});
}
...
/* 남작령 권역 경계선 — 카운티 폴리곤을 남작령 점들의 보로노이 셀로 쪼갬 (showBaronies 토글 시 표시) */
function renderMap(){
  ...
  try { const _pp=playerChar();
    for(const cid in COUNTIES){ const C=COUNTIES[cid]; const poly=C.poly; if(!poly) continue;
      ...
      const underSiege=state.wars.some(w=>w.targetRid===cid&&w.occupied?.length>0);
      const strokeCol=underSiege?'#c83030':realmStrokeColor(holder);
...
/* 남작령 오버레이 토글 (Phase 2) */
function toggleBaronies(){ state.showBaronies=!state.showBaronies;
  const b=document.getElementById('baronyToggle'); if(b){ b.style.color=state.showBaronies?'var(--gold)':'var(--parch-dim)'; b.style.borderColor=state.showBaronies?'var(--gold)':'var(--gold-dim)'; }
  renderMap(); }
