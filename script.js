...
const state = {
  ...
  council:{ chancellor:null, marshal:null, steward:null, spymaster:null, chaplain:null },
  claims:[], popupQ:[], modalOpen:false,
  over:false, victory:false, introDone:false,
};
...
const WORLD_EVENTS = [
  { id:'norman_shadow', triggerYear:1066, maxYear:1075, chance:0.6, fired:false,
    run:(p)=>popup({title:'노르만의 그림자', sub:'세계 소식', body:`잉글랜드에서 충격적인 소식이 전해집니다.\n노르만 공작 윌리엄이 해럴드 왕을 헤이스팅스에서 꺾고 왕좌를 차지했습니다.\n에이레 서쪽에 새로운 강자가 등장했습니다.`,
      opts:[
  ...
  { id:'viking_dublin', triggerYear:1068, maxYear:1090, chance:0.5, fired:false,
    run:(p)=>{
      ...
      popup({title:'더블린의 바이킹', sub:'해안 위협', body:`더블린의 노르드 해상 세력이 다시 활동을 시작했습니다.\n바다에서 온 배들이 동부 해안을 약탈하고 있습니다.`,
        opts:[
  ...
  { id:'papal_legate', triggerYear:1070, maxYear:1090, chance:0.5, fired:false,
    run:(p)=>popup({title:'교황 특사의 방문', sub:'신앙', body:`로마에서 온 특사가 아일랜드 교회의 개혁을 촉구합니다.\n그레고리우스 7세의 개혁 운동이 에이레에도 파급되고 있습니다.`,
      opts:[
  ...
  { id:'norman_interest', triggerYear:1075, maxYear:1100, chance:0.45, fired:false,
    run:(p)=>popup({title:'노르만의 야욕', sub:'외교 위기', body:`노르만 귀족들이 아일랜드의 풍요로운 땅에 눈독을 들이고 있다는 정보가 들어왔습니다.\n강한 아일랜드 왕국만이 그들을 막을 수 있습니다.`,
      opts:[
  ...
  { id:'scotland_pressure', triggerYear:1072, maxYear:1100, chance:0.4, fired:false,
    run:(p)=>popup({title:'알바 왕국의 시선', sub:'북방 위협', body:`스코틀랜드 말콤 3세의 기사들이 얼스터 국경을 넘어 정찰하고 있습니다.\n북쪽의 왕국이 아일랜드에 관심을 보이기 시작했습니다.`,
      opts:[
  ...
  { id:'storm_of_north', triggerYear:1080, maxYear:1100, chance:0.3, fired:false, repeatable:true,
    run:(p)=>popup({title:'북해의 폭풍', sub:'자연 재해', body:`북해에서 몰아친 폭풍이 아일랜드 해안을 강타했습니다.\n어선들이 파손되고 해안 마을이 피해를 입었습니다.`,
      opts:[
...
function askLifestyle(c){
  ...
  popup({title:'인생관', sub:'삶의 방향', body:`${c.name}은(는) 앞으로 어떤 통치자가 되려 합니까?\n(퍼크 시스템은 추후 — 현재는 경험치만 축적됩니다)`, opts});
}
...
function goldPulse(){
  for(const bid in BARONIES){
    const b=BARONIES[bid];
    const rgn=4+buildingBonus(bid,'troops_regen');
    b.troops=Math.min(b.cap, b.troops+rgn);
  }
...
function naturalDeathChance(a){
  if(a<16)  return 0.001;   
  if(a<40)  return 0.0015;  
  if(a<50)  return 0.003;   
  if(a<55)  return 0.006;   
  if(a<60)  return 0.010;   
  if(a<65)  return 0.016;   
  if(a<70)  return 0.025;   
  if(a<75)  return 0.038;   
  if(a<80)  return 0.055;   
  return 0.08;               
}
  ...
  state.schemes=state.schemes.filter(s=>{
    ...
    if(roll<succ){
      ...
      if(s.target===state.player){
        popup({title:'독배', sub:'암살', body:'연회의 술잔에 독이 들어 있었습니다.\n시야가 흐려지고, 다리에 힘이 풀립니다...', opts:[{t:'...', f:()=>kill(t,'독살')}]});
      } else kill(t,'독살');
      return false;
    }
...
function declareWar(atk,def,targetRid){
  ...
  let tCid = null; 
  if(COUNTIES[tRid]){ tCid=tRid; tRid=COUNTIES[tRid].capital; } 
  else if(BARONIES[tRid]){ tCid=BARONIES[tRid].county; }        
  if(!tRid||!BARONIES[tRid]){ return false; }
  state.wars.push({atk:atk.id, def:def.id, targetRid:tCid||tRid, score:0, months:0, allies:[]});
  chOp(def,atk,-40);
...
function warPulse(){
  state.wars=state.wars.filter(w=>{
    ...
    if(w.months%3===0&&(w.atk===state.player||w.def===state.player)){
      ...
      log(`전황: ${my>=0?'+':''}${Math.round(my)}%${siege} — ${my>30?'아군 우세':my<-30?'적군 우세':'교착 상태'}`,'war');
    }
    ...
    if((w.atk===state.player||w.def===state.player) && Math.random()<0.15 && !state.modalOpen){
      ...
      const foe=isAtk?d:a;
      const events=[
        {cond:isAtk&&ratio>0.1, t:'전선 돌파!', body:`${foe.name}의 진지가 흔들립니다. 결정적 추격을 명할 수 있습니다.`,
         opts:[{t:'추격한다', d:'전황 +15, 소모 +', f:()=>{w.score=Math.min(100,w.score+15); for(const b of regionsOf(a.id)) BARONIES[b].troops=Math.max(80,Math.round(BARONIES[b].troops*0.94));}},
               {t:'진지를 굳힌다', d:'현 상태 유지', f:()=>{}}]},
        {cond:!isAtk&&ratio<-0.1, t:'적군 포위!', body:`${foe.name}의 군대가 보급선을 끊었습니다. 돌파를 시도합니까?`,
         opts:[{t:'돌파한다', d:'50% 성공, 실패시 대손실', f:()=>{if(Math.random()<0.5){w.score=Math.min(100,w.score+20);}else{for(const b of regionsOf(d.id)) BARONIES[b].troops=Math.max(50,Math.round(BARONIES[b].troops*0.75));}}},
               {t:'방어를 유지한다', d:'전황 -5', f:()=>{w.score=Math.max(-100,w.score-5);}}]},
        {cond:true, t:'소규모 충돌', body:`국경에서 소규모 전투가 벌어졌습니다. 양측 모두 피해를 입었습니다.`,
         opts:[{t:'전선을 정비한다', d:'소모 증가', f:()=>{ for(const b of regionsOf(a.id)) BARONIES[b].troops=Math.max(80,Math.round(BARONIES[b].troops*0.96)); }}]},
      ];
      const ev=events.find(e=>e.cond);
      if(ev) popup({title:ev.t, sub:'전황', body:ev.body, opts:ev.opts});
    }
    ...
    if(w.score<=-100){
      ...
      if(w.atk===state.player){ addStress(a,30,'패전의 굴욕'); addClaim(tCid||w.targetRid,'unpressed'); }
      if(w.def===state.player){ addStress(d,-10,'승전의 기쁨'); if(dSeat) dSeat.gold+=80; }
      return false;
    }
    if(w.months>60){
...
function conquerTarget(a, d, targetCid){
  let cid = targetCid;
  if(BARONIES[targetCid]) cid = BARONIES[targetCid].county; 
  ...
  if(a.id===state.player){
    ...
    const canVassal=remCnt>0&&!d.dead; 
    popup({title:'정복', sub:cname, body:`<b>${cname}</b>이(가) 당신의 깃발 아래 들어왔습니다!${remCnt>0?`\n${d.name}에게는 아직 ${remCnt}개 백작령이 남아있습니다.`:'\n'+d.name+'은(는) 당신의 궁정에 무릎 꿇었습니다.'}`,
      opts:[
...
function npcCouncilPulse(){
  for(const id in chars){
    ...
    if(r.council.chancellor && Math.random()<0.06){
      const cid = r.council.chancellor;
      const chan = chars[cid];
      if(!chan||chan.dead){ r.council.chancellor=null; }
      const sk = (cid===r.id) ? Math.round(stat(r,'dip')*0.6) : (chan?stat(chan,'dip'):4);
      const adjCids = COUNTY_ADJ[countyOf(r.region)]||[];
      const targets = adjCids.filter(cid=>{
        const holder=countyHolder(cid);
        return holder && holder.id!==r.id && !r.claims.find(c=>c.rid===cid);
      });
      if(targets.length && Math.random() < 0.3+sk*0.04){
        const cid = targets[Math.floor(Math.random()*targets.length)];
        r.claims.push({rid:cid, type:'unpressed', obtained:state.year});
        if(countyHolder(cid)?.id===state.player)
          log(`${r.name}의 재상이 <b>${COUNTIES[cid].n}</b>에 대한 명분을 위조했습니다!`,'war');
      }
    }
...
const NPC_ACTIVITIES = [
  {
    id:'feast', n:'연회', icon:'🍖', cond:(r,reg)=>reg.gold>80,
    run:(r,reg,adj)=>{
      reg.gold -= 60; r.lastActivity = state.year;
      const guests = adj.filter(t=>t&&t.id!==r.id&&!t.dead);
      const inviteCount = Math.min(guests.length, 1+Math.floor(Math.random()*2));
      const invited = guests.sort(()=>Math.random()-0.5).slice(0,inviteCount);
      invited.forEach(g=>{ chOp(g,r,10); chOp(r,g,7); });
      if(invited.some(g=>g.id===state.player) && Math.random()<0.5){
        const p=playerChar();
        popup({title:`${r.name}의 연회 초대`, sub:`${COUNTIES[countyOf(r.region)]?.n||''} 왕국`, body:`${r.name}이(가) 성대한 연회를 열고 당신을 초대했습니다.`,
          opts:[
        ...
        log(`${r.name}이(가) ${invited[0].name}을(를) 연회에 초대했습니다.`,'dip');
      }
    }
  },
  {
    id:'hunt', n:'사냥', icon:'🦌', cond:(r,reg)=>reg.gold>20,
    run:(r,reg,adj)=>{
      reg.gold -= 15; r.lastActivity = state.year;
      const partner = adj.find(t=>t&&t.id!==r.id&&!t.dead&&opinion(r,t)>-20);
      if(!partner) return;
      chOp(partner,r,8); chOp(r,partner,8);
      if(partner.id===state.player){
        popup({title:`${r.name}의 사냥 초대`, sub:'여흥', body:`${r.name}이(가) 가을 사냥에 동행할 것을 청합니다.\n자연 속에서 동료로서의 유대를 다질 기회입니다.`,
          opts:[
            {t:'함께 사냥을 나선다', d:'관계 +10, 스트레스 -8', f:()=>{ chOp(r,playerChar(),10); addStress(playerChar(),-8,'사냥의 즐거움'); }},
            {t:'바쁘다고 한다', d:'관계 -5', f:()=>chOp(r,playerChar(),-5)},
          ]});
      }
    }
  },
  {
    id:'pilgrimage', n:'순례', icon:'✝', cond:(r)=>r.traits.includes('zealous')||r.traits.includes('pious')||Math.random()<0.3,
    run:(r,reg,adj)=>{
      r.lastActivity = state.year; reg.pop = Math.min(100,(reg.pop||60)+6);
      adj.filter(t=>t&&t.id!==r.id&&(t.traits.includes('zealous')||t.traits.includes('pious'))).forEach(t=>{ chOp(t,r,6); chOp(r,t,6); });
      if(adj.some(t=>t&&t.id===state.player)) log(`${r.name}이(가) 클론맥노이즈 수도원으로 순례를 떠났습니다.`,'dip');
    }
  },
  {
    id:'diplomacy', n:'외교 방문', icon:'🤝', cond:(r,reg,adj)=>adj.some(t=>t&&t.id!==r.id&&opinion(r,t)>0),
    run:(r,reg,adj)=>{
      r.lastActivity = state.year;
      const target = adj.find(t=>t&&t.id!==r.id&&!t.dead&&opinion(r,t)>0);
      if(!target) return;
      chOp(target,r,10); chOp(r,target,10);
      if(target.id===state.player){
        popup({title:'외교 방문', sub:`${r.name}의 방문`, body:`${r.name}이(가) 직접 당신의 궁정을 찾아왔습니다.\n「먼스터의 명성이 에이레에 울려 퍼집니다, 전하.」`,
          opts:[
            {t:'환대한다', d:'관계 +15, 금 -20', f:()=>{
              if(REGIONS[playerChar().region].gold>=20) REGIONS[playerChar().region].gold-=20;
              chOp(r,playerChar(),15); log(`${r.name}을(를) 극진히 맞이했습니다.`,'good');
            }},
...
function npcActivityPulse(){
  for(const id in chars){
    const r = chars[id];
    if(r.dead || !r.ruler || id===state.player || !r.region || !REGIONS[r.region]) continue;
    if(r.lastActivity >= state.year-1) continue; 
    if(Math.random()>0.15) continue;
    const reg = REGIONS[r.region];
    const adj = (ADJ[r.region]||[]).map(x=>ownerOf(x)).filter(Boolean);
    const possible = NPC_ACTIVITIES.filter(a=>a.cond(r,reg,adj));
  ...
  for(const rid in REGIONS){
    ...
    if(claimRid){
      ...
      if(defChar && defChar.id!==r.id && !isAllied(r.id,defChar.id) && !truceBetween(r.id,defChar.id)
        ...
        if(defChar.id!==p.id) log(`<b>${r.name}</b>이(가) 명분을 내세워 <b>${COUNTIES[claimRid]?.n||claimRid}</b>에 선전포고했습니다.`,'war');
        continue;
      }
    }
    ...
    if(schemeTarget && Math.random()<0.10+(venge*0.02)){
      if(!state.schemes.some(s=>s.plotter===r.id)){
        ...
        if(schemeTarget.id===p.id) log(`${r.name}이(가) 어둠 속에서 당신을 노리고 있다는 첩보가 들어왔습니다.`,'war');
      }
      continue;
    }
...
function rebellionEvent(rid){
  const p=playerChar();
  popup({title:'반란의 기운', sub:`${COUNTIES[countyOf(rid)]?.n||BARONIES[rid]?.n||'영지'} — 민중의견 위기`, body:`해당 지역의 농민들이 창과 낫을 들었습니다.\n세금과 전쟁에 지친 그들의 눈에 분노가 타오르고 있습니다.\n\n민중의견: ${REGIONS[rid].pop}/100`,
    opts:[
...
function npcDiplomacyToPlayer(r){
  ...
  if(kind<0.4&&op>-20){
    popup({title:`${REGIONS[r.region].n}의 사절`, sub:'외교 — 동맹 제안', body:`${r.name}이(가) 사절을 보냈습니다.\n"에이레의 평화를 위해 손을 잡읍시다. 동맹을 제안합니다."`,
      opts:[
    ...
    popup({title:`${REGIONS[r.region].n}의 선물`, sub:'외교', body:`${r.name}이(가) 우호의 표시로 은제 술잔과 사냥개를 보냈습니다.`,
      opts:[{t:'받아들인다', d:'관계 +15', f:()=>{chOp(p,r,15);chOp(r,p,10);}}, {t:'돌려보낸다', d:'관계 -15', f:()=>{chOp(r,p,-15);}}]});
  } else {
    ...
    popup({title:`혼담`, sub:`외교 — ${REGIONS[r.region].n}`, body:`${r.name}이(가) 가문 간 혼인을 제안합니다.\n대상: 당신의 ${kid.sex==='m'?'아들':'딸'} <b>${kid.name}</b>(${age(kid)}세)\n\n혼인은 두 가문을 묶는 가장 단단한 사슬입니다.`,
      opts:[
        {t:'혼약을 맺는다', d:'관계 +35, 동맹', f:()=>{
          const sp=mk({name:r.dyn+' 가문의 '+(kid.sex==='m'?'딸':'아들'), dyn:r.dyn, sex:kid.sex==='m'?'f':'m', byear:kid.byear, bmonth:1, bday:1, traits:randTraits(2), base:randStats(), edu:1, eduFocus:'dip', courtOf:p.region});
          if(age(kid)>=16){ kid.spouse=sp.id; sp.spouse=kid.id; }
          chOp(p,r,35); chOp(r,p,35); formAlliance(p.id, r.id);
...
/* ---------- 랜덤 이벤트 ---------- */
const EVENTS=[
  {cond:c=>true, w:3, run:c=>popup({title:'흉작의 소문', sub:'영지', body:'올해 보리 수확이 시원치 않다는 보고가 올라왔습니다. 농민들이 동요하고 있습니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'전염병 소식', sub:'영지', body:'인근 마을에서 발병이 시작됐습니다. 아직 성 안까지는 들어오지 않았지만, 농민들이 도망치고 있습니다.',
    opts:[
  ...
  {cond:c=>REGIONS[c.region].gold>200, w:2, run:c=>popup({title:'성벽 보수', sub:'영지', body:'성벽 일부가 무너졌습니다. 이번 겨울 전에 손을 봐야 할 것 같습니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'음유시인', sub:'궁정', body:'떠돌이 시인이 궁정에 찾아와 당신의 가문을 칭송하는 노래를 지어 바치겠다 합니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>{
    const others=Object.values(chars).filter(k=>!k.dead&&k.ruler&&k.id!==c.id); if(!others.length) return;
    const o=others[Math.floor(Math.random()*others.length)];
    popup({title:'국경의 분쟁', sub:REGIONS[o.region]?REGIONS[o.region].n:'인근 왕국', body:`${REGIONS[o.region]?REGIONS[o.region].n:'인근'}의 목동들이 국경을 넘어 우리 농민들과 충돌했습니다. 사상자가 나왔습니다.`,
      opts:[
  ...
  {cond:c=>Object.values(chars).filter(k=>!k.dead&&k.courtOf===c.region).length>0, w:2, run:c=>{
    const court=Object.values(chars).filter(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id); if(!court.length) return;
    const npc=court[Math.floor(Math.random()*court.length)];
    popup({title:'신하의 청원', sub:'궁정', body:`${npc.name}이(가) 개인적인 청원을 올렸습니다.\n가문의 오래된 영지를 되찾게 도와달라는 것입니다.`,
      opts:[
  ...
  {cond:c=>age(c)>=45, w:2, run:c=>popup({title:'건강의 신호', sub:'내면', body:'아침마다 무릎이 쑤십니다. 시의가 조심스럽게 식단을 바꾸라고 권합니다.\n나이는 속일 수 없는 것입니다.',
    opts:[
      {t:'시의의 말을 따른다', d:'스트레스 +10', f:()=>addStress(c,10,'늙어가는 몸을 받아들임')},
      {t:'무시한다', d:'스트레스 +5', f:()=>addStress(c,5,'외면한 경고')},
    ]})},
  {cond:c=>state.wars.some(w=>w.atk===c.id||w.def===c.id), w:4, run:c=>popup({title:'전장의 소식', sub:'전쟁', body:'전선에서 전령이 왔습니다. 병사들의 사기가 흔들리고 있다고 합니다.\n보급이 부족한 것이 원인입니다.',
    opts:[
  ...
  {cond:c=>!state.wars.some(w=>w.atk===c.id||w.def===c.id)&&playerRegions().length>=2, w:2, run:c=>popup({title:'새 영토의 민심', sub:'통치', body:'새로 얻은 영지의 백성들이 아직 당신을 낯설어합니다.\n그들의 마음을 얻는 데는 시간이 필요합니다.',
    opts:[
  ...
  {cond:c=>opinion(c, Object.values(chars).find(k=>k.ruler&&k.id!==c.id&&!k.dead)||c)<-30, w:3, run:c=>{
    const enemy=Object.values(chars).find(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)<-30); if(!enemy) return;
    popup({title:'적의의 소문', sub:'첩보', body:`${enemy.name}이(가) 당신을 두고 모욕적인 말을 했다는 소문이 들립니다.\n"먼스터의 왕좌는 오래 버티지 못할 것이다."`,
      opts:[
        {t:'공개 반박한다', d:'위신 +10, 적대 관계 -10', f:()=>{state.prestige+=10; chOp(enemy,c,-10); log(`${enemy.name}의 모욕에 당당히 답했습니다.`,'dip');}},
        {t:'무시한다', d:'스트레스 +10', f:()=>{addStress(c,10,'삼킨 치욕');}},
  ...
  {cond:c=>Object.values(chars).filter(k=>!k.dead&&(k.father===c.id||k.mother===c.id)).length>0, w:2, run:c=>{
    const child=Object.values(chars).filter(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)<16)[0]; if(!child) return;
    popup({title:'아이의 장난', sub:'가문', body:`${child.name}(${age(child)}세)이(가) 말을 훔쳐 타다 넘어졌습니다. 다행히 크게 다치지는 않았습니다.\n아이의 눈에는 아직 두려움보다 흥분이 더 많습니다.`,
      opts:[
  ...
  {cond:c=>chars[c.spouse]&&!chars[c.spouse].dead, w:2, run:c=>{
    ...
    popup({title:'배우자의 말', sub:'가문', body:`${sp.name}이(가) 오늘 밤 조용히 말했습니다.\n"당신이 너무 지쳐 보여요. 잠시라도 쉬어요."`,
      opts:[
        {t:'고맙다고 한다', d:'스트레스 -15, 관계 +5', f:()=>{addStress(c,-15,'배우자의 위로'); chOp(sp,c,5);}},
        {t:'괜찮다고 한다', d:'스트레스 +8', f:()=>addStress(c,8,'혼자 버티는 무게')},
      ]});
  }},
  {cond:c=>stat(c,'intr')>=6, w:2, run:c=>popup({title:'밀정의 보고', sub:'계책', body:'당신의 밀정이 인근 궁정에서 흥미로운 소문을 가져왔습니다.\n한 영주가 비밀스러운 약점을 숨기고 있다는 것입니다.',
    opts:[
      {t:'약점을 캔다', d:'음모 경험, 스트레스 +5', f:()=>{addStress(c,5,'은밀한 모의'); state.prestige+=8; log('약점을 손에 넣었습니다. 언젠가 쓸모가 있겠지요.','dip');}},
      {t:'관심 없다', d:'—', f:()=>{}},
    ]})},
  {cond:c=>true, w:2, run:c=>popup({title:'독초를 아는 노파', sub:'계책', body:'늪지에 사는 한 노파가 온갖 약초와 독초에 능하다는 소문이 돕니다.\n그녀를 궁정에 들일 수도 있습니다.',
    opts:[
      {t:'은밀히 부른다', d:'금 -20, 음모 +위험', f:()=>{REGIONS[c.region].gold-=20; if(c.traits.includes('deceitful'))addStress(c,-5,'기만하는 자의 만족'); log('노파가 궁정에 자리잡았습니다.','dip');}},
      {t:'마녀라며 쫓는다', d:'민심 +5', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5);}},
    ]})},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)<-25), w:2, run:c=>{
    const foe=Object.values(chars).find(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)<-25); if(!foe) return;
    popup({title:'위조된 편지', sub:'계책', body:`서기가 은밀히 제안합니다. ${foe.name}의 필체를 흉내내어 그를 곤경에 빠뜨릴 편지를 위조할 수 있다고 합니다.`,
      opts:[
        ...
        {t:'정도를 지킨다', d:'스트레스 -5', f:()=>{addStress(c,-5,'양심의 평안');}},
      ]});
  }},
  {cond:c=>true, w:3, run:c=>popup({title:'수도원의 청원', sub:'신앙', body:'클론퍼트 수도원의 원장이 찾아왔습니다.\n낡은 필사실을 새로 짓는 데 후원을 청합니다. 신의 가호가 따를 것이라 합니다.',
    opts:[
      {t:'후하게 기부한다', d:'금 -50, 위신 +20, 스트레스 -10', f:()=>{REGIONS[c.region].gold-=50; state.prestige+=20; addStress(c,-10,'신앙의 위안');}},
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'순례자의 무리', sub:'신앙', body:'성 패트릭의 발자취를 따르는 순례자들이 영지를 지나갑니다.\n그들에게 잠자리와 음식을 베풀 수 있습니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'불길한 징조', sub:'신앙', body:'한낮에 까마귀 떼가 성탑 위를 맴돌았습니다.\n늙은 사제들이 불길한 징조라며 수군거립니다. 백성들도 동요합니다.',
    opts:[
      ...
      {t:'미신이라 일축한다', d:'민심 -8', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-8);}},
    ]})},
  {cond:c=>true, w:3, run:c=>popup({title:'상인 길드의 제안', sub:'내정', body:'워터퍼드의 상인들이 교역 특권을 요청합니다.\n그 대가로 매달 일정한 상납을 약속합니다.',
    opts:[
  ...
  {cond:c=>REGIONS[c.region].gold>150, w:2, run:c=>popup({title:'다리 건설', sub:'내정', body:'섀넌 강을 가로지르는 다리를 놓자는 제안이 올라왔습니다.\n비용은 크지만 교역과 이동이 크게 편해질 것입니다.',
    opts:[
      {t:'건설을 명한다', d:'금 -100, 민심 +15, 위신 +10', f:()=>{REGIONS[c.region].gold-=100; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+15); state.prestige+=10; log('다리가 완성되었습니다. 백성들이 기뻐합니다.','good');}},
      {t:'시기상조다', d:'—', f:()=>{}},
    ]})},
  {cond:c=>true, w:2, run:c=>popup({title:'세금 징수 보고', sub:'내정', body:'징세관이 올해 세수를 보고합니다.\n일부 지주가 세금을 체납하고 있는데, 강하게 나갈지 정하셔야 합니다.',
    opts:[
      {t:'엄격히 징수한다', d:'금 +50, 민심 -10', f:()=>{REGIONS[c.region].gold+=50; REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-10);}},
      {t:'관대히 봐준다', d:'민심 +10, 위신 +5', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+10); state.prestige+=5;}},
    ]})},
  {cond:c=>true, w:2, run:c=>popup({title:'대장간의 호황', sub:'내정', body:'영지의 대장장이들이 좋은 철광을 발견했습니다.\n무기와 농기구 생산이 늘어날 조짐입니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'용병단의 방문', sub:'군사', body:'바다 건너온 노르드 용병단이 일자리를 찾고 있습니다.\n금을 주면 당신의 깃발 아래 싸우겠다 합니다.',
    opts:[
  ...
  {cond:c=>stat(c,'mar')>=7, w:2, run:c=>popup({title:'무술 시합', sub:'군사', body:'기사들이 무술 시합을 열자고 청합니다.\n왕이 직접 참가하면 사기가 오를 것입니다. 다만 부상의 위험도 있습니다.',
    opts:[
      {t:'직접 출전한다', d:'위신 +15, 부상 위험', f:()=>{state.prestige+=15; if(Math.random()<0.2){addStress(c,15,'시합 중 부상'); log('시합에서 부상을 입었습니다.','war');}else{addStress(c,-15,'승리의 영광');}}},
      {t:'관전만 한다', d:'위신 +5', f:()=>{state.prestige+=5;}},
    ]})},
  {cond:c=>true, w:2, run:c=>popup({title:'탈영병', sub:'군사', body:'몇몇 병사가 군영을 이탈했습니다.\n붙잡혔는데, 어떻게 처리할지 본보기가 필요합니다.',
    opts:[
      {t:'처형한다', d:'병력 규율 +, 스트레스 +10', f:()=>{addStress(c,10,'본보기의 무게'); log('탈영병을 처형해 군기를 세웠습니다.','war');}},
      {t:'용서한다', d:'민심 +5, 군기 -', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5);}},
    ]})},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)>10), w:1, run:c=>{
    const friend=Object.values(chars).find(k=>!k.dead&&k.ruler&&k.id!==c.id&&opinion(c,k)>10); if(!friend) return;
    popup({title:'우호의 초대', sub:`외교 — ${REGIONS[friend.region]?REGIONS[friend.region].n:''}`, body:`${friend.name}이(가) 자신의 궁정 연회에 당신을 초대했습니다.\n참석하면 관계가 깊어지겠지만, 자리를 비우는 동안의 위험도 있습니다.`,
      opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'바이킹의 그림자', sub:'외교', body:'더블린 만에 노르드 장선들이 나타났다는 소식입니다.\n약탈인지 교역인지 아직 분명치 않습니다.',
    opts:[
  ...
  {cond:c=>true, w:2, run:c=>popup({title:'떠돌이 학자', sub:'궁정', body:'먼 곳에서 온 학자가 궁정에 머물기를 청합니다.\n그의 지식은 깊어 보이지만, 출신이 불분명합니다.',
    opts:[
      {t:'궁정에 들인다', d:'금 -20, 학문 자극', f:()=>{REGIONS[c.region].gold-=20; mk({name:randName(),dyn:'',byear:state.year-35,bmonth:3,bday:3,traits:randTraits(2),base:{dip:5,mar:2,stew:5,intr:4,learn:9,prow:1},edu:3,eduFocus:'learn',courtOf:c.region}); log('학자가 궁정에 합류했습니다.','fam');}},
      {t:'거절한다', d:'—', f:()=>{}},
    ]})},
  {cond:c=>true, w:2, run:c=>popup({title:'사냥개 선물', sub:'궁정', body:'한 봉신이 혈통 좋은 아이리시 울프하운드 한 마리를 선물로 바쳤습니다.\n늠름한 사냥개입니다.', opts:[{t:'기쁘게 받는다', d:'스트레스 -10', f:()=>{addStress(c,-10,'충직한 벗'); log('사냥개에게 이름을 지어주었습니다.','fam');}}, {t:'사냥터지기에게 준다', d:'관계 +5', f:()=>{}}]})},
  {cond:c=>chars[c.spouse]&&!chars[c.spouse].dead&&age(chars[c.spouse])<45&&c.sex==='m', w:2, run:c=>{
    const sp=chars[c.spouse]; if(!sp||sp.dead) return;
    popup({title:'후사의 기대', sub:'가문', body:`${sp.name}과(와)의 사이에 더 많은 자손을 보아야 한다는 신하들의 조언이 있습니다.\n가문의 미래가 걸린 일입니다.`, opts:[{t:'함께 시간을 보낸다', d:'관계 +10, 스트레스 -10', f:()=>{chOp(sp,c,10); chOp(c,sp,10); addStress(c,-10,'부부의 정');}}, {t:'국정이 우선이다', d:'스트레스 +5', f:()=>{addStress(c,5,'미뤄둔 가정');}}]});
  }},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=16&&!k.spouse), w:2, run:c=>{
    const adult=Object.values(chars).find(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=16&&!k.spouse); if(!adult) return;
    popup({title:'장성한 자식', sub:'가문', body:`당신의 ${adult.sex==='m'?'아들':'딸'} ${adult.name}(${age(adult)}세)이(가) 이제 제 앞가림을 할 나이가 되었습니다.\n혼처를 알아봐야 할 때입니다.`, opts:[{t:'좋은 혼처를 찾아본다', d:'외교에서 혼인 교섭 가능', f:()=>log(`${adult.name}의 혼처를 알아보기로 했습니다.`,'fam')}, {t:'아직 이르다', d:'관계 -5', f:()=>{chOp(adult,c,-5);}}]});
  }},
  {cond:c=>age(c)>=50, w:2, run:c=>popup({title:'지난 세월', sub:'내면', body:'벽난로 앞에서 문득 지나온 날들을 떠올립니다.\n젊은 날의 야망, 잃어버린 벗들, 그리고 아직 이루지 못한 꿈들.', opts:[{t:'회한에 잠긴다', d:'스트레스 +8', f:()=>addStress(c,8,'지난날의 회한')}, {t:'남은 날을 다짐한다', d:'스트레스 -10, 위신 +5', f:()=>{addStress(c,-10,'새로운 각오'); state.prestige+=5;}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'겨울 폭풍', sub:'재해', body:'유난히 혹독한 겨울입니다. 폭풍이 농가의 지붕을 날리고 가축이 얼어 죽었습니다.', opts:[{t:'구휼미를 푼다', d:'금 -50, 민심 +15', f:()=>{REGIONS[c.region].gold-=50; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+15);}}, {t:'각자 버티게 한다', d:'민심 -12', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-12);}}]})},
  {cond:c=>true, w:1, run:c=>popup({title:'풍년', sub:'영지', body:'올해는 보기 드문 풍년입니다. 곳간이 가득 차고 백성들의 얼굴에 웃음이 돕니다.', opts:[{t:'추수 축제를 연다', d:'금 -20, 민심 +15, 스트레스 -10', f:()=>{REGIONS[c.region].gold-=20; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+15); addStress(c,-10,'풍요의 기쁨');}}, {t:'잉여를 비축한다', d:'금 +50', f:()=>{REGIONS[c.region].gold+=50;}}]})},
  {cond:c=>true, w:1, run:c=>popup({title:'들불', sub:'재해', body:'마른 가을 들판에 불이 번졌습니다. 농민들이 필사적으로 진화에 나섰습니다.', opts:[{t:'병력을 동원해 돕는다', d:'민심 +12, 병력 일시 차출', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+12); log('병사들이 들불을 잡았습니다.','good');}}, {t:'농민에게 맡긴다', d:'민심 -8', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-8);}}]})},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id), w:2, run:c=>{
    const v=Object.values(chars).filter(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id); if(!v.length) return;
    const va=v[Math.floor(Math.random()*v.length)];
    popup({title:'봉신의 불만', sub:'궁정', body:`${va.name}이(가) 다른 봉신들 앞에서 당신의 통치에 불만을 토로했습니다.\n그냥 두면 권위가 흔들릴 수 있습니다.`, opts:[{t:'엄히 질책한다', d:'관계 -15, 위신 +10', f:()=>{chOp(va,c,-15); state.prestige+=10;}}, {t:'달래어 회유한다', d:'금 -30, 관계 +15', f:()=>{REGIONS[c.region].gold-=30; chOp(va,c,15);}}, {t:'무시한다', d:'위신 -8', f:()=>{state.prestige=Math.max(0,state.prestige-8);}}]});
  }},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id&&stat(k,'mar')>=8), w:2, run:c=>{
    const knight=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id&&stat(k,'mar')>=8); if(!knight) return;
    popup({title:'충성스러운 기사', sub:'궁정', body:`용맹한 기사 ${knight.name}이(가) 당신에게 충성을 맹세하며 영지를 청합니다.\n그의 무용은 의심할 여지가 없습니다.`, opts:[{t:'약속한다', d:'관계 +25, 스트레스 +5', f:()=>{chOp(knight,c,25); addStress(c,5,'지킬 약속의 무게');}}, {t:'공을 더 세우라 한다', d:'관계 -5', f:()=>{chOp(knight,c,-5);}}]});
  }},
  {cond:c=>c.lifestyle=='dip', w:1, run:c=>popup({title:'외교의 묘', sub:'인생관 — 외교', body:'협상 자리에서 당신은 상대의 속내를 꿰뚫어 보았습니다.\n말 한마디로 분위기를 뒤집는 법을 터득해갑니다.', opts:[{t:'경험을 새긴다', d:'인생관 경험 +30', f:()=>{c.lifeXP+=30; addStress(c,-3,'성장의 기쁨');}}]})},
  {cond:c=>c.lifestyle=='mar', w:1, run:c=>popup({title:'전술의 깨달음', sub:'인생관 — 무예', body:'옛 전투를 복기하던 중, 당신은 새로운 진형의 가능성을 발견했습니다.', opts:[{t:'연구를 거듭한다', d:'인생관 경험 +30', f:()=>{c.lifeXP+=30; addStress(c,-3,'성장의 기쁨');}}]})},
  {cond:c=>c.lifestyle=='stew', w:1, run:c=>popup({title:'장부의 지혜', sub:'인생관 — 내정', body:'복잡한 세무 장부를 들여다보다, 낭비되던 재정의 구멍을 찾아냈습니다.', opts:[{t:'개선책을 적용한다', d:'금 +30, 인생관 경험 +30', f:()=>{REGIONS[c.region].gold+=30; c.lifeXP+=30;}}]})},
  {cond:c=>c.lifestyle=='intr', w:1, run:c=>popup({title:'그림자의 기술', sub:'인생관 — 음모', body:'밀정들을 다루는 법, 비밀을 캐는 법, 흔적을 지우는 법.\n당신은 어둠의 기술에 점점 능해집니다.', opts:[{t:'기술을 연마한다', d:'인생관 경험 +30', f:()=>{c.lifeXP+=30; addStress(c,-3,'은밀한 성취');}}]})},
  {cond:c=>c.lifestyle=='learn', w:1, run:c=>popup({title:'고서의 발견', sub:'인생관 — 학문', body:'수도원 서고 깊은 곳에서 오래된 라틴어 필사본을 발견했습니다.\n잊혀진 지식이 그 안에 잠들어 있습니다.', opts:[{t:'밤새 탐독한다', d:'위신 +5, 인생관 경험 +30', f:()=>{state.prestige+=5; c.lifeXP+=30; addStress(c,-3,'지식의 기쁨');}}]})},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=6&&age(k)<16), w:2, run:c=>{
    const child=Object.values(chars).find(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=6&&age(k)<16); if(!child) return;
    popup({title:'아이의 재능', sub:'가문', body:`${child.name}(${age(child)}세)이(가) 또래보다 영민하다는 것이 보입니다.\n어떻게 길러야 할지 고민이 됩니다.`, opts:[{t:'엄격히 교육시킨다', d:'교육 점수 +3, 관계 -5', f:()=>{child.eduScore+=3; chOp(child,c,-5);}}, {t:'자유롭게 둔다', d:'관계 +10', f:()=>{chOp(child,c,10);}}]});
  }},
  {cond:c=>age(c)>=40, w:2, run:c=>popup({title:'노쇠의 조짐', sub:'건강', body:'이른 아침 일어서려다 현기증을 느꼈습니다. 시의가 조심스럽게 말합니다. 몸이 과거와 같지 않습니다.', opts:[{t:'규칙적인 생활을 시작한다', d:'스트레스 -10', f:()=>{addStress(c,-10,'절제의 시작');}}, {t:'무시한다', d:'스트레스 +8', f:()=>addStress(c,8,'외면한 몸의 경고')}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'부상당한 기사', sub:'군사', body:'훈련 중 믿음직한 기사 한 명이 크게 다쳤습니다. 치료비가 만만치 않지만, 그의 충성심은 값을 매기기 어렵습니다.',
    opts:[
      {t:'최선의 치료를 해준다', d:'금 -30, 관계 +20', f:()=>{
        if(REGIONS[c.region].gold<30){log('금이 부족합니다.');return;} REGIONS[c.region].gold-=30;
        const ct=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id&&stat(k,'mar')>=6); if(ct) chOp(ct,c,20);
      }},
      {t:'운명에 맡긴다', d:'스트레스 +5', f:()=>addStress(c,5,'외면한 충성')},
    ]})},
  {cond:c=>stressLvl(c)>=2, w:4, run:c=>popup({title:'몸이 말을 듣지 않는다', sub:'건강', body:'스트레스가 몸으로 나타나기 시작했습니다. 손이 떨리고 밥맛이 없습니다. 주변에서 걱정하는 눈빛이 역력합니다.', opts:[{t:'완전히 쉰다', d:'스트레스 -40, 금 -20', f:()=>{addStress(c,-40,'휴식의 치유'); REGIONS[c.region].gold-=20;}}, {t:'계속 버틴다', d:'스트레스 +15', f:()=>addStress(c,15,'몸을 혹사한 대가')}]})},
  {cond:c=>chars[c.spouse]&&!chars[c.spouse].dead, w:2, run:c=>{
    const sp=chars[c.spouse]; if(!sp||sp.dead) return;
    popup({title:'배우자의 병환', sub:'가문', body:`${sp.name}이(가) 며칠째 앓아눕고 있습니다. 시의는 원인을 잘 모르겠다고 합니다.`, opts:[{t:'곁에서 간호한다', d:'스트레스 -15, 관계 +10', f:()=>{addStress(c,-15,'곁에 있어줌'); chOp(sp,c,10);}}, {t:'시의에게 맡긴다', d:'금 -30', f:()=>{if(REGIONS[c.region].gold>=30) REGIONS[c.region].gold-=30;}}]});
  }},
  {cond:c=>true, w:1, run:c=>popup({title:'기적의 샘', sub:'신앙', body:'인근 마을에서 병을 고쳤다는 샘이 발견됐습니다. 순례자들이 몰려들고 있습니다.', opts:[{t:'성지로 공인한다', d:'민심 +12, 위신 +8', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+12); state.prestige+=8;}}, {t:'미신이라며 단속한다', d:'민심 -5', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-5);}}]})},
  {cond:c=>c.traits.includes('ambitious'), w:3, run:c=>popup({title:'야심의 속삭임', sub:'내면', body:'밤마다 같은 꿈을 꿉니다. 타라 언덕에서 하이킹의 관을 쓰는 꿈. 하지만 꿈과 현실 사이의 거리가 너무 멉니다.', opts:[{t:'집념을 불태운다', d:'스트레스 +8, 위신 +5', f:()=>{addStress(c,8,'야망의 무게'); state.prestige+=5;}}, {t:'현실을 받아들인다', d:'스트레스 -5', f:()=>{addStress(c,-5,'잠깐의 평온');}}]})},
  {cond:c=>c.traits.includes('wrathful'), w:3, run:c=>popup({title:'분노의 폭발', sub:'내면', body:'신하가 실수를 저질렀습니다. 사소한 일이지만 순간 이성을 잃고 소리를 질렀습니다.', opts:[{t:'사과하고 수습한다', d:'스트레스 +10, 관계 회복', f:()=>{addStress(c,10,'자존심을 굽힘');}}, {t:'위엄으로 포장한다', d:'위신 -5, 스트레스 -5', f:()=>{addStress(c,-5,'분노의 해소'); state.prestige=Math.max(0,state.prestige-5);}}]})},
  {cond:c=>c.traits.includes('greedy'), w:3, run:c=>popup({title:'탐욕의 유혹', sub:'내면', body:'세금을 올려 금고를 채울 기회가 있습니다. 백성들은 힘들어지겠지만, 금화는 달콤합니다.', opts:[{t:'올린다', d:'금 +80, 민심 -15', f:()=>{REGIONS[c.region].gold+=80; REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-15);}}, {t:'참는다', d:'스트레스 +12', f:()=>addStress(c,12,'탐욕을 억누름')}]})},
  {cond:c=>c.traits.includes('kind'), w:3, run:c=>popup({title:'친절의 대가', sub:'내면', body:'어려운 농민 가족이 문을 두드렸습니다. 다음 달 세금을 낼 돈이 없다고 합니다.', opts:[{t:'면제해준다', d:'금 -20, 민심 +8, 스트레스 -8', f:()=>{REGIONS[c.region].gold-=20; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+8); addStress(c,-8,'친절의 보람');}}, {t:'원칙대로 징수한다', d:'스트레스 +15', f:()=>addStress(c,15,'친절한 자의 냉혹')}]})},
  {cond:c=>c.traits.includes('brave'), w:2, run:c=>popup({title:'용기의 시험', sub:'내면', body:'혼자서 적의 첩자를 발견했습니다. 호위병 없이 직접 대면할 수 있습니다.', opts:[{t:'직접 맞선다', d:'위신 +15, 위험', f:()=>{state.prestige+=15; if(Math.random()<0.15){addStress(c,20,'부상');}else{addStress(c,-10,'용기의 짜릿함');}}}, {t:'호위병을 부른다', d:'스트레스 +5', f:()=>addStress(c,5,'용감한 자의 망설임')}]})},
  {cond:c=>c.traits.includes('just'), w:2, run:c=>popup({title:'공정의 딜레마', sub:'내면', body:'두 봉신이 소유권 분쟁을 가져왔습니다. 법적으로는 한 쪽이 옳지만, 다른 한 쪽은 당신의 오랜 충신입니다.', opts:[{t:'법대로 판결한다', d:'위신 +10, 충신 관계 -15', f:()=>{state.prestige+=10; const kt=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&opinion(c,k)>20); if(kt) chOp(kt,c,-15);}}, {t:'충신의 편을 든다', d:'스트레스 +15', f:()=>addStress(c,15,'공정한 자의 편파')}]})},
  {cond:c=>c.traits.includes('craven'), w:2, run:c=>popup({title:'두려움의 그림자', sub:'내면', body:'국경 너머에서 군대가 움직인다는 소식이 들려왔습니다. 아직 확인도 안 됐는데 손이 떨립니다.', opts:[{t:'즉시 병력을 소집한다', d:'금 -40, 병력 +100', f:()=>{REGIONS[c.region].gold-=40; REGIONS[c.region].troops+=100;}}, {t:'일단 정탐꾼을 보낸다', d:'스트레스 -5', f:()=>addStress(c,-5)}]})},
  {cond:c=>c.traits.includes('paranoid')||c.traits.includes('deceitful'), w:2, run:c=>popup({title:'의심의 씨앗', sub:'내면', body:'가장 믿었던 신하가 적국과 서신을 교환했다는 제보가 들어왔습니다.', opts:[{t:'즉시 가둔다', d:'관계 -30, 위신 -5', f:()=>{const kt=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id); if(kt){chOp(kt,c,-30);} state.prestige=Math.max(0,state.prestige-5);}}, {t:'증거를 먼저 확인한다', d:'스트레스 +5', f:()=>addStress(c,5)}]})},
  {cond:c=>!state.wars.some(w=>w.atk===c.id||w.def===c.id)&&playerRegions().length>=2, w:2, run:c=>popup({title:'전후 처리', sub:'군사', body:'전쟁이 끝난 지 얼마 지나지 않았습니다. 부상병들이 고향으로 돌아가고 있고, 유족들의 눈물이 마르지 않았습니다.', opts:[{t:'위로금을 지급한다', d:'금 -50, 민심 +10, 병력 회복', f:()=>{REGIONS[c.region].gold-=50; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+10);}}, {t:'그냥 해산시킨다', d:'민심 -5', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-5);}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'포로 협상', sub:'군사', body:'인근 전투에서 잡힌 귀족 포로가 당신의 영지에 있습니다. 몸값을 받을 수도, 풀어줄 수도 있습니다.', opts:[{t:'몸값을 받는다', d:'금 +70', f:()=>{REGIONS[c.region].gold+=70;}}, {t:'인심 쓰며 풀어준다', d:'위신 +10, 관계 +15', f:()=>{state.prestige+=10;}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'탈주한 노르드 전사', sub:'군사', body:'인근에서 전쟁에 지친 노르드 전사 몇 명이 약탈을 벌이다 잡혔습니다.', opts:[{t:'처형한다', d:'민심 +5, 스트레스 +5', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5); addStress(c,5);}}, {t:'병력으로 흡수한다', d:'병력 +60, 위험', f:()=>{REGIONS[c.region].troops+=60;}}]})},
  {cond:c=>state.wars.some(w=>w.atk===c.id), w:3, run:c=>{
    const w=state.wars.find(ww=>ww.atk===c.id); if(!w) return; const d=chars[w.def]; if(!d) return;
    popup({title:'협상 제안', sub:'전쟁', body:`${d.name}이(가) 협상을 요청해왔습니다. 현재 전황: ${Math.round(w.score)}점`,
      opts:[
        {t:'협상에 응한다', d:'전쟁 종결, 위신 소폭 손실', f:()=>{ setTruce(w.atk,w.def,3); state.wars=state.wars.filter(x=>x!==w); state.prestige=Math.max(0,state.prestige-10); }},
        {t:'거부한다', d:'전쟁 계속', f:()=>{} },
      ]});
  }},
  {cond:c=>Object.values(chars).filter(k=>!k.dead&&k.ruler&&k.id!==c.id).length>=2, w:2, run:c=>{
    const rulers=Object.values(chars).filter(k=>!k.dead&&k.ruler&&k.id!==c.id); const a=rulers[0], b=rulers[1]; if(!a||!b) return;
    popup({title:'두 왕국의 갈등', sub:'외교', body:`${a.name}과(와) ${b.name} 사이에 분쟁이 생겼습니다. 둘 다 당신에게 중재를 요청합니다.`,
      opts:[
        {t:`${a.name}의 편을 든다`, d:`관계 +20/${b.name} -15`, f:()=>{chOp(a,c,20); chOp(b,c,-15); state.prestige+=5;}},
        {t:`${b.name}의 편을 든다`, d:`관계 +20/${a.name} -15`, f:()=>{chOp(b,c,20); chOp(a,c,-15); state.prestige+=5;}},
        {t:'중립을 지킨다', d:'위신 +8', f:()=>{state.prestige+=8;}},
      ]});
  }},
  {cond:c=>true, w:2, run:c=>popup({title:'사절단 도착', sub:'외교', body:'해외에서 온 낯선 사절단이 왕좌 앞에 섰습니다. 그들은 교역 협정을 원합니다.', opts:[{t:'환대하며 협정을 맺는다', d:'금 +50, 위신 +10', f:()=>{REGIONS[c.region].gold+=50; state.prestige+=10;}}, {t:'거절한다', d:'위신 -5', f:()=>{state.prestige=Math.max(0,state.prestige-5);}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'귀족의 결투 요청', sub:'궁정', body:'영지의 한 귀족이 결투를 신청해왔습니다. 거절하면 비겁자로 낙인찍힐 수 있습니다.', opts:[{t:'직접 결투에 응한다', d:'위신 +15, 부상 위험', f:()=>{state.prestige+=15; if(Math.random()<0.25){addStress(c,20);}else{addStress(c,-5);}}}, {t:'대리인을 세운다', d:'위신 -5, 스트레스 -5', f:()=>{state.prestige=Math.max(0,state.prestige-5); addStress(c,-5);}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'궁정 시인의 도전', sub:'궁정', body:'방랑 시인이 당신의 궁정을 찾아와 즉흥시 대결을 제안합니다.', opts:[{t:'대결에 응한다', d:'위신 ±15', f:()=>{if(Math.random()<0.5){state.prestige+=15; addStress(c,-10);}else{state.prestige=Math.max(0,state.prestige-10); addStress(c,8);}}}, {t:'웃으며 거절한다', d:'스트레스 -3', f:()=>addStress(c,-3)}]})},
  {cond:c=>REGIONS[c.region].gold>100, w:2, run:c=>popup({title:'귀족의 청원 — 토지', sub:'궁정', body:'오랜 봉신 가문이 선대에 빼앗긴 땅을 돌려달라는 청원을 올렸습니다.', opts:[{t:'돌려준다', d:'금 -30, 관계 +25', f:()=>{REGIONS[c.region].gold-=30; const kt=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id); if(kt) chOp(kt,c,25);}}, {t:'거절한다', d:'관계 -10, 위신 +5', f:()=>{state.prestige+=5; const kt=Object.values(chars).find(k=>!k.dead&&k.courtOf===c.region&&k.id!==c.id); if(kt) chOp(kt,c,-10);}}]});
  }},
  {cond:c=>true, w:1, run:c=>popup({title:'고문관의 조언', sub:'궁정', body:'경험 많은 고문관이 찾아와 조언합니다. "공포보다 사랑으로 다스리는 것이 오래갑니다."', opts:[{t:'귀담아듣는다', d:'스트레스 -8', f:()=>addStress(c,-8)}, {t:'일축한다', d:'스트레스 +5', f:()=>addStress(c,5)}]})},
  {cond:c=>state.month>=3&&state.month<=5, w:2, run:c=>popup({title:'봄비', sub:'계절', body:'오랜 봄비가 내립니다. 들판이 초록으로 물들고 농민들의 얼굴에 기대가 피어납니다.', opts:[{t:'풍요로운 봄을 기원한다', d:'민심 +5, 스트레스 -5', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5); addStress(c,-5);}}]})},
  {cond:c=>state.month>=6&&state.month<=8, w:2, run:c=>popup({title:'여름 가뭄', sub:'계절', body:'6월부터 비가 내리지 않았습니다. 강이 줄어들고 농작물이 타들어갑니다.', opts:[{t:'저수지를 만든다', d:'금 -60', f:()=>{REGIONS[c.region].gold-=60; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+8);}}, {t:'기우제를 올린다', d:'민심 +5', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5);}}]})},
  {cond:c=>state.month>=9&&state.month<=11, w:2, run:c=>popup({title:'수확 철', sub:'계절', body:'가을 수확이 끝났습니다. 올해 농사는 나쁘지 않았습니다.', opts:[{t:'잔치를 벌인다', d:'금 -30, 민심 +10, 스트레스 -8', f:()=>{REGIONS[c.region].gold-=30; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+10); addStress(c,-8);}}, {t:'비축해둔다', d:'금 +20', f:()=>{REGIONS[c.region].gold+=20;}}]})},
  {cond:c=>state.month<=2||state.month>=12, w:2, run:c=>popup({title:'한겨울 밤', sub:'계절', body:'밤이 너무 깁니다. 난롯가에 앉아 에이레 통일에 대한 꿈을 생각합니다.', opts:[{t:'전략을 다듬는다', d:'위신 +5, 스트레스 -5', f:()=>{state.prestige+=5; addStress(c,-5);}}, {t:'그냥 잠에 든다', d:'스트레스 -10', f:()=>addStress(c,-10)}]})},
  {cond:c=>true, w:1, run:c=>popup({title:'개기 월식', sub:'자연', body:'밤하늘의 달이 붉게 물들었습니다. 백성들이 두려워하며 수군거립니다.', opts:[{t:'신의 뜻이라 선포한다', d:'위신 +10, 민심 +5', f:()=>{state.prestige+=10; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+5);}}, {t:'과학적으로 설명한다', d:'스트레스 -3', f:()=>addStress(c,-3)}]})},
  {cond:c=>Object.values(chars).filter(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=16&&!k.ruler).length>0, w:2, run:c=>{
    const adult=Object.values(chars).find(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=16&&!k.ruler); if(!adult) return;
    popup({title:'자식의 앞날', sub:'가문', body:`${adult.name}(${age(adult)}세)이(가) 자신만의 영지를 원합니다. 야망이 보입니다.`, opts:[{t:'독립적인 역할을 준다', d:'관계 +20, 위신 +5', f:()=>{chOp(adult,c,20); state.prestige+=5;}}, {t:'곁에 두고 교육한다', d:'관계 -5, 스탯 +', f:()=>{chOp(adult,c,-5); adult.base.dip=Math.min(15,adult.base.dip+1);}}]});
  }},
  {cond:c=>Object.values(chars).filter(k=>!k.dead&&k.dyn===c.dyn&&k.id!==c.id).length>=3, w:1, run:c=>popup({title:'가문의 연회', sub:'왕조', body:'우어 브리언 가문의 친척들이 모였습니다. 연회는 밤새 이어졌습니다.', opts:[{t:'성대하게 열었다', d:'금 -40, 위신 +15', f:()=>{REGIONS[c.region].gold-=40; state.prestige+=15; addStress(c,-10);}}, {t:'소박하게 치렀다', d:'금 -15, 스트레스 -5', f:()=>{REGIONS[c.region].gold-=15; addStress(c,-5);}}]})},
  {cond:c=>age(c)>=35&&!c.spouse, w:2, run:c=>popup({title:'후계자 걱정', sub:'가문', body:'후계자 없이 당신이 죽으면 우어 브리언 왕조가 끊깁니다. 혼인을 서둘러야 합니다.', opts:[{t:'혼처를 알아보겠다', f:()=>{}}, {t:'아직 시간이 있다', d:'스트레스 +5', f:()=>addStress(c,5)}]})},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=3&&age(k)<10), w:2, run:c=>{
    const child=Object.values(chars).find(k=>!k.dead&&(k.father===c.id||k.mother===c.id)&&age(k)>=3&&age(k)<10); if(!child) return;
    popup({title:'아이의 첫 말', sub:'가문', body:`${child.name}(${age(child)}세)이(가) 처음으로 진지하게 물었습니다.\n"아버지, 언젠가 저도 왕이 될 수 있나요?"`, opts:[{t:'"그래, 열심히 하면."', d:'관계 +10, 스트레스 -8', f:()=>{chOp(child,c,10); addStress(c,-8);}}, {t:'"왕이 되려면 많이 배워야 한단다."', d:'교육점수 +2', f:()=>{child.eduScore+=2; chOp(child,c,5);}}]});
  }},
  {cond:c=>Object.values(chars).some(k=>!k.dead&&k.spouse===c.id&&chars[k.spouse]), w:2, run:c=>{
    const sp=chars[c.spouse]; if(!sp||sp.dead) return; if(Math.random()<0.5) return;
    popup({title:'배우자와의 논쟁', sub:'가문', body:`${sp.name}이(가) 당신의 최근 결정에 의문을 제기했습니다. 분위기가 어색해졌습니다.`, opts:[{t:'솔직하게 대화한다', d:'스트레스 -8, 관계 +8', f:()=>{addStress(c,-8); chOp(sp,c,8);}}, {t:'고집을 꺾지 않는다', d:'스트레스 +5, 관계 -5', f:()=>{addStress(c,5); chOp(sp,c,-5);}}]});
  }},
  {cond:c=>c.lifestyle=='dip'&&stat(c,'dip')>=8, w:2, run:c=>popup({title:'협상의 달인', sub:'외교 인생관', body:'협상 테이블에서 당신은 점점 강해지고 있습니다.', opts:[{t:'기술을 연마한다', d:'경험 +20', f:()=>{c.lifeXP+=20; addStress(c,-5);}}]})},
  {cond:c=>c.lifestyle=='mar'&&stat(c,'mar')>=8, w:2, run:c=>popup({title:'전략의 통찰', sub:'무예 인생관', body:'분석을 통해 새로운 전략적 통찰을 얻었습니다.', opts:[{t:'훈련에 적용한다', d:'병력 +50, 경험 +20', f:()=>{c.lifeXP+=20; REGIONS[c.region].troops+=50;}}]})},
  {cond:c=>c.lifestyle=='stew'&&REGIONS[c.region].gold>200, w:2, run:c=>popup({title:'재정 혁신', sub:'내정 인생관', body:'새로운 세금 체계를 도입할 수 있습니다. 단기적 불만이 따릅니다.', opts:[{t:'도입한다', d:'민심 -8, 금 +50', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-8); REGIONS[c.region].gold+=50; c.lifeXP+=20;}}, {t:'현상 유지', d:'경험 +10', f:()=>{c.lifeXP+=10;}}]})},
  {cond:c=>c.lifestyle=='intr'&&stat(c,'intr')>=7, w:2, run:c=>popup({title:'그림자 네트워크', sub:'음모 인생관', body:'밀정 네트워크가 예상치 못한 정보를 가져왔습니다.', opts:[{t:'정보를 활용한다', d:'경험 +20', f:()=>{state.prestige+=10; c.lifeXP+=20;}}, {t:'덮어둔다', d:'경험 +10', f:()=>{c.lifeXP+=10;}}]})},
  {cond:c=>c.lifestyle=='learn'&&stat(c,'learn')>=7, w:2, run:c=>popup({title:'새로운 이론', sub:'학문 인생관', body:'연구 끝에 새로운 농업 기술을 발견했습니다.', opts:[{t:'적용한다', d:'민심 +8, 금 +40, 경험 +20', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+8); REGIONS[c.region].gold+=40; c.lifeXP+=20;}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'리머릭 시장', sub:'내정', body:'상인들이 몰려오고 있습니다. 시장세를 올릴 기회입니다.', opts:[{t:'시장세를 올린다', d:'금 +60, 민심 -8', f:()=>{REGIONS[c.region].gold+=60; REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-8);}}, {t:'그대로 둔다', d:'민심 +3', f:()=>{REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+3);}}]})},
  {cond:c=>REGIONS[c.region].gold>80, w:1, run:c=>popup({title:'항구 확장', sub:'내정', body:'킨세일 항구를 확장하면 더 많은 교역선이 들어올 수 있습니다.', opts:[{t:'투자한다', d:'금 -80', f:()=>{REGIONS[c.region].gold-=80; log('항구 확장이 완료됐습니다.','good');}}, {t:'다음으로 미룬다', d:'—', f:()=>{}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'도둑 무리', sub:'영지', body:'국경 지대에 도둑 무리가 출몰해 상인들이 피해를 보고 있습니다.', opts:[{t:'기사를 보내 소탕한다', d:'금 -25, 민심 +10', f:()=>{REGIONS[c.region].gold-=25; REGIONS[c.region].pop=Math.min(100,(REGIONS[c.region].pop||60)+10);}}, {t:'지역민에게 맡긴다', d:'민심 -5', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-5);}}]})},
  {cond:c=>true, w:2, run:c=>popup({title:'이단 설교자', sub:'신앙', body:'교리에 반하는 설교를 하는 자가 나타났습니다.', opts:[{t:'체포해 주교에게 넘긴다', d:'위신 +8, 민심 -5', f:()=>{state.prestige+=8; REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-5);}}, {t:'무시한다', d:'민심 -8', f:()=>{REGIONS[c.region].pop=Math.max(0,(REGIONS[c.region].pop||60)-8);}}]})},
  {cond:c=>true, w:1, run:c=>popup({title:'로마에서 온 소식', sub:'신앙', body:'교황청에서 십자군 소집령이 떨어졌다는 소식이 들려왔습니다.', opts:[{t:'참여를 고려한다', d:'위신 +5', f:()=>{state.prestige+=5;}}, {t:'에이레가 우선이다', d:'스트레스 -3', f:()=>addStress(c,-3)}]})},
  {cond:c=>stressLvl(c)>=1, w:4, run:c=>popup({title:'악몽', sub:'내면', body:'같은 악몽이 반복됩니다. 새벽에 식은땀을 흘리며 깨어납니다.', opts:[{t:'사제에게 고해성사를 한다', d:'스트레스 -25', f:()=>addStress(c,-25)}, {t:'혼자 버텨낸다', d:'스트레스 +10', f:()=>addStress(c,10)}]})},
  {cond:c=>stressLvl(c)>=2, w:5, run:c=>popup({title:'정신 붕괴 직전', sub:'내면', body:'모든 것이 무너질 것 같습니다. 이대로는 안 됩니다.', opts:[{t:'업무를 중단하고 쉰다', d:'스트레스 -50, 금 -30', f:()=>{addStress(c,-50); REGIONS[c.region].gold-=30;}}, {t:'술에 기댄다', d:'스트레스 -30', f:()=>{addStress(c,-30);}}, {t:'신에게 매달린다', d:'스트레스 -20, 위신 +5', f:()=>{addStress(c,-20); state.prestige+=5;}}]})},
];

function randomEventPulse(){
  if(state.over) return;
  const c=playerChar(); if(!c||c.dead||!c.region||!REGIONS[c.region]) return;
  if(Math.random()<0.18){
...
function openBuildMenu(bid){
  ...
  opts.push({t:'취소'});
  showModal({title:`${b.n} 건설`, sub:`슬롯 ${b.slots-(existing.length+(b.building_queue?1:0))}/${b.slots} · 금 ${Math.round(seatGold)}`, body:'건설할 건물을 선택하세요.', opts});
  pause();
}
...
function openRegion(rid, cid_hint){
  ...
  const c=ownerOf(rid); if(!c) return;

  if(c.id===p.id){
    const cid=cid_hint||countyOf(rid); const cnt=COUNTIES[cid];
    ...
    showModal({title:dispName, sub:'내 영지', body:'',
  ...
  if(!atWar){
    ...
    if(!isAllied(p.id,c.id)){
      ...
      opts.push({t:'동맹 파기', d:'관계 -40, 5년 휴전', f:()=>{ breakAlliance(p.id,c.id); }});
    }
    if(c.liege!==p.id){
      opts.push({t:'봉신 요청', d:`수락 가능성: ${vassalChance(c,p)}%`, f:()=>{
        if(Math.random()*100<vassalChance(c,p)){
          c.liege=p.id; c.ruler=true; chOp(c,p,20);
          log(`<b>${c.name}</b>이(가) 봉신을 수락했습니다!`,'good'); checkVictoryHint();
        } else { chOp(c,p,-20); log(`<b>${c.name}</b>이(가) 봉신 요청을 거절했습니다.`,'dip'); }
      }});
    }
    opts.push({t:'혼인 교섭', d:'가문 간 혼약 — 관계 +30', f:()=>tryMarriage(c)});
    opts.push({t:'살해 모략', f:()=>{ if(startScheme(p,c)) log('어둠 속에서 칼을 갈기 시작합니다...','war'); }});
    if(!truce){
      if(isAllied(p.id,c.id)){ opts.push({t:'동맹 파기 후 선전포고 가능', d:'먼저 동맹을 파기하세요', f:()=>{}}); }
      else {
        const myClaims=claimsForRegion(c);
        opts.push({t:myClaims.length>0?`선전포고 (명분 ${myClaims.length}개)`:'선전포고 (명분 없음)', d:`전력 ${Math.round(power(p))} vs ${Math.round(power(c))}`, f:()=>{ closePanel('court'); closePanel('dec'); openDeclareWar(c.id); }});
      }
    } else { opts.push({t:'휴전 중', d:'전쟁 불가', f:()=>{}}); }
  } else { opts.push({t:'교전 중', f:()=>{}}); }
  opts.push({t:'닫기'});
...
function allianceChance(c,p){
  let v=30+opinion(c,p); v+=aiW(c,'soc')*5; if(power(p)>power(c)) v+=10; v+=prestigeBonus();
  return Math.max(2,Math.min(95,Math.round(v)));
}
function vassalChance(c,p){
  let v=-20+opinion(c,p); const ratio=power(p)/power(c); v+=ratio>2?45:ratio>1.5?25:ratio>1.2?10:-20; v-=aiW(c,'bold')*6; if(c.traits.includes('content'))v+=15; if(c.traits.includes('craven'))v+=15; if(c.traits.includes('ambitious'))v-=20; v+=Math.round(prestigeBonus()*1.5);
  return Math.max(1,Math.min(85,Math.round(v)));
}
function doMarriage(candidate, c){
  const p=playerChar(); const acc=40+opinion(c,p)+aiW(c,'soc')*4+prestigeBonus();
  if(Math.random()*100<acc){
    ...
    if(age(candidate)>=16){ candidate.spouse=sp.id; sp.spouse=candidate.id; }
    chOp(c,p,30); chOp(p,c,30);
    log(`<b>${candidate.name}</b>과(와) ${c.dyn} 가문의 혼약 성사.`,'good');
...
function tryMarriage(c){
  ...
  const opts=candidates.map(kid=>({
    t:kid.id===p.id ? `${kid.name} (본인 · ${age(kid)}세)` : `${kid.name} (${kid.sex==='m'?'아들':'딸'} · ${age(kid)}세)`,
    f:()=>doMarriage(kid.id===p.id?p:kid, c)
  }));
  opts.push({t:'그만둔다'});
  showModal({title:'혼인 교섭', sub:`${c.dyn} 가문과의 혼담`, body:'누구의 혼처를 알아보시겠습니까?', opts});
}
...
function claimsForRegion(def){
  ...
  return state.claims.filter(c=>defCids.includes(c.rid));
}
function claimName(rid){ return COUNTIES[rid]?.n||BARONIES[rid]?.n||rid; }
function addClaim(rid, type){
  if(hasClaim(rid)) return;
  state.claims.push({rid, type, obtained:state.year});
  log(`<b>${claimName(rid)}</b>에 대한 ${CB_TYPES[type].n}을(를) 획득했습니다.`, 'dip');
}
function removeClaim(rid){
  state.claims = state.claims.filter(c=>c.rid!==rid);
}
...
function openDeclareWar(defId){
  ...
  if(truceBetween(p.id, def.id)){
    showModal({title:'선전포고 불가', sub:'휴전 중', body:`${def.name}과(와) 현재 휴전 협정이 유지되고 있습니다.`, opts:[{t:'닫기'}]}); return;
  }
  if(isAllied(p.id, def.id)){
    showModal({title:'선전포고 불가', sub:'동맹국', body:`${def.name}은(는) 현재 동맹국입니다. 먼저 동맹을 파기하세요.`, opts:[{t:'동맹 파기', f:()=>{ breakAlliance(p.id,def.id); openDeclareWar(defId); }}, {t:'닫기'}]}); return;
  }
  const myClaims = claimsForRegion(def);
  if(myClaims.length === 0){
    showModal({title:'명분 없음', sub:'선전포고 불가', body:`${def.name}에 대한 명분이 없습니다.`, opts:[{t:'닫기'}]}); return;
  }
  const myPow = Math.round(power(p)); const defPow = Math.round(power(def));
  let html = `<div class="kv"><span>상대 전력</span><span>${defPow} vs 내 전력 ${myPow}</span></div><div class="kv"><span>위신</span><span>${state.prestige}</span></div>`;
  const opts = [];
  myClaims.forEach(cl=>{
    ...
    opts.push({
      ...
      d:`위신 ${cbInfo.cost} 소모`,
      f: canAfford ? ()=>{ state.prestige -= cbInfo.cost; if(cl.type==='unpressed') cl.type='pressed'; declareWar(p, def, rid); removeClaim(rid); } : ()=>{}
    });
  });
  opts.push({t:'취소'});
  showModal({title:`${def.name}에 선전포고`, body:'', html, opts});
}
...
const COUNCIL_ROLES = {
  ...
  chaplain:   { n:'사제',     skill:'learn', icon:'✝', desc:'민심 안정 안정' },
};
    ...
    if(role==='chancellor'){
      ...
      if(Math.random()<0.08){
        if(positive){
          ...
          if(rnd){ chOp(rnd,p, Math.round(sk*0.8)); popup({title:'재상의 외교', body:`${c.name}이(가) 인접 왕국과의 관계를 개선했습니다.`, opts:[{t:'잘 했다'}]}); }
        } else { state.prestige=Math.max(0,state.prestige-10); }
      }
    }
    if(role==='marshal'){
      reg.troops = Math.min(reg.cap, reg.troops + Math.round(sk*1.5));
      if(Math.random()<0.08 && positive){ popup({title:'원수의 훈련 보고', body:`병사들의 훈련 성과가 좋습니다.`, opts:[{t:'격려한다'}]}); }
    }
    if(role==='steward'){
      reg.gold += Math.round(sk * 1.2);
      if(Math.random()<0.08 && positive){ reg.gold += (30 + sk*4); }
    }
    if(role==='spymaster'){
      state.schemes.forEach(s=>{ if(s.target===p.id) s.defBonus=(s.defBonus||0)+sk*2; });
    }
    if(role==='chaplain'){
      reg.pop = Math.min(100, (reg.pop||60) + Math.round(sk*0.15)); state.prestige += Math.round(sk * 0.3);
    }
  }
}
...
function renderCourt(){
  ...
  const roleBonus={
    chancellor: cid=>{ const sk=stat(chars[cid],'dip'); return `위신 +${Math.round(sk*0.4)}/월`; },
    marshal:    cid=>{ const sk=stat(chars[cid],'mar'); return `병력 +${Math.round(sk*1.5)}/월`; },
    steward:    cid=>{ const sk=stat(chars[cid],'stew'); return `금 +${Math.round(sk*1.2)}/월`; },
    spymaster:  cid=>{ const sk=stat(chars[cid],'intr'); return `모략 방어 +${sk*2}`; },
    chaplain:   cid=>{ const sk=stat(chars[cid],'learn'); return `민심 +${(sk*0.15).toFixed(1)}/월`; },
  };
  ...
  for(const role in COUNCIL_ROLES){
    ...
    html+=`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px dotted #2c2316">
      <span style="font-size:1.1rem;width:22px;text-align:center">${rinfo.icon}</span>
      <div style="flex:1">
        <div style="font-size:.82rem;color:var(--parch)">${rinfo.n}</div>
      </div>
      <div style="text-align:right">`;

    if(councilor){
      html+=`<div style="font-size:.8rem;color:var(--gold)">${councilor.name}</div>
        <button class="p-action" style="margin:3px 0 0;padding:2px 8px;font-size:.7rem" onclick="appointCouncilor('${role}',null)">해임</button>`;
    } else {
      html+=`<div style="font-size:.75rem;color:var(--parch-dim)">공석</div>`;
      if(candidates.length){
        html+=`<select style="background:#191309;color:var(--parch);font-size:.72rem;" onchange="if(this.value)appointCouncilor('${role}',this.value)"><option value="">임명...</option>`;
        candidates.forEach(c=>{ html+=`<option value="${c.id}">${c.name}</option>`; });
        html+=`</select>`;
      }
    }
    html+=`</div></div>`;
...
let _decActs=[];
function renderDec(){
  const p=playerChar(); if(!p||!REGIONS[p.region]) return; const n=playerRegions().length;
  _decActs=[]; const items=[];
  ...
  if(n>=4&&!state.kingdomFormed){
    addDec('⚜ 아일랜드 왕국 선포',`금 250 필요`, REGIONS[p.region].gold>=250, ()=>{
      REGIONS[p.region].gold-=250; state.kingdomFormed=true; state.prestige+=200; closePanel('dec');
    });
  }
  addDec('연회를 연다',`금 60`, REGIONS[p.region].gold>=60, ()=>{ REGIONS[p.region].gold-=60; addStress(p,-20,'연회'); renderDec(); });

  let html = ``;
  items.forEach(it=>{ html+=`<button class="p-action${it.enabled?'':' off'}" onclick="_decActs[${it.i}]()">${it.t}</button>`; });
  document.getElementById('decContent').innerHTML=html;
}
function openDecisions(){ togglePanel('dec'); }

function victory(){
  state.victory=true; state.over=true; pause(); const p=playerChar();
  showModal({title:'에이레의 하이킹', sub:'승리', body:`브리언 보루 이후 처음으로, 에이레 전토가 한 사람의 깃발 아래 통일되었습니다.`, opts:[{t:'새 연대기', f:()=>location.reload()}]});
}
...
function renderChar(){
  ...
  document.getElementById('stressFill').style.width=Math.min(100,c.stress/1.5)+'%';
}
function renderMap(){
  ...
  for(const cid in COUNTY_ADJ){
    for(const nb of (COUNTY_ADJ[cid]||[])){
      const k=[cid,nb].sort().join('|'); if(drawn.has(k)) continue; drawn.add(k);
      const A=COUNTIES[cid],B=COUNTIES[nb]; if(!A||B==null) continue;
      h+=`<line class="edge" x1="${A.x}" y1="${A.y}" x2="${B.x}" y2="${B.y}"/>`;
    }
  }
  for(const cid in COUNTIES){
    const C=COUNTIES[cid]; const holder=countyHolder(cid); const mine=holder&&holder.id===p.id;
    const col=mine?'#3d6b4a':'#6a5836';
    h+=`<g class="node" onclick="openCounty('${cid}')">
      <circle class="body" cx="${C.x}" cy="${C.y}" r="18" fill="${col}" stroke="#6a5836"/>
