...
let audioCtx = null;
let _decActs = []; // 엄격 모드(Strict Mode) 변수 할당 에러 방지용 선언
...
function playSynthSFX(type) {
  ...
  switch(type) {
    ...
    case 'gold': // 국고 소비, 건물 건설, 선물 배정 등 금화 소리
    ...
    case 'death': // 영주 노환 및 암살 성공 장송 종소리
  ...
  if(c.id===state.player){
    ...
    if(dist){
      ...
      popup({title:'왕은 죽었다', sub:'계승',
        body:`${c.name}의 시대가 끝났습니다. (${cause})\n이제 <b>${mainH.name}</b>이(가) 통치를 승계합니다.${splitMsg}`,
        opts:[{t:'왕은 만세하리라'}]});
    } else {
...
function openRegion(rid, cid_hint){
  ...
  if(c.id===p.id){ 
    ...
    const bldgs=cnt?cnt.baronies.flatMap(b=>BARONIES[b]?.buildings?.map(x=>BUILDING_TYPES[x]?.n||x)||[]).filter(Boolean).join('·')||'없음':'없음';
    showModal({title:dispName, body:`총 동원력: ${totalTroops} · 금고 수지: ${Math.round(BARONIES[rid].gold)}\n완공 건물: ${bldgs}`, opts:[{t:'닫기'}]}); return; 
  }
  const op=opinion(c,p); const atWar=state.wars.some(w=>(w.atkexport===p.id&&w.def===c.id)||(w.atk===c.id&&w.def===p.id)); const truce=truceBetween(p.id,c.id);
...
function renderMap(){
  ...
  for(const cid in COUNTIES){ 
    const C=COUNTIES[cid]; const holder=countyHolder(cid); const mine=holder&&holder.id===p.id; const isVassalOf=holder&&holder.liege===p.id; const col=mine?'#3d6b4a':isVassalOf?'#4a7a55':(DUCHIES[C.duchy]?.color||#555); const rad=mine?22:18; const underSiege=state.wars.some(w=>w.targetRid===cid&&w.occupied?.length>0); 
    const bids=C.baronies; const totalT=bids.reduce((s,b)=>s+(BARONIES[b]?.troops||0),0); const avgPop=Math.round(bids.reduce((s,b)=>s+(BARONIES[b]?.pop||60),0)/bids.length);
    h+=`<g class="node" onclick="openCounty('${cid}')">
      <circle class="body" cx="${C.x}" cy="${C.y}" r="${rad}" fill="${col}" stroke="${underSiege?'#c83030':'#6a5836'}"/>
