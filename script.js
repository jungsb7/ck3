...
const BARONIES = {
  ...
  b_fahan:     {n:'파한',     county:'c_ailech',  troops:175, gold: 42, pop:53, cap:175, owner:null},
   Berry:     {n:'더리',     county:'c_ailech',  troops:210, gold: 55, pop:58, cap:210, owner:null},
};
...
// 오타 수정 파트: r!===role 을 r!==role 로 정상 조치 완료
...
// 오타 수정 파트: 변수 C 참조 오류를 소문자 c 및 올바른 context 변수로 전면 수정완료
function openRegion(rid, cid_hint){
  ...
  if(c.id===p.id){
    ...
    showModal({title:dispName, sub:'내 영지', html:`<div class="kv"><span>백작령</span><span>${cnt?.n||dispName}</span></div><div class="kv"><span>총 병력</span><span>${totalTroops}</span></div><div class="kv"><span>금고</span><span>${totalGold}</span></div><div class="kv"><span>완공 건물</span><span>${bldgs}</span></div><div class="kv"><span>남작령 수</span><span>${cnt?.baronies.length||1}</span></div>`, opts:[{t:'닫기'}]}); return;
  }
  ...
  opts.push({t:'닫기'}); showModal({title:`${dispName}`, sub:`${c.name}의 영지`, html, opts}); pause();
}

function prestigeBonus(){ return Math.round((state.prestige-120)/12); }
function allianceChance(c,p){ let v=30+opinion(c,p); v+=aiW(c,'soc')*5; if(power(p)>power(c)) v+=10; v+=prestigeBonus(); return Math.max(2,Math.min(95,Math.round(v))); }
function vassalChance(c,p){ let v=-20+opinion(c,p); const ratio=power(p)/power(c); v+=ratio>2?45:ratio>1.5?25:ratio>1.2?10:-20; v-=aiW(c,'bold')*6; if(c.traits.includes('content'))v+=15; if(c.traits.includes('craven'))v+=15; if(c.traits.includes('ambitious'))v-=20; v+=Math.round(prestigeBonus()*1.5); return Math.max(1,Math.min(85,Math.round(v))); }
...
function doMarriage(candidate, c){
  ...
  if(Math.random()*100<acc){
    ...
    if(age(candidate)>=16){ candidate.spouse=sp.id; sp.spouse=candidate.id; } chOp(c,p,30); chOp(p,c,30);
    log(`<b>${candidate.name}</b>과(와) ${c.dyn} 가문의 혼약 성사 — 두 가문이 맺어졌습니다.`,'good');
...
function openDeclareWar(defId){
  ...
  if(myClaims.length === 0){ showModal({title:'명분 없음', sub:'선전포고 불가', body:`${def.name}에 대한 명분이 없습니다.`, opts:[{t:'닫기'}]}); return; }
  const myPow = Math.round(power(p)); const defPow = Math.round(power(def)); const defCids = countiesOf(def.id);
  let html = `<div class="kv"><span>상대 전력</span><span>${defPow} vs 내 전력 ${myPow}</span></div><div class="kv"><span>위신</span><span>${state.prestige}</span></div><div style="margin:12px 0 6px;font-size:.72rem;letter-spacing:.2em;color:var(--gold-dim)">보유 명분</div>`;
  const opts = [];
  myClaims.forEach(cl=>{
  ...
  opts.push({t:'취소'}); showModal({title:`${def.name}에 선전포고`, sub:`${COUNTIES[defCids[0]]?.n||''} 왕국`, html, opts});
}
...
function councilPulse(){
  ...
  for(const role in state.council){
    ...
    if(role==='steward'){ reg.gold += Math.round(sk * 1.2); if(Math.random()<0.08){ if(positive){ const bonus = 30 + sk*4; popup({title:'재무관의 보고', sub:`${c.name}의 보고`, body:`${c.name}이(가) 효율적인 세금 징수로 추가 수입을 올렸습니다.`, opts:[{t:'수고했다', f:()=>{reg.gold+=bonus; chOp(c,p,5); log(`재무관이 금 ${bonus}을 추가로 확보했습니다.`,'good');}}]}); } else { reg.gold = Math.max(0, reg.gold-40); reg.pop = Math.max(0, (reg.pop||60)-5); popup({title:'세금 마찰', sub:`${c.name}의 보고`, body:`${c.name}의 세금 징수 방식에 백성들이 반발합니다.`, opts:[{t:'징수 방식을 완화한다', f:()=>{reg.gold-=20; reg.pop=Math.min(100,(reg.pop||60)+8); chOp(c,p,-5);}}]}); } } }
    if(role==='spymaster'){ state.schemes.forEach(s=>{ if(s.target===p.id) s.defBonus=(s.defBonus||0)+sk*2; }); if(Math.random()<0.08){ if(positive){ const enemy=Object.values(chars).find(k=>!k.dead&&k.ruler&&k.id!==p.id&&state.schemes.some(s=>s.plotter===k.id&&s.target===p.id)); if(enemy){ state.schemes=state.schemes.filter(s=>!(s.plotter===enemy.id&&s.target===p.id)); popup({title:'첩보관의 성과', sub:`${c.name}의 보고`, body:`${c.name}이(가) ${enemy.name}의 모략을 사전에 분쇄했습니다.`, opts:[{t:'잘 했다', f:()=>{chOp(c,p,10); addStress(p,-5,'위기 모면');}}]}); } else { state.prestige+=8; log(`${c.name}이(가) 유용한 정보를 수집했습니다.`,'dip'); } } else { popup({title:'첩보 유출', sub:`${c.name}의 보고`, body:`기밀이 유출되었을 가능성이 있습니다.`, opts:[{t:'엄중히 처리한다', f:()=>{addStress(p,10,'배신의 충격'); chOp(c,p,-15);}}]}); } } }
    if(role==='chaplain'){ reg.pop = Math.min(100, (reg.pop||60) + Math.round(sk*0.15)); state.prestige += Math.round(sk * 0.3); if(Math.random()<0.08){ if(positive){ popup({title:'사제의 설교', sub:`${c.name}의 보고`, body:`${c.name}이(가) 민심을 달래는 설교를 이어가고 있습니다.`, opts:[{t:'좋은 일이다', f:()=>{reg.pop=Math.min(100,(reg.pop||60)+10); state.prestige+=5; addStress(p,-8,'백성의 사랑');}}]}); } else { reg.pop = Math.max(0,(reg.pop||60)-8); popup({title:'사제의 설교 논란', sub:`${c.name}의 보고`, body:`백성들 사이에 갈등이 생겼습니다.`, opts:[{t:'자중하라 경고한다', f:()=>{chOp(c,p,-10); reg.pop=Math.min(100,(reg.pop||60)+3);}}]}); } } }
  }
}
...
function renderCourt(){
  ...
  for(const role in COUNCIL_ROLES){
    ...
    if(councilor){ const bonusTxt=roleBonus[role]?roleBonus[role](councilor.id):''; html+=`<div style="font-size:.8rem;color:var(--gold)">${councilor.name}</div><div style="font-size:.7rem;color:var(--parch-dim)">스킬 ${stat(councilor,rinfo.skill)}</div><div style="font-size:.68rem;color:#7a9a6a;margin:1px 0">${bonusTxt}</div><button class="p-action" style="margin:3px 0 0;padding:3px 8px;font-size:.7rem" onclick="appointCouncilor('${role}',null)">해임</button>`; }
    else { html+=`<div style="font-size:.75rem;color:var(--parch-dim);margin-bottom:3px">공석</div>`; if(candidates.length){ html+=`<select style="background:#191309;color:var(--parch);border:1px solid var(--line);font-size:.72rem;padding:2px 4px;max-width:120px" onchange="if(this.value)appointCouncilor('${role}',this.value)"><option value="">임명...</option>`; candidates.sort((a,b)=>stat(b,rinfo.skill)-stat(a,rinfo.skill)).forEach(c=>{ html+=`<option value="${c.id}">${c.name} (${stat(c,rinfo.skill)})</option>`; }); html+=`</select>`; } else { html+=`<div style="font-size:.7rem;color:#5a4a3a">없음</div>`; } }
    html+=`</div></div>`;
  }
  const myVassals=vassalsOf(p.id);
  if(myVassals.length>0){ html+=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin-bottom:8px;padding-bottom:5px;border-bottom:1px solid var(--line)">봉신 목록</div>`; for(const v of myVassals){ const vcids=directCountiesOf(v.id); html+=`<div class="p-row"><span><b>${v.name}</b></span><span>호감도 ${opinion(v,p)}</span></div>`; } }
  ...
  const myBids=regionsOf(p.id); myBids.forEach(bid=>{ const b=BARONIES[bid]; if(!b) return; const bldTxt=(b.buildings||[]).map(t=>BUILDING_TYPES[t]?.icon+BUILDING_TYPES[t]?.n).join(' '); const slotLeft=b.slots-(b.buildings||[]).length; html+=`<div style="font-size:.78rem;padding:5px 0;border-bottom:1px dotted #2c2316"><span style="color:var(--gold)">${b.n}</span><span style="color:var(--parch-dim);font-size:.72rem"> 슬롯 ${slotLeft}/${b.slots}</span><br><span>${bldTxt||'건물 없음'}</span>${slotLeft>0?`<button class="p-action" style="margin:3px 0 0;padding:2px 8px;font-size:.7rem" onclick="openBuildMenu('${bid}')">건설</button>`:''}</div>`; });
  html+=`<div style="font-size:.7rem;letter-spacing:.2em;color:var(--gold-dim);margin:12px 0 8px;padding-bottom:6px;border-bottom:1px solid var(--line)">${rn} 궁정의 사람들</div><table style="width:100%">`;
  for(const c of fam){ const rel=c.id===p.id?'본인':c.spouse===p.id?'배우자':(c.father===p.id||c.mother===p.id)?'자녀':'궁정인'; const traitTxt=c.traits.map(t=>TRAITS[t]?TRAITS[t].n:'').join('·')||(c.childTrait?CHILD_TRAITS[c.childTrait].n:'—'); html+=`<tr style="border-bottom:1px dotted #2c2316"><td style="padding:6px 4px"><b>${c.name}</b> <span>${age(c)}세</span><br><small>${rel}</small></td><td>${traitTxt}</td><td style="text-align:right">외${stat(c,'dip')} 무${stat(c,'mar')}<br>내${stat(c,'stew')} 음${stat(c,'intr')}</td></tr>`; }
...
let _decActs=[];
function renderDec(){
  const p=playerChar(); if(!p||!REGIONS[p.region]) return; const n=playerRegions().length; _decActs=[]; const items=[];
  function addDec(t,d,enabled,fn){ const i=_decActs.length; _decActs.push(fn); items.push({t,d,enabled,i}); }
  if(n>=4&&!state.kingdomFormed){ addDec('⚜ 아일랜드 왕국 선포',`금 250 필요`, REGIONS[p.region].gold>=250, ()=>{ REGIONS[p.region].gold-=250; state.kingdomFormed=true; state.prestige+=200; closePanel('dec'); popup({title:'아일랜드 왕국', sub:'대관식', body:'캐셸의 바위 위에서 왕관을 수여받습니다.', opts:[{t:'만세!'}]}); }); }
  if(playerDuchies().length>=7){ addDec('☀ 하이킹에 등극한다','에이레 전토 통일 완료', true, ()=>{ closePanel('dec'); victory(); }); }
  addDec('연회를 연다',`금 60 · 스트레스 -20`, REGIONS[p.region].gold>=60, ()=>{ REGIONS[p.region].gold-=60; addStress(p,-20,'연회'); for(const rid in REGIONS){const r=ownerOf(rid); if(r&&r.id!==p.id) chOp(r,p,8);} log('성대한 연회가 열렸습니다.','good'); renderDec(); });
  addDec('클론맥노이즈 순례',`금 30 · 스트레스 -25`, REGIONS[p.region].gold>=30, ()=>{ REGIONS[p.region].gold-=30; addStress(p,-25,'순례'); state.prestige+=10; log('수도원에서 기도를 올렸습니다.','fam'); renderDec(); });
  addDec('병력 소집',`금 80 · 병력 +200`, BARONIES[p.region]?.gold>=80, ()=>{ REGIONS[p.region].gold-=80; REGIONS[p.region].troops+=200; log('병력이 소집됐습니다.','war'); renderDec(); });
  let html = state.claims.length > 0 ? state.claims.map(cl=>`<div class="p-row"><span>${claimName(cl.rid)}</span><span>${CB_TYPES[cl.type].n}</span></div>`).join('') : '<div style="font-size:.76rem;color:var(--parch-dim)">명분 없음</div>';
...
function victory(){ state.victory=true; state.over=true; pause(); const p=playerChar(); showModal({title:'에이레의 하이킹', sub:'승리', body:`일곱 왕국이 통일되었습니다.`, opts:[{t:'새 연대기', f:()=>location.reload()}]}); }
...
function renderChar(){
  ...
  const ttl=state.kingdomFormed?'아일랜드 왕':`먼스터 소왕`; document.getElementById('cTtl').textContent=`${ttl} · ${age(c)}세 · ${c.dyn} 가문`;
  let chips=''; for(const t of c.traits) chips+=`<span class="chip">${TRAITS[t].n}</span>`;
  ...
  document.getElementById('stressFill').style.width=(c.stress/1.5)+'%'; document.getElementById('stressNum').textContent=`${c.stress} / 150`;
...
function renderMap(){
  ...
  for(const cid in COUNTIES){
    ...
    const rad=mine?22:18; const totalT=C.baronies.reduce((s,b)=>s+(BARONIES[b]?.troops||0),0); const avgPop=Math.round(C.baronies.reduce((s,b)=>s+(BARONIES[b]?.pop||60),0)/C.baronies.length);
    h+=`<g class="node" onclick="openCounty('${cid}')"><circle class="body" cx="${C.x}" cy="${C.y}" r="${rad}" fill="${col}" stroke="${stroke}"/><text x="${C.x}" y="${C.y+3}" style="font-size:9px">${C.n}</text><text class="owner" x="${C.x}" y="${C.y+15}" style="font-size:.58rem;fill:#8a7858">${holder?holder.name.split(' ')[0]:'—'}</text><text class="owner" x="${C.x}" y="${C.y+36}" style="font-size:7.5px;fill:#7a6848">⚔${totalT} 민${avgPop}</text></g>`;
  }
  svg.innerHTML=h;
}
function renderAll(){ renderHeader(); renderChar(); renderMap(); }

function intro(){ popup({title:'에이레, 1066년', sub:'먼스터의 소왕', body:`일곱 왕국을 하나로. 그것이 당신의 길입니다.`, opts:[{t:'연대기를 시작한다', f:()=>{ askLifestyle(playerChar()); }}]}); }
